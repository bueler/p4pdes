...
PetscErrorCode Form2DFunctionLocal(DMDALocalInfo *info, double **au,
    ...
    PetscErrorCode ierr;
    int     i, j;
    double  xymin[2], xymax[2], hx, hy, cdiag, x, y, ue, uw, un, us;
    ierr = DMDAGetBoundingBox(info->da,xymin,xymax); CHKERRQ(ierr);
    hx = (xymax[0] - xymin[0]) / (info->mx - 1);
    hy = (xymax[1] - xymin[1]) / (info->my - 1);
    cdiag = 2.0 * (hy / hx + hx / hy);
    for (j = info->ys; j < info->ys + info->ym; j++) {
        y = xymin[1] + j * hy;
        for (i = info->xs; i < info->xs + info->xm; i++) {
            x = xymin[0] + i * hx;
            if (i==0 || i==info->mx-1 || j==0 || j==info->my-1) {
                aF[j][i] = cdiag * (au[j][i] - user->g_bdry(x,y,0.0,user));
            } else {
...
PetscErrorCode Form3DFunctionLocal(DMDALocalInfo *info, double ***au,
    ...
    PetscErrorCode ierr;
    int    i, j, k;
    double xyzmin[3], xyzmax[3], hx, hy, hz, dvol, cx, cy, cz, cdiag,
        ...
        for (j = info->ys; j < info->ys + info->ym; j++) {
            ...
            for (i = info->xs; i < info->xs + info->xm; i++) {
                ...
                if (   i==0 || i==info->mx-1
                    || j==0 || j==info->my-1
                    || k==0 || k==info->mz-1) {
                    aF[k][j][i] = cdiag * (au[k][j][i] - user->g_bdry(x,y,z,user));
                } else {
...
PetscErrorCode Form2DJacobianLocal(DMDALocalInfo *info, PetscScalar **au,
                                   Mat J, Mat Jpre, PoissonCtx *user) {
    PetscErrorCode  ierr;
    double       xymin[2], xymax[2], hx, hy, hyhx, hxhy, cdiag, v[5];
    int          i,j,ncols;
...
PetscErrorCode Form3DJacobianLocal(DMDALocalInfo *info, PetscScalar ***au,
                                   Mat J, Mat Jpre, PoissonCtx *user) {
    PetscErrorCode  ierr;
    double       xyzmin[3], xyzmax[3], hx, hy, hz, dvol, cx, cy, cz, cdiag, v[7];
    int          i,j,k,ncols;
