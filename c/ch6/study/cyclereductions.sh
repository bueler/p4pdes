...
# FIXME need to only test with initial conditions that satisfy the boundary
# conditions, as otherwise the initial residual norm is mostly the mismatch
# along the boundary

# notes:
#   * timings do not matter, but run with --with-debugging=0 configuration just
#     to get it done (timings given below)
...
COMMON="-fsh_dim 3 -fsh_problem manupoly -da_refine $LEV -snes_type ksponly -ksp_type richardson -ksp_max_it 1 -ksp_norm_type unpreconditioned -ksp_monitor -pc_type mg -mg_levels_${LEV}_ksp_converged_reason"

function runcase() {
  CMD="../fish $COMMON -pc_mg_type $1 -mg_levels_ksp_max_it $2 $3 $4"
  echo $CMD >> transcript.txt
  /usr/bin/time -f "real %e" $CMD &> tmp.txt
  cat tmp.txt >> transcript.txt
  grep "KSP Residual norm" tmp.txt | awk 'NF>1{print $NF}' >> residuals.txt  # extract number only
  rm -f tmp.txt
}

rm -f transcript.txt residuals.txt
for extra in "" "-fsh_init_random" "-fsh_cx 0.01 -fsh_cz 100.0" ; do
