...
\newcommand{\PETSCVERSION}{3.7\xspace}
...
% approach: construction needed before analysis; experts are confident about discretization, and proceed to discussions of performance, but we have built a number of concrete, documented examples before getting there
...
\ref{chap:ok}
    & \tblockeqncode{Stokes (2D)}{$\begin{matrix} -\grad^2 \bu + \grad p = 0 \\ \Div \bu = 0 \end{matrix}$}{stokes.c}
          &     &      \\ \bigskip
\ref{chap:pr}
    & \tblockcode{Poisson (2D,3D)}{\begin{tabular}{l} fish2.c \\ fish3.c\end{tabular}}    &     &     &      \\ \bigskip
\ref{chap:sc}
