...
\begin{equation}
    \kappa(A) = \|A\| \|A^{-1}\|
\end{equation}
is the \emph{condition number} of $A$; by definition $\kappa(A)=+\infty$ if $A$ is not invertible.  This quantity measures how perturbation-sensitive is the (exact) solution of a linear system $A \bv = \bb$ \citep{TrefethenBau1997}.


\section{Some facts of (numerical) life}

Our goal is to compute more interesting quantities than Euler's constant $e$---see Chapter \ref{chap:gs}---and at the core of many numerical PDE solutions is a finite-dimensional linear system.  In this Chapter we both recall basic ideas of numerical linear algebra and apply \PETSc to solve linear systems.
