...
\begin{enumerate}
\item[iv)] \emph{processor-count dependence of preconditioning}:  Many preconditioning schemes, including block-Jacobi and Schwarz methods (Chapter \ref{chap:sc}), act differently depending on the number of processors.
\end{enumerate}

\noindent Note that this processor-dependence has a stronger influence on final answers than the other parallel-only ``fact of life'', namely that parallel reductions are not deterministic (Chapter \ref{chap:gs}).
...
% FIXME from Barry Smith on petsc-users: "What does your -log_summary output look like? One thing that GMRES does is it introduces a global reduction with each multiple (hence a barrier across all your processes) on some systems this can be deadly."  Turn this into GMRES exercise on many processors with tri.c?
