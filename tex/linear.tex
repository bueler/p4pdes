...
\Triangle uses a simply-formatted ASCII file as input to describe a polygonal region $\Omega$ and to indicate the Dirichlet and Neumann portions.  For example, the triangulation shown in Figure \ref{fig:triangulation} was generated from the input file  \texttt{bump.poly} shown in Figure \ref{code:bumppoly}.
...
\end{verbatim}
This command generates three files, \texttt{bump.1.poly}, \texttt{bump.1.node}, and  \texttt{bump.1.ele}, which define the new (i.e.~refined relative to \texttt{bump.poly}) polygonal boundary, and the nodes and elements of the triangulation, respectively.  This is enough information to draw Figure \ref{fig:triangulation}, in particular.  For example, the file \texttt{bump.1.node} has the numbering (though not zero-based as shown) and node locations shown in blue in Figure \ref{fig:triangulation}.

\inputfromline{../c/bump.poly}{bump.poly}{A description of the polygon in Figure \ref{fig:triangulation}, suitable for reading by \Triangle.}{16}{code:bumppoly}
...
\cinputpart{c2triangle.c}{The standard preamble.}{I}{//STARTPREAMBLE}{//ENDPREAMBLE}{code:trianglepartone}
...
\cinputpart{c2triangle.c}{Determine filenames using \PETSc option processing.}{II}{//ENDPREAMBLE}{//ENDFILENAME}{code:triangleparttwo}
...
\cinputpart{c2triangle.c}{Start to read ASCII files with \textsc{triangle}-generated mesh by going to rank zero processor, reading node header, and allocating accordingly.}{III}{//ENDFILENAME}{//ENDRANK0ALLOC}{code:trianglepartthree}
...
\cinputpart{c2triangle.c}{On rank zero processor: Read node information.}{IV}{//ENDRANK0ALLOC}{//ENDREADNODES}{code:trianglepartfour}

\inputwhole{../c/convenience.h}{convenience.h}{Some definitions.}{code:convenience}

\clearpage

The fifth part of \texttt{c2triangle.c} is in Figure \ref{code:trianglepartfive}.  This looks like the fourth part, because we simply read element information from the \texttt{.ele} file.  Likewise the sixth part shown in Figure \ref{code:trianglepartsix}, which reads polygon information from the \texttt{.poly} file.

\cinputpart{c2triangle.c}{On rank zero processor: Read element information.}{V}{//ENDREADNODES}{//ENDREADELEMENTS}{code:trianglepartfive}

\cinputpart{c2triangle.c}{On rank zero processor: Read boundary polygon information.}{VI}{//ENDREADELEMENTS}{//ENDREADPOLYGONS}{code:trianglepartsix}
...
\cinputpart{c2triangle.c}{On rank zero processor: Write the mesh out in \PETSc binary format.}{VII}{//ENDREADPOLYGONS}{//ENDRANK0}{code:trianglepartseven}
...
\cinputpart{readmesh.c}{Read the mesh from the file.  Then put a sequential copy of each mesh \pVec on each processor.}{II}{//ENDGET}{//END}{code:readmeshparttwo}


\cinputpart{c2testprealloc.c}{Set up \pMat $A$ and ctually preallocate it.  Fill it with junk entries so the pattern can be visualized.}{II}{//ENDLOAD}{//ENDTEST}{code:testpreallocparttwo}
