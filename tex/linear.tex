...
Each entry $\tilde a_{ij}$ of the initial matrix $\tilde A$ is not, however, computed in turn.  Rather, we build the matrix by computing contributions to these entries from each element.

To explain element-by-element assembly we first define the integral over just one triangle $\triangle_k$:
    $$\tilde a_{ij}^k = \int_{\triangle_k} \grad \phi_j \cdot \grad \phi_i.$$
Also we introduce the idea of FIXME \texttt{P(k,q)}

Denoting $\tilde a_{ij}^k$ by \texttt{a(k,i,j)}, we to make the element-by-element assembly procedure clear:
\begin{verbatim}
...
for k = 0 to K-1 {        // loop through all elements
  for q = 0 to 2 {        // loop through vertices of element k
    i = P(k,q)            // row index
    for r = 0 to 2 {
      j = P(k,q)          // column index
      a(i,j) += a(k,i,j)  // need to compute this ...
    }
  }
}
// now we need to edit A for Dirichlet conditions ...
\end{verbatim}
