...
\end{equation}
By the chain rule,\sidenote{{\color{red} Caution} for readers of \citet{Elmanetal2005}: The ``Jacobian'' in their formula (1.37) is the transpose of the Jacobian here.} the derivatives of a composition $F(\xi,\eta) \equiv f(x(\xi,\eta),y(\xi,\eta))$ satisfy
\begin{equation*}
\begin{bmatrix}
\frac{\partial F}{\partial \xi} \\[0.4em]
\frac{\partial F}{\partial \eta}
\end{bmatrix}
=
J^\top
\begin{bmatrix}
\frac{\partial f}{\partial x} \\[0.4em]
\frac{\partial f}{\partial y}
\end{bmatrix}.
\end{equation*}
Applied to $F(\xi,\eta)=x(\xi,\eta)$ and $F(\xi,\eta)=y(\xi,\eta)$, in turn, to get the columns of a matrix, and inverting the $2\times 2$ matrix $J^\top$, we get these facts which will be useful below:
\begin{equation*}
\begin{bmatrix}
    \frac{\partial \xi}{\partial x} & \frac{\partial \eta}{\partial x} \\[0.2em]
    \frac{\partial \xi}{\partial y} & \frac{\partial \eta}{\partial y}
\end{bmatrix}
=
(J^\top)^{-1}
\begin{bmatrix}
    \frac{\partial x}{\partial \xi} & \frac{\partial y}{\partial \xi} \\[0.2em]
...
\end{align}
The last expression is the low point of this calculation.  From now on the formulas get simpler because the integrand is \emph{constant}.  Indeed, in this case the derivatives $\partial \chi_q/\partial\{\xi,\eta\}$ are constant because $\chi_q$ is linear on $\triangle_\ast$, and the derivatives $\partial\{\xi,\eta\}/\partial\{x,y\}$ are constant because the map to and from the reference triangle is linear.  Thus also $\det(J)$ is constant.

We need to calculate these constant derivatives, in terms of node coordinates, so as to compute \texttt{a(k,q,r)}.  First, the derivatives $\partial\{\xi,\eta\}/\partial\{x,y\}$ are the entries of the \emph{inverse} of the Jacobian $J$.  But
\begin{equation*}
\begin{bmatrix}
