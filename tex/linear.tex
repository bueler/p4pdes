...
Each entry $\tilde a_{ij}$ of the initial matrix $\tilde A$ is not even computed in one step.  Rather, we compute contributions to these entries from each element in turn.  Furthermore we do this in parallel.  Recall that \texttt{c2convert.c} above creates a distributed \pVec called \texttt{E}, which is an array of elements of type \texttt{elementtype},\sidenote{Defined just before Figure \ref{code:trianglepartfour}.} and \texttt{readmesh.c} reads it.
...
The Jacobian\sidenote{By definition, the \emph{Jacobian} $J=J(\bx)$ of a smooth map $F$ at a point $\bx$ is its linearization at $\bx$.  That is, if $\by=F(\bx)$ and $\by+\Delta\by = F(\bx+\Delta\bx)$ then $J(\bx)$ satisfies $\Delta \by = J(\bx) \Delta\bx + O(|\Delta\bx|^2)$.} of map \eqref{trianglemap} is
...
\end{equation}
Recalling both the change-of-variables formula for integrals\sidenote{If $F$ is smooth and maps points $\bx \in U$ to points $\by=F(\bx) \in F(U)$, and if $J$ is the Jacobian of $F$, then $$\int_{F(U)} f(\by)\,d\by = \int_U f(F(\bx))\,|\det(J)|\,d\bx.$$} and the chain rule, by \eqref{phichimap} we can write
\begin{align}
...
\cinputpart{c2poisson.c}{Read in mesh in parallel (i.e.~\pVec \texttt{E}) and create \pMat \texttt{A} and \pVec \texttt{b}.}{I}{//GETMESH}{//ENDMATVECCREATE}{code:ctwopoissongetmesh}

In the next part of the code (Figure \ref{code:ctwopoissonchecks}) we do two basic checks on our assembly procedure, without solving a linear system.  First we check the construction of \texttt{A} by checking if constant functions are in the kernel when $\partial_D\Omega=\emptyset$.  In this check the data $f$, $g$, and $\gamma$ are all not used, and the boundary information in \texttt{E} is not used either.

\cinputpart{c2poisson.c}{Do two checks on the construction of the linear system.}{II}{//TWOCHECKS}{//ENDTWOCHECKS}{code:ctwopoissonchecks}
...
\cinputpart{c2poisson.c}{Solve a Dirichlet problem for which we know the solution.}{III}{//SOLVEMANU}{//ENDSOLVEMANU}{code:ctwopoissonsolvemanu}
