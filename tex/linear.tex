...
because on a given triangle $\triangle_k$, the global indices $i,j$ are functions of the local node indices $q,r\in\{0,1,2\}$ on the triangle.  So now we need to both compute \texttt{a(k,i,j)} concretely and we need to implement the ``editing'' strategy which turns $\tilde A$ into $A$.

A standard approach to computing the element-wise integrals \texttt{a(k,i,j)} is to refer each triangle $\triangle_k$ to a reference triangle $\triangle_\ast$ with vertices $(0,0),\,(1,0),\,(0,1)$, as shown in Figure \ref{fig:isoparametric}.  The linear map from $\triangle_\ast$ to a generic triangle $\triangle$ with vertices $(x_0,y_0),\,(x_1,y_1),\,(x_2,y_2)$, as shown in the Figure, is
\begin{align*}
x(\xi,\eta) &= x_0 + (x_1-x_0) \xi + (x_2-x_0) \eta, \\
y(\xi,\eta) &= y_0 + (y_1-y_0) \xi + (y_2-y_0) \eta.
\end{align*}
Thus the Jacobian of the map is
\begin{equation}
J = \begin{bmatrix}
...
\chi_2(\xi,\eta) &= \eta.
\end{align*}
Now a key observation is that, using a local index $q=0,1,2$ for the vertices of $\triangle$, each of the basis functions $\phi_q=\phi_{j(q)}$ is the mapped version of the corresponding $\chi_q$:
...
\end{align}
The last expression is the low point of this calculation.  Indeed, the integrand is \emph{constant} in this case because $\partial \chi_q/\partial\{\xi,\eta\}$ are both constant (i.e.~$\chi_q$ is linear on $\triangle_\ast$), the partial derivatives $\partial\{\xi,\eta\}/\partial\{x,y\}$ are all constant (i.e.~the map to and from the reference triangle is linear), and $\det(J)$ is constant.

We need to calculate the four partial derivatives $\partial\{\xi,\eta\}/\partial\{x,y\}$ to simplify the last form of \eqref{elementcontrib}.  These are the entries of the \emph{inverse} of the Jacobian $J$.  But
\begin{equation*}
\begin{bmatrix}
...
= \frac{1}{\det(J)} \begin{bmatrix}
    ...
    \end{bmatrix}.
\end{equation*}
Using notation from \citet{Elmanetal2005}, let
\begin{equation*}
b_2 = y_2-y_0, \quad b_3 = y_0-y_1, \quad c_2 = x_0-x_2, \quad c_3 = x_1-x_0.
\end{equation*}
Then
\begin{equation}
\begin{bmatrix}
...
= \frac{1}{\det(J)} \begin{bmatrix}
    ...
    \end{bmatrix}
\quad \text{ and } \quad
\det(J) = b_2 c_3 - b_3 c_2. \label{jacobianFINAL}
\end{equation}
Returning to \eqref{elementcontrib}, by \eqref{jacobianFINAL} our element-wise contribution to $\tilde a_{ij}$ simplifies to
\begin{align}
\text{\texttt{a(k,q,r)}} = \frac{1}{2\; \det(J)} &\bigg[\left(\frac{\partial \chi_q}{\partial \xi} b_2 + \frac{\partial \chi_q}{\partial \eta} c_2\right) \left(\frac{\partial \chi_r}{\partial \xi} b_2 + \frac{\partial \chi_r}{\partial \eta} c_2\right) \label{elementcontribFINAL} \\
&\quad + \left(\frac{\partial \chi_q}{\partial \xi} b_3 + \frac{\partial \chi_q}{\partial \eta} c_3\right) \left(\frac{\partial \chi_r}{\partial \xi} b_3 + \frac{\partial \chi_r}{\partial \eta} c_3\right)\bigg] \notag
\end{align}
because the area of the reference triangle $\triangle_\ast$ is $1/2$\,.  But also note
\begin{align}
...
\end{align}
Thus we know enough to write a concrete procedure to compute \texttt{a(k,q,r)}, and thus all the entries in $\tilde A$.
