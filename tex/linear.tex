...
Let $\Omega \subset \RR^d$ be a region.  Suppose its boundary is decomposed into well-behaved, disjoint subsets $\partial_D \Omega$ and $\partial_N \Omega$ whose union is the entire boundary $\partial \Omega$.  The \emph{Poisson problem}, in strong form and including nonhomogeneous Dirichlet and Neumann boundary conditions, is\marginnote{%
...
As \eqref{poissonstrong} is stated there may be no solution where ``$\grad^2 u$'' makes sense as a function, even for reasonably nice boundary and source functions.  In particular, there may be no $u\in C^2(\Omega)$ which is continuous up to the boundary (i.e.~$u\in C(\bar\Omega)$).  There is, however, a solution\sidenote{Many mathematical concepts, including a well-posedness proof [in FIXME: specific] that justifies this claim, are all covered by \citep{Evans}.  These are technicalities for us.  Our goal is computational performance in cases where the Poisson problem is mathematically well-behaved.} if we change to a \emph{weak formulation}.  Furthermore, if $\partial_D \Omega$ has positive size, in the appropriate sense [FIXME: specific ref] then the solution is unique.  We will state the weak formulation after defining function spaces.
...
\caption{The polygon described in \texttt{bump.poly} (Figure \ref{code:bumppoly}).  The bold part is the Dirichlet boundary.}
\label{fig:bump-poly}
\end{marginfigure}
