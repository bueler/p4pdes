...
\section{A $P^1$ finite element method (FEM)}
...
As input to describe a polygonal region $\Omega$, \Triangle uses an ASCII file with extension \texttt{.poly}.  This input also indicates Dirichlet and Neumann portions of the boundary $\partial \Omega$.  FIXME: use trap.poly.  For example, the polygon shown in Figure \ref{fig:un:polygon} was created from the hand-built input file \texttt{blob.poly} shown in Code \ref{code:blobpoly}.

\inputfromline{../c/\CODELOC/meshes/blob.poly}{\CODELOC blob.poly}{FIXME: use trap.poly.  A description of the boundary polygon in Figure \ref{fig:un:polygon}, suitable for reading by \Triangle.}{8}{code:blobpoly}
