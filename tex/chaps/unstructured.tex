...
Let's try it.\sidenote{The reader should understand that showing the actual process of debugging in a textbook is hard.  Bugs occurred, and were found, as this subsection was created.}  The first run uses the coarse triangulation in Figure \ref{fig:un:trapone}.  Because our first tests use only the residual evaluation function \texttt{FormFunction()}, and not an analytical Jacobian, we add \texttt{-snes\_fd}:
\begin{cline}
$ cd c/ch7/
$ make unfem
...
\end{cline}
%$
Note that \texttt{unfem} describes the mesh by the number of nodes $N$ and the maximum triangle side length $h$.  Also note that \texttt{unfem} defaults to the ``case $0$'' exact solution---see the description of this linear problem on page \pageref{eq:un:exactsolution}---and quadrature of degree 1.

The actual test we care about, for code correctness, is whether the error decreases at the theoretically-optimum rate as the mesh is refined.  For the next level of refinement note that we have already generated the mesh (\texttt{trap.2} in Figure \ref{fig:un:traptwo}), but that it needs to be converted to \PETSc binary files for reading by \texttt{unfem}:
\begin{cline}
...
The error is already an order of magnitude smaller.  So as to measure a convergence rate, however, we generate, by the following snippet of Bash code, nine levels of refined grids  \texttt{trap.1}--\texttt{trap.9}.  The last of these has $N\approx 10^6$ nodes:
\begin{code}
area[0]=0.5
area[1]=0.1
area[2]=0.02
...
\caption{Numerical error (left axis) and number of evaluations of \texttt{FormFunction()} (right axis) using \texttt{-snes\_fd} runs on meshes \texttt{trap.1}--\texttt{trap.5}.}
\label{fig:un:unfem-snesfd}
\end{figure}

A bit of testing shows, however, that these \texttt{-snes\_fd} are far too slow, and even fail with a too-many-evaluations error,\sidenote{Add \texttt{-snes\_converged\_reason} to see the error message.} for levels \texttt{trap.6}--\texttt{trap.9}.  The following snippet of Bash code tests the available coarser levels and generates the results shown in Figure \ref{fig:un:unfem-snesfd}:
\begin{code}
for (( N=1; N<=5; N++ )); do
    rm -f foo
    ./unfem -un_mesh meshes/trap.$N -snes_fd -log_view > foo
    grep "|u-u_ex|_inf" foo
    grep "SNESFunctionEval" foo
done
\end{code}
%$
The evidence suggests convergence, but we will do more work before being confident of the rate.

Of course, there is no mystery in why these runs slow down dramatically with refinement.  As explained in Chapter \ref{chap:nl}, because there is no structured-grid information to allow coloring, the number of evaluations of \texttt{FormFunction()} scales with the number of nodes (unknowns) $N$.\sidenote{The number of evaluations shown in Figure \ref{fig:un:unfem-snesfd} is essentially $2N$.}  Obviously, this is an unsustainable amount of work as the grid is refined.  We are motivated to write additional code for an analytical Jacobian in the next subsection.

Before moving on, however, we can view the symmetry and sparsity pattern of the finite-differenced Jacobian using X windows:
\begin{cline}
$ ./unfem -un_mesh meshes/trap.2 -snes_fd -mat_view draw -draw_pause 2
case 0 result for N=33 nodes with h = 6.212e-01 :  |u-u_ex|_inf = 0.0289678
\end{cline}
