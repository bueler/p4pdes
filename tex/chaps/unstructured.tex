...
At this point writing additional code for an analytical Jacobian is well-motivated.  In part because it is easier, and in part to illustrate a common technique, in this Chapter we do not construct the full Jacobian but instead the \emph{Picard matrix}, an approximate linearization.

Recall that the Newton method linearizes the algebraic system \eqref{eq:un:fullsystem}, namely $\bF(\bu)=0$.  Given the current iterate $\bu^k$ the method solves a linear system
\begin{equation}
J(\bu^k) \bs = -\bF(\bu^k)  \label{eq:un:newtonstep}
\end{equation}
for $\bs = \bu^{k+1}-\bu^k$, the step for computing the new iterate $\bu^{k+1}$.

In our case system \eqref{eq:un:fullsystem} has a nearly-linear form already.  In fact, the FEM \eqref{eq:un:weakformhat}, \eqref{eq:un:uhexpand} has the form
\begin{equation}
A(\bu) \bu = \bb(\bu) \label{eq:un:picardformsystem}
\end{equation}
and the residual is in the form $\bF(\bu) = A(\bu) \bu - \bb(\bu)$.  The left side of \eqref{eq:un:weakformhat} contains all the derivative terms and it can be regarded as a matrix-vector multiplication for computing $\bF(\bu)$.  In general, and in the ``case $1$'' test problem in particular, functions $A(\bu)$ and $\bb(\bu)$ are nonlinear.

A reasonably-obvious iteration applies to \eqref{eq:un:picardformsystem}.  The matrix $A(\bu)$ and right-hand side $\bb(\bu)$ can be ``frozen'' at the current iterate and the linear system solved for a new iterate:
\begin{equation}
A(\bu^k) \bu^{k+1} = \bb(\bu^k). \label{eq:un:picardnaive}
\end{equation}

Equation \eqref{eq:un:picardnaive} is the usual form of the Picard iteration.  To illustrate how natural is the Picard approach, at the level of the continuum equation we propose to turn \eqref{eq:un:poissonstrong} into the iteration
    $$-\Div\left(a(u^k) \grad u^{k+1}\right) = f(u^k).$$

However, it is useful to transform \eqref{eq:un:picardnaive} into form \eqref{eq:un:newtonstep} so that we may use \PETSc's \pSNES machinery for the Picard iteration.  We first subtract $A(\bu^k) \bu^k$ from each side of \eqref{eq:un:picardnaive}:
\begin{equation*}
...
\end{equation}
This is just like the Newton equation \eqref{eq:un:newtonstep} but with a different matrix $A=A(\bu)\in\RR^{N\times N}$ replacing the Jacobian $J=J(\bu)$.

The matrix is the same as the Jacobian, $A=J$, in the linear case where $\partial a/\partial u=0$ and $\partial f/\partial u=0$.  More generally we expect $A$ to be a good spectral approximation (see Chapter \ref{chap:ls}) of $J$ because it captures the highest-order derivative terms in the PDE \eqref{eq:un:poissonstrong}, which generate the largest eigenvalues.


\section{Assembling the Picard matrix}

Our plan is now to write a function called \texttt{FormPicard()} which computes \texttt{A} $=A(\bu)$ from the current iterate.  Then we will pass \texttt{FormPicard()} and a \pMat \texttt{A} to \texttt{SNESSetJacobian()}.  In the linear cases $0$ and $2$, this provides the correct Jacobian.  We will see that this approximation of the Jacobian is highly-effective in the nonlinear case $1$.

Considering \eqref{eq:un:dirichletresiduals}, \eqref{eq:un:elementweakform}, and \eqref{eq:un:elementwisesum}, we see that the entries of $A$ are sums over elements
\begin{equation}
...
Of course, we have already computed integral \eqref{eq:un:picardentryelement}, namely in evaluating \eqref{eq:un:elementintegralreference}.  This gives us a software choice, whether to
\begin{itemize}
\item[option 1] FIXME side-effect
\item[option 2] FIXME code duplication
\end{itemize}
