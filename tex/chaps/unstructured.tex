...
Recall $L^2(\Omega)$ is the space of square-integrable real functions on $\Omega$ and that $W^{1,2}(\Omega)$ is the Sobolev space in which the gradient is also square integrable.\sidenote{See definition \eqref{eq:of:sobolevdefn}.}  We use two subsets of $W^{1,2}(\Omega)$: \emph{trial functions} from $W^{1,2}_g(\Omega)$, the functions with value $g_D$ along $\partial_D \Omega$, and \emph{test functions} from $W^{1,2}_0(\Omega)$, with value $0$ along $\partial_D \Omega$.

Now choose any $v\in W^{1,2}_0(\Omega)$, multiply the first equation in \eqref{eq:un:poissonstrong} by $v$, and integrate by parts:
\begin{equation*}
...
\end{itemize}
For the former see Exercise \ref{chap:un}.\ref{exer:un:bratu}.\sidenote{Exercise \ref{chap:nl}.\ref{exer:nl:bratu} solves the 1D version.}  Later in the Chapter we test our code on an easy case of the porous medium equation, namely with $a(u)=1+u^2$.
...
In contrast to many references, such as \citet{Elmanetal2005} which we follow in many ways, numbering here is zero-based, suitable for a C implementation.  Thus the $K$ triangles $\triangle_k$ in the triangulation are indexed $k=0,\dots,K-1$.  The $N$ nodes are located at $\bx_i=(x_i,y_i)$ for $i=0,\dots,N-1$. 
...
Using hat functions $\psi_j$ and also $\hat g_D$ we can now specify the finite-dimensional subspaces of $W^{1,2}(\Omega)$ used in the method.  First, the \emph{test functions}
\begin{align*}
...
Triangulation $\mathcal{T}_h$ covers\sidenote{At least, the set $\Omega \setminus \cup_k \triangle_k$ has measure zero.} $\Omega$ with non-overlapping triangles so we can write integral \eqref{eq:un:weakformhat} as a sum over elements.  To be specific, for each $k=0,\dots,K-1$ let
\begin{equation}
...
\cinputpart{unfem.c}{\CODELOC}{Context for solving FEM \eqref{eq:un:weakformhat} on an unstructured mesh.}{I}{//STARTCTX}{//ENDCTX}{code:unfemctx}
...
\cinputpart{unfem.c}{\CODELOC}{The first job in \texttt{main()} is to read an unstructured mesh from \PETSc binary-format files.}{II}{//STARTMAINREADMESH}{//ENDMAINREADMESH}{code:unfemmainreadmesh}
...
\cinputpart{unfem.c}{\CODELOC}{Setting-up a \pSNES object is similar to grid-less examples in Chapter \ref{chap:nl}.  We also set the default \pKSP and \pPC appropriately to our symmetric, positive-definite problem.}{III}{//STARTMAINSOLVER}{//ENDMAINSOLVER}{code:unfemmainsolver}

Though we do not, for now, need to do any work to assemble it, the symmetry and positive-definiteness of the Jacobian matrix is already relevant here.  In the linear case, in which $\partial a/\partial u=0$ and $\partial f/\partial u=0$ in \eqref{eq:un:poissonweak}, the Jacobian $J_{ij} = \partial F_i/\partial u_j$ is symmetric and positive-definite.\sidenote{We return to this idea, and prove the claim, in the subsection below on assembling an approximate Jacobian.}  This fact is sufficient reason to reset the default \pKSP and \pPC types to the conjugate gradient (CG) and incomplete-Cholesky (ICC) types, respectively.  This is done in Code \ref{code:unfemmainsolver} by first getting a pointer to the \pKSP inside the \pSNES (Figure \ref{fig:un:unfemstack}), and then setting its type.  Similarly, accessing a pointer to the \pPC inside the \pKSP allows us to set the \pPC type.

\cinputpart{unfem.c}{\CODELOC}{Our FEM uses local basis functions $\chi_\ell$ and some quadrature tools.}{IV}{//STARTFEM}{//ENDFEM}{code:unfemfem}
...
\cinputpart{unfem.c}{\CODELOC}{\texttt{FormFunction()} first evaluates Dirichlet boundary residuals \eqref{eq:un:dirichletresiduals} and Neumann boundary integrals \eqref{eq:un:segmentweakform}.}{V}{//STARTBDRYRESIDUALS}{//ENDBDRYRESIDUALS}{code:unfembdryresiduals}

\cinputpart{unfem.c}{\CODELOC}{Next, \texttt{FormFunction()} traverses the elements to compute residuals \eqref{eq:un:elementintegralreference}.}{VI}{//STARTELEMENTRESIDUALS}{//ENDELEMENTRESIDUALS}{code:unfemelementresiduals}



\section{Initial testing}

FIXME first run; the result is not shown 

\begin{cline}
$ ./unfem -un_mesh meshes/trap.1 -un_view -snes_fd
...
case 0 result for N=8 nodes with h_max = 1.414e+00 :  |u-u_ex|_inf = 0.230159
\end{cline}
%$

FIXME graphical \pMat view
...
FIXME figure showing result for \texttt{trap.1,2,3}
...
for (( N=1; N<9; N++ )); do
    ...
    ./tri2petsc.py meshes/trap.$N meshes/trap.$N  # generate .vec,.is
done
\end{code}
