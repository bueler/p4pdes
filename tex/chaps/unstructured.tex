...
\inputwhole{misc/trap.1.poly}{\CODELOC meshes/trap.1.poly}{\Triangle-generated ASCII file for the boundary polygon, as segments with nodal indices, and with segment boundary flags.}{code:traponepoly}
...
The \PETSc \pIS type is convenient for storing these integers.\sidenote{In this Chapter we do not exploit the ``main purpose'' of an \pIS in distributed (multi-process) indexing.}  The reader may regard an \pIS as an ``integer-valued \pVec'' without any loss of meaning in the current use.  The \pIS storing the element index triples holds $3K$ integers and the \pIS for boundary segments holds $2P$ integers.
...
Before showing these details, and because the reader might have become too comfortable with \pDMDA-using structured grid examples, a look at the beginning of Chapter \ref{chap:nl} might be appropriate.  Indeed, Figure \ref{fig:un:unfemstack} shows the structure of \texttt{unfem.c}, including the approximate Jacobian (\texttt{FormPicard()}) which we will eventually implement.  In terms of \PETSc objects it has the same structure as the grid-free nonlinear equations-solving example \texttt{ecjac.c} in Chapter \ref{chap:nl}.
...
\draw[line width=1pt] (3,7) node[userfcn,minimum width=110mm] (usercode) {user code \\ \vspace{15mm}};

\draw[line width=1pt] (0,7.2) node[userfcn] (rescode) {residual \emph{\texttt{FormFunction()}}};
\draw[line width=1pt] (5.5,6.2) node[userfcn] (jaccode) {approximate Jacobian \emph{\texttt{FormPicard()}}};
...
\path
   ...
   (ksp) edge node {} (pc);
\end{tikzpicture}
\caption{The solver structure of \texttt{unfem.c}.}
