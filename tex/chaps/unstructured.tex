...
Before proceeding, note that $n_D$ is the number of \emph{nodes} in the Dirichlet boundary, whereas $n_N$ is the number of \emph{segments} (edges) in the Neumann boundary.  The former correspond to degrees of freedom, while the latter are domains of integration.
...
Recalling both the change-of-variables formula for integrals and the chain rule, by \eqref{eq:un:phichimap} we can write
\begin{equation}
