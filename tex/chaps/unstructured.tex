...
As in Chapter \ref{chap:of}, the integrals \eqref{eq:un:elementintegralreference} will be done approximately by quadrature.  If there are $Q$ quadrature points in $\triangle_\ast$ at locations $(\xi_q,\eta_q)$ and with weights $w_q$ then the rule for \eqref{eq:un:elementintegralreference} is
\begin{equation}
F_i^k(\bu) \approx |\det J_k| \sum_{q=0}^{Q-1} w_q H_\ell^k(\bu,\xi_q,\eta_q). \label{eq:un:elementquadraturereference}
\end{equation}

Unlike in Chapter \ref{chap:of}, however, the elements are not rectangular.  Quadrature rules based on products of one-dimensional integrals can be built \citep{KarniadakisSherwin2013}, but there are also rules constructed specifically for integrating on the reference triangle $\triangle_\ast$.  Degree $1,2,3$ cases appear in Table \ref{tab:un:quadrature}.
...
\vspace{0.1in}
\caption{Symmetric quadrature rules on the reference triangle $\triangle_\ast$.} \label{tab:un:quadrature}
\end{table}

FIXME: go ahead and address quadrature here
