...
At this point, as in earlier Chapters, writing additional code for an analytical Jacobian is now well-motivated.  However, so as to illustrate this common alternative technique, we do not construct the full Jacobian but instead construct the \emph{Picard} linearization.

Recall that the Newton method linearizes the algebraic system \eqref{eq:un:fullsystem}, namely
\begin{equation*}
\bF(\bu)=0.
\end{equation*}
The steps of the Newton method solve linear systems
\begin{equation}
J(\bu^k) \bs = -\bF(\bu^k)  \label{eq:un:newtonstep}
\end{equation}
where $\bs = \bu^{k+1}-\bu^k$ is the step which computes the new iterate.

In our case system \eqref{eq:un:fullsystem} has a nearly-linear form in the sense that the FEM \eqref{eq:un:weakformhat}, \eqref{eq:un:uhexpand} has the form
\begin{equation}
A(\bu) \bu = \bb(\bu) \label{eq:un:picardformsystem}
\end{equation}
so that the residual is in the form $\bF(\bu) = A(\bu) \bu - \bb(\bu)$.  That is, the residual function $\bF(\bu)$ can be regarded as a matrix-vector multiplication for the left side of \eqref{eq:un:weakformhat}, containing all derivative terms, and a right side which is zeroth-order.  In general, and in the ``case $1$'' problem in particular, functions $A(\bu)$ and $\bb(\bu)$ are nonlinear.

A reasonably-obvious iteration applies to \eqref{eq:un:picardformsystem}.  The matrix and right-hand side can be ``frozen,'' based on a current iterate, and the linear system solved for a new iterate:
\begin{equation}
A(\bu^k) \bu^{k+1} = \bb(\bu^k). \label{eq:un:picardnaive}
\end{equation}
Though it is the usual form of the Picard iteration, it is interesting and useful to note that \eqref{eq:un:picardnaive} can be transformed to the form \eqref{eq:un:newtonstep}, which allows us to use any of \PETSc's Newton machinery for the Picard iteration.  First make this simple transformation of \eqref{eq:un:picardnaive}:
\begin{equation*}
A(\bu^k) (\bu^{k+1}-\bu^k) = - \left(A(\bu^k) \bu^k - \bb(\bu^k)\right).
\end{equation*}
Exactly as in the Newton iteration \eqref{eq:un:newtonstep} above, let $\bs = \bu^{k+1}-\bu^k$.  From the form of the residual in \eqref{eq:un:picardformsystem}, we now have
\begin{equation}
A(\bu^k) \bs = -\bF(\bu^k).  \label{eq:un:picardstep}
\end{equation}
This is just Newton's method with a different matrix.
