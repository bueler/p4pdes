...
\end{center}
and thereby get a functioning, \pSNES-using code without even thinking about matrices as we write code.  Only after making sure it works do we
\begin{center}
\emph{write additional code to assemble the Jacobian matrix.}
\end{center}

Our code will solve certain nonlinear and linear Poisson equations.  It uses \PETSc for these new tasks:
\begin{itemize}
...
\end{equation}
Note that these values do not depend on the solution $u^h$ (or $\bu$).

Before proceeding, observe that $n_D$ is the number of \emph{nodes} in the Dirichlet boundary, whereas $n_N$ is the number of \emph{segments} (edges) in the Neumann boundary.  The former correspond to degrees of freedom, while the latter are domains of integration.
...
\end{equation}
Because the support of the hat function $\psi_i$ only overlaps with a few triangles $\triangle_k$, only a few values of $F_i^k(\bu)$ and $\varphi_i^\nu$ are nonzero.  Also, only a few nodal values $\bu=\{u_j\}$ enter into $F_i^k(\bu)$, namely those values $u_j$ such that the support of $\psi_j$ overlaps with $\triangle_k$.  These facts make \eqref{eq:un:elementwisesum} a \emph{sparse}, and generally nonlinear, system.  The Jacobian $J_\bF(\bu)$ is a sparse matrix, the stiffness matrix.
...
\begin{equation}
\rho_k = \left|\det \frac{\partial (x,y)}{\partial (\xi,\eta)}\right| = \left|(x_1-x_0) (y_2-y_0) - (x_2-x_0) (y_1-y_0)\right|.  \label{eq:un:trianglejacobian}
\end{equation}
...
\begin{equation}
\psi_i(x(\xi,\eta),y(\xi,\eta)) = \chi_q(\xi,\eta) \label{eq:un:psichimap}
\end{equation}
for all $(\xi,\eta)\in\triangle_\ast$, where vertex $q$ of $\triangle_\ast$ maps to $\bx_i$.

Recalling both the change-of-variables formula for integrals and the chain rule, by \eqref{eq:un:psichimap} we can write
\begin{equation}
F_i^k(\bu) = \rho_k \int_{\triangle_\ast} H_q^k(\bu,\xi,\eta)\,d\xi\,d\eta \label{eq:un:elementintegralreference}
\end{equation}
where
\begin{equation}
H_q^k(\bu,\xi,\eta) = \left[a(u^h) \grad u^h \cdot \grad \psi_i - f(u^h) \psi_i\right]_{\triangle_\ast}  \label{eq:un:elementintegrand}
\end{equation}
for $q=0,1,2$.  Here node $\bx_i$ corresponds to local node number $q$ on $\triangle_\ast$.  Note that ``$\grad$'' in \eqref{eq:un:elementintegrand} refers to derivatives in variables $x,y$.  Exercises \ref{chap:un}.\ref{exer:un:gradientdetails} and \ref{chap:un}.\ref{exer:un:elementintegranddetails} addresses the details implicit in \eqref{eq:un:elementintegrand}.


\section{Triangular meshes from \Triangle}

\PETSc itself does not include tools for triangulating regions of the plane.  We use the widely-available \Triangle\sidenote{See \href{http://www.cs.cmu.edu/~quake/triangle.html}{www.cs.cmu.edu/$\sim$quake/ triangle.html} for documentation and source code.  For this Chapter we recommend that your \PETSc configuration include option \texttt{---download-triangle} and that \texttt{\$PETSC\_ARCH/bin} be on your \texttt{PATH}.} software \citep{Shewchuk1996} for this task.  \Triangle is limited to 2D, and it only writes ASCII files, but it has straightforward details.
...
FIXME If \texttt{E} denotes an array of \texttt{elementtype} then for each triangle $\triangle_k$ we can get the node index for each of its three vertices.  Using $q=0,1,2$ for these vertices,
...
\item  \label{exer:un:gradientdetails}  For the map \eqref{eq:un:trianglemap} from $\triangle_\ast$ to $\triangle_k$, let
    $$J = \frac{\partial (x,y)}{\partial (\xi,\eta)} = \begin{pmatrix}
    x_1 - x_0 & x_2 - x_0 \\
...
\begin{equation}
\grad_{x,y} \psi_i = \frac{1}{\det J} \left<\Delta y_2 \frac{\partial \chi_q}{\partial \xi} - \Delta y_1 \frac{\partial \chi_q}{\partial \eta}, - \Delta x_2 \frac{\partial \chi_q}{\partial \xi} + \Delta x_1 \frac{\partial \chi_q}{\partial \eta}\right>. \label{eq:un:gradpsionref}
\end{equation}
Here $\det J = \Delta x_1 \Delta y_2 - \Delta x_2 \Delta y_1$ and indices $i$ and $q$ have the same relationship as in \eqref{eq:un:psichimap}.  Compare formula \eqref{eq:of:gradpsionref} for the structured case; why is \eqref{eq:un:gradpsionref} more complicated?  % underlying reason is that affine map here includes rotations, while in Chapter \ref{chap:of} it was just translation and axes scaling
