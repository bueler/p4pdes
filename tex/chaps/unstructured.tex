...
Let's try it.\sidenote{The reader should understand that showing the actual process of debugging in a textbook is hard.  Bugs occurred, and were found, as this subsection was created.}  The first run uses the coarse triangulation in Figure \ref{fig:un:trapone}, and, because we initially test only the residual evaluation function \texttt{FormFunction()} and not an analytical Jacobian, we add \texttt{-snes\_fd}:
\begin{cline}
$ cd c/ch7/
$ make unfem
...
The actual test we care about, for code correctness, is whether the error decreases at the theoretically-optimum rate as the mesh is refined.  For the next level of refinement note that we have already generated the mesh (\texttt{trap.2} in Figure \ref{fig:un:traptwo}), but that it first needs to be converted to \PETSc binary files for reading by \texttt{unfem}:
\begin{cline}
...
The error is already an order of magnitude smaller.  So as to measure a convergence rate we start by writing the following snippet of Bash code for nine levels of refined grids  \texttt{trap.1}--\texttt{trap.9}.  The last of these has $N\approx 10^6$ nodes:
\begin{code}
area[0]=0.5
area[1]=0.1
area[2]=0.02
...
A bit of testing shows, however, that \texttt{-snes\_fd} runs are far too slow, and even fail with a too-many-evaluations error,\sidenote{Add \texttt{-snes\_converged\_reason} to see the error message.} for levels \texttt{trap.6}--\texttt{trap.9}.  On the other hand, the matrix-free Newton-Krylov method (see Chapter \ref{chap:nl}) is also available without an assembled Jacobian, i.e.~with option \texttt{-snes\_mf}.  For example, 
\begin{cline}
...
The graph of errors in Figure \ref{fig:un:unfem-fdmf} suggests convergence, but we will confirm the rate of convergence using an analytical Jacobian (next subsection).  Note that errors from the \texttt{-snes\_fd} and  \texttt{-snes\_mf} runs coincide for all grids where both complete successfully.

These runs slow dramatically with refinement.  The reason is not a mystery: see the graph of the number of residual evaluations, as a function of the mesh spacing, in Figure \ref{fig:un:unfem-fdmf}.  As explained in Chapter \ref{chap:nl},\sidenote{Also, there is no structured-grid information to allow coloring, i.e.~\texttt{-snes\_fd\_color}.  That option was quite effective in Chapter \ref{chap:of}, for example.} for option \texttt{-snes\_fd} the number of evaluations of \texttt{FormFunction()} scales with the number of nodes (unknowns) $N$.  Indeed, the number of evaluations shown in Figure \ref{fig:un:unfem-fdmf} is essentially $2N$ for \texttt{-snes\_fd}.

Recall from Chapter \ref{chap:nl} that a different issue arises to explain the large number of residual evaluations for option \texttt{-snes\_mf}.  This number scales with the number of Krylov solver iterations.  Such iterations grow rapidly with refinement because---this is \emph{pure} matrix-free Newton-Krylov---we can have no preconditioning on each the linear system.

As in earlier Chapters, by now we are motivated to write additional code for an analytical Jacobian; see the next subsection.

Before moving on, however, we can already examine the symmetry and sparsity pattern of the Jacobian using X windows:
\begin{cline}
$ ./unfem -un_mesh meshes/trap.2 -snes_fd -mat_view draw -draw_pause 2
case 0 result for N=33 nodes with h = 6.212e-01 :  |u-u_ex|_inf = 0.0289678
\end{cline}
%$
The middle frame of Figure \ref{fig:un:unfem-matsparsity} shows the result sandwiched between comparable results for the coarser \texttt{trap.1} and the further-refined \texttt{trap.3} meshes.
...
\end{equation}
Again this is computed by quadrature, similar to \eqref{eq:un:elementintegralreference} and \eqref{eq:un:elementquadraturereference}.
