...
\begin{tikzpicture}[>={Stealth[length=2.5mm]},scale=0.9,
  component/.style={
     rectangle,
     draw,
     fill=white,
...
\draw[line width=1pt] (3,8) node[rounded corners,fill=white,draw,align=center,minimum width=80mm,minimum height=15mm] (usercode) {user code \\ \vspace{20mm}};
\draw[line width=1pt] (0.5,8) node[rounded corners,fill=white,draw,align=center] (objcode) {form objective};
\draw[line width=1pt] (3,7) node[rounded corners,fill=white,draw,align=center] (rescode) {form residual};
\draw[line width=1pt] (5,8) node[dashed,rounded corners,fill=white,draw,align=center] (jaccode) {form Jacobian};

\draw[line width=1pt] (0,4) node[component] (snes) {\complabel{\pSNES}{nonlinear solver}};
\draw[line width=1pt] (4,4) node[component] (dmda) {\complabel{\pDMDA}{structured grid}};
\draw[line width=1pt,dashed] (8,4) node[component] (dmplex) {\complabel{\pDMPlex}{unstructured grid}};

\draw[line width=1pt] (0,2.5) node[component] (ksp)  {\complabel{\pKSP}{linear solver}};
\draw[line width=1pt] (0,1) node[component] (pc)   {\complabel{\pPC}{preconditioner}};

\draw[line width=1pt] (3,0) node[component] (matj)   {\usedlabel{\pMat}{Jacobian}};
\draw[line width=1pt] (7,0) node[component] (vecs)   {\pVec \\ \footnotesize  \emph{approx.~solution, exact solution,} \\ \footnotesize \emph{other fields}};
...
\draw[->] (objcode) -- (vecs);
\draw[->] (rescode) -- (vecs);
\draw[->,dashed] (jaccode) -- (vecs);
\draw[->,dashed] (jaccode) -- (matj);
\end{tikzpicture}
\caption{The ``standard \PETSc stack for PDEs,'' used by every example in the rest of the book.  Solid boxes show the cases used in the current chapter.}
