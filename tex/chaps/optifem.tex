...
\draw[line width=1pt] (3,8) node[rounded corners,fill=white,draw,align=center,minimum width=80mm] (usercode) {\small user code \\ \vspace{15mm}};
\draw[line width=1pt] (-0.5,8.2) node[rounded corners,fill=white,draw,align=center] (objcode) {\small form objective};
\draw[line width=1pt] (3,7) node[rounded corners,fill=white,draw,align=center] (rescode) {\small form residual};
\draw[line width=1pt] (6,8.2) node[dashed,rounded corners,fill=white,draw,align=center] (jaccode) {\small form Jacobian};

\draw[line width=1pt] (-4,4.7) node[component,dashed] (ts) {\complabel{\pTS}{time-stepping}};
...
\draw[line width=1pt] (4,4) node[component] (dmda) {\complabel{\pDMDA}{structured grid}};
\draw[line width=1pt,dashed] (8.5,4.5) node[component] (dmplex) {\complabel{\pDMPlex}{mesh}};

\draw[line width=1pt] (3,0) node[component] (matj)   {\usedlabel{\pMat}{Jacobian}};
\draw[line width=1pt] (7,0) node[component] (vecs)   {\pVecs \\ \footnotesize  \emph{approx.~\& exact solutions,} \\ \footnotesize \emph{other fields}};

\path
   ([xshift=2em]objcode.south) edge[->,bend right,distance=5em] node [bottom] {} (vecs)
   ([xshift=4em]rescode.south) edge[->,bend left] node [bottom] {} ([xshift=2em]vecs.north)
   ([xshift=4em]jaccode.south) edge[->,dashed] node [bottom] {} ([xshift=-1em]vecs.north)
   ([xshift=3em]jaccode.south) edge[->,dashed,bend left,distance=5em] node [bottom] {} (matj)
   ([xshift=-15em]usercode.south) edge[->,dashed] node [bottom] {} (ts)
   ([xshift=-10em]usercode.south) edge[->] node [bottom] {} (snes)
   ([xshift=2em]usercode.south) edge[->] node [bottom] {} (dmda)
   ([xshift=14em]usercode.south) edge[->,dashed] node [bottom] {} (dmplex)
   ([xshift=4em]usercode.south) edge[->,bend left] node [bottom] {} (vecs)
   (ts) edge[dashed] node [bottom] {} (snes)
   (snes) edge node [bottom] {} (ksp)
   (ksp) edge node [bottom] {} (pc);
\end{tikzpicture}
\caption{The ``standard \PETSc stack for PDEs,'' used by every example in the rest of the book.  Chapter \ref{chap:of} (current) cases are shown as solid.  Arrows should be read as ``acts directly on.''}
\label{fig:of:standardstack}
\end{figure}

Figure \ref{fig:of:standardstack} shows the ``\PETSc stack'' used in all examples in the rest of the book.  The code \texttt{reaction.c} in Chapter \ref{chap:nl} is already an example.  Initial implementations require user code for a residual or objective implementation, as otherwise \PETSc has \emph{no} description of your problem, but Jacobians are also implemented for most examples.  Arrows from user code represent the create/set/use/destroy sequence (Chapter \ref{chap:ls}).  Downward lines from one \PETSc type to another represent internal sub-objects, sometimes exposed to the user, whether within the code or at the command line at runtime, and sometimes not.
