...
\cinputpart{ecjac.c}{\CODELOC}{Solves the same nonlinear system as \texttt{expcircle.c}, but with an exact Jacobian and parameter passing.}{I}{//START}{//END}{code:ecjacI}
...
\cinputpart{ecjac.c}{\CODELOC}{This \texttt{main()} function allocates a \pMat to hold the Jacobian.}{II}{//STARTMAIN}{//ENDMAIN}{code:ecjacII}

An interesting detail appears here, however.  There are actually \emph{two} output \pMats for \texttt{FormJacobian()} to set.  The first, called \texttt{J} here, corresponds to the Jacobian matrix itself.  The second, called \texttt{P} here, is the ``material'' from which \PETSc can build a preconditioner.  In this case \texttt{FormJacobian()} sets the entries of \pMat \texttt{P} to the correct, accurate derivatives of $\bF$.

We then assemble the \pMat \texttt{P}.  Often \texttt{J} is the same object as \texttt{P}, in which case it is now assembled.  However, if \texttt{J} points\sidenote{Note the \PETSc \pMat type is reall a C pointer.} to a different object then we also assemble it.  For example, \texttt{J} is a finite-difference approximation of the Jacobian in the \texttt{-snes\_mf\_operator} usage described later.  In that case we do not want to modify \texttt{J}, but we do need to tell \PETSc that we are done with it.  These actions on \texttt{J} and \texttt{P} allow \emph{preconditioned} Jacobian-free Newton-Krylov methods to work; see page \pageref{sec:JFNK} below.
...
\cinputpart{reaction.c}{\CODELOC}{C functions describing the initial iterate, the exact solution, and the residual function from finite-difference equation \eqref{eq:nl:drfdscheme}.}{I}{//CALLBACK}{//ENDCALLBACK}{code:reactionfunction}

Two functions appear in Code \ref{code:reactionfunction}.  The first one (\texttt{InitialAndExact()}) computes both the initial iterate\sidenote{A linear function connecting the boundary conditions.} and the exact solution.  Next is the residual evaluation function \texttt{FormFunctionLocal()}.  The Jacobian evaluation function \texttt{FormJacobianLocal()} appears in Code \ref{code:reactionjacobian}.  These two are ``call-back'' functions which the \pSNES will use when it needs the values of the residual and Jacobian at a given location in the space of candidate solutions.

\cinputpart{reaction.c}{\CODELOC}{A C function describing the derivatives of \eqref{eq:nl:drfdscheme}.}{II}{//JACOBIAN}{//ENDJACOBIAN}{code:reactionjacobian}
...
Though the previous paragraphs adequately summarize program \texttt{reaction.c}, the diagram in Figure \ref{fig:nl:reactionstack} communicates the overall structure.  Arrows from the large ``user code'' block, i.e.~\texttt{main()} in general, represent the create/set/use/destroy sequence, as introduced in Chapter \ref{chap:ls}, for the major \PETSc components (e.g.~\pSNES and \pDMDA), but user code also acts upon the \pVecs which store the important fields, and upon the \pMat Jacobian matrix if present.  Downward lines from one \PETSc type to another represent internal sub-objects.  Such sub-components are sometimes exposed to the user, whether within the code or at the command line at runtime, and sometimes not.
...
\cinputpart{reaction.c}{\CODELOC}{In \texttt{main()} we create a \pDMDA, some \pVecs, and a \pSNES.  We hand call-back functions to the \pSNES.  Then we solve the equation.}{III}{//MAIN}{//ENDMAIN}{code:reactionmain}
...
From the code side, at minimum a C function for $\bF$ must be implemented in all cases, as there is no other way for \PETSc to know what equations you are solving.\sidenote{Except in the case where the residual function $\bF$ itself represents a derivative (gradient) of an objective function.  See Chapter \ref{chap:of}.}   There are two ways of providing $\bF$:  If the problem is based on a structured grid, as in \texttt{reaction.c} above, use \texttt{DMDASNESSetFunctionLocal()}.  In general, use \texttt{SNESSetFunction()}.  If only $\bF$ is provided, do not create or preallocate a \pMat for the Jacobian, as this is done internally by the \pSNES for \texttt{-snes\_fd} and \texttt{-snes\_fd\_color} runs, while no assembled Jacobian \pMat exists for \texttt{-snes\_mf} runs.
...
A boolean variable, set by run-time option \texttt{-rct\_noRinJ}, modifies the way the diagonal of the Jacobian is computed in \texttt{FormJacobianLocal()}.  These lines appear in Code \ref{code:reactionjacobian}:
\begin{code}
    if (user->noRinJ)
        v[1] = 2.0;
    else
...
\end{cline}
%$
The count of Newton iterations\sidenote{And the time; not shown.} is reduced to almost the same as for the exact Jacobian case (page \pageref{etc:nl:bestreaction}).
...
\begin{cline}
$ ./ecjac -snes_type test -snes_test_display
\end{cline}
%$
...
% from petsc-users: From: Matthew Knepley <knepley@gmail.com>
%Subject: Re: [petsc-users] snes set function and Jacobian concurrently
%On Mon, Apr 18, 2016 at 9:34 AM, Hoang Giang Bui <hgbk2008@gmail.com> wrote:
