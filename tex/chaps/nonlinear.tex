...
Evaluating $\bF$ can dominate the work in the Newton iteration, and there are systems where it is an intrinsically-expensive function to evaluate.  The work done in solving linear system \eqref{eq:nl:newton:eq} is the other main concern.  A good \PETSc habit, to start right now, is to use \texttt{-log\_view} to see which kind of work actually dominates.
