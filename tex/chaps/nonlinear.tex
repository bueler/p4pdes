...
The simplest thing to say about nonlinear equations is that, when compared to the linear case, they change the functional form of the residual.  For a linear system the residual $\br$ is a certain linear (affine) function of the unknowns $\bu$, namely $\br = \bb - A \bu$.  Now we consider cases in which the residual, as a function of $\bu$, might be a polynomial, a transcendental function, or something more general.
...
Though the previous paragraphs give an adequate text summary of program \texttt{reaction.c}, the diagram in Figure \ref{fig:nl:reactionstack} communicates the overall structure much better.  Our program refers to six different major \PETSc types, of which three (\pDMDA, \pSNES, \pVec) involve explicit \texttt{Create()}/action/\texttt{Destroy()} sequences.  Additionally, as noted above, we set entries in a \pMat which is allocated internally.  However, it is essential to remember that each step of Newton's method solves a linear system, and thus that \pKSP and \pPC objects are ``inside'' the \pSNES.  We will see later, both in this and future Chapters, when seeking better performance on fine grids, that exposure and control of these linear solver objects is essential.
...
\path
   ...
   (ksp) edge node {} (pc);
\end{tikzpicture}
\caption{An overview of \texttt{reaction.c}, our first structured-grid solution of a nonlinear PDE using \PETSc.  Arrows should be read as ``user code acts directly on.''  Compare Figure \ref{fig:st:linearstack}.}
