...
Options \texttt{-da\_grid\_x MX -da\_grid\_y MY} set the grid at runtime, and we can examine both the grid (i.e.~the \pDM object) and our assembled \pMat graphically to check these objects.  If the X window system is correctly linked in your \PETSc installation then
\begin{cline}
...
\item If you have difficulty reproducing timings like those in Table \ref{tab:cgdirectmg}, note that they come from a \PETSc configuration with ``optimized'' configuration option\sidenote{\emph{Not} runtime option!} \verb|--with-debugging=0|.  If you have not already done so, see the \PETSc installation page
\begin{quote}
