...
A ``movie'' of the \pKSP iterates $\{\bu_k\}$ comes from running
\begin{cline}
...
\path
   ...
   ([xshift=-1em]usercode.south) edge[->] node {} (matj)
...
In fact, so as to exploit the power of geometric-multigrid preconditioning (Chapter \ref{chap:pr}), our future codes will do a better job of using \PETSc's tools for assembling the finite linear system.  Specifically, the \pDMDA object will ``tell us,'' via ``call-back,'' the grid on which we are assembling the linear system, something which is not done in \texttt{poisson.c}.  The simplest, most-uniform way to do this turns out to be via a generalization which we want anyway: allowing nonlinearity in our PDEs.  That is, starting in the next Chapter, nonlinear problems are addressed using a \pSNES object which is already set-up for call-backs.  Using nonlinear tools from \PETSc, even for linear problems, is simply more convenient if we want geometric-multigrid preconditioners as well.
