...
The fifth and sixth arguments ``\texttt{-9}'' of \texttt{DMDACreate2d()} in Code \ref{code:dmdacreatetwod} are used to set default dimensions $m_x=m_y=9$.  Thus
\begin{cline}
$ mpiexec -n 4 ./poisson
\end{cline}
%$
distributes the default $9\times 9$ grid in a reasonably well-balanced manner among four processes.  (These defaults can be overridden by runtime options \texttt{-da\_grid\_x} and \texttt{-da\_grid\_y}.)  Figure \ref{fig:unitsquaregrideight} shows the perfectly-balanced result of
\begin{cline}
...
In Code \ref{code:dmdacreatetwod}, the second and third arguments are \texttt{DM\_BOUNDARY\_NONE} because our Dirichlet boundary conditions do not need communication to the next process' domain, nor periodic wrapping.  In our FD method below we treat the nodes along the boundary as unknowns so that we do \emph{not} need \PETSc to supply ghost values at additional boundary nodes.
...
\caption{Ordering of unknowns, $k= j\,m_x + i$ shown in bold, on a $m_x=4$ and $m_y=3$ grid.}
...
\section{Runtime control and visualization}
...
\end{cline}
%$
Recall that a default $9\times 9$ grid was chosen in calling \texttt{DMDACreate2d()}.  Seven iterations of the KSP method yield a small final residual norm and an apparently small numerical error.
...
We can generate a X windows ``movie'' of \pKSP iterates $\{\bu_k\}$ on a further-refined grid:
\begin{cline}
$ ./poisson -da_refine 4 -ksp_monitor_solution draw -draw_pause 0.1
on 129 x 129 grid:  error |u-uexact|_inf = 3.76924e-06
\end{cline}
%$
The last frame is shown as a contour map in Figure \ref{fig:st:poisson4contour}.

\begin{figure}
\includegraphics[width=0.9\textwidth]{figs/poisson4contour}
\caption{Contours of the computed solution $u(x,y)$ on a $129\times 129$ grid.}
\label{fig:st:poisson4contour}
\end{figure}

As in Chapter \ref{chap:ls}, we can visualize the \pKSP convergence.  Recall that a line graph of the (preconditioned) residual norm comes from \texttt{-ksp\_monitor\_lg\_residualnorm} (not shown).

We have delayed questions of convergence and efficiency until runtime.  Indeed, the \PETSc paradigm is that choices of solver and parameters should be made at runtime, once we have adequately-described the problem in terms of \PETSc objects.  Here is what we might want to know now:\begin{itemize}
...
\end{itemize}
In the next three sections we address these in turn, though our stab at ``efficiency'' is only preliminary.  Attaining efficiency requires better choices for preconditioners than we have now.
...
From Table \ref{tab:poissontiming} we see that for GMRES, having a preconditioner is essential.  That is, ILU substantially reduces both iteration count and time compared to no preconditioner.
...
Unfortunately, even with the preconditioners tried above, CG has a distinct flaw.\sidenote{At least from a 21st-century point of view.  Table \ref{tab:poissontiming} might have been satisfactory in 1970.}  Namely, the iteration count grows with refined grids, so that \citep[p.~76]{Elmanetal2005}
\begin{quote}
...
Specifically, suppose we run \texttt{poisson} with option \texttt{-da\_refine 7}, to give a $1025 \times 1025$ grid and about $10^6$ degrees of freedom.  Suppose we compare ICC-preconditioned CG iterations with two other methods:
\begin{cline}
...
Option \texttt{ -pc\_factor\_mat\_ordering\_type nd } indicates that the direct method be applied using the \emph{nested dissection} ordering \citep{George1973}.  While we give no details, in outline the unknowns and equations are re-ordered to reduce the cost of the factorization.  For LU factorization the nested dissection ordering is actually the \PETSc default for single-process runs.\sidenote{The observant reader may have noted the omission of \texttt{-ksp\_type preonly -pc\_type lu} from Table \ref{tab:poissontiming}.}  The two direct solvers in Table \ref{tab:cgdirectmg} are clearly competitive with ICC-preconditioned CG.

The impulsive reader should not give up on Krylov methods; that would be premature.  However, we do need to find a better preconditioner if we are going to have scalable solutions of PDE problems.  Chapter \ref{chap:pr} shows such methods, which give both efficient and parallel-scalable (Chapter \ref{chap:sc}) solutions of large nonlinear and variable-coefficient elliptic PDE problems in 2D and 3D.  This is something direct methods of linear algebra do not achieve.
...
\draw[line width=1pt] (3,4) node[userfcn,minimum width=70mm] (usercode) {user code \\ \vspace{1mm}};

\draw[line width=1pt] (-1,1.7) node[component] (ksp)  {\complabel{\pKSP}{linear solver}};
\draw[line width=1pt] (-1,0) node[component] (pc)   {\complabel{\pPC}{preconditioner}};

\draw[line width=1pt] (7,2) node[component] (dmda) {\complabel{\pDMDA}{structured grid}};

\draw[line width=1pt] (2,0) node[component] (matj)   {\usedlabel{\pMat}{$A$}};
\draw[line width=1pt] (6,0) node[component] (vecs)   {\pVecs \\ \footnotesize  \emph{solution, other fields}};

\path
   ([xshift=-10em]usercode.south) edge[->] node {} (ksp)
   ([xshift=-3em]usercode.south) edge[->] node {} (matj)
   ([xshift=2em]usercode.south) edge[->] node {} (vecs)
