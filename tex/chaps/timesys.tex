...
In the interior we choose a particular formula for the source term, which takes both positive and negative values, $f(x,y) = 3 e^{-25(x-0.6)^2} \sin(2 \pi y)$, and a non-homogeneous boundary condition as $\gamma(y) = \sin(6\pi y)$.  It is easy to verify (Exercise \ref{chap:ts}.\ref{exer:ts:zerototalsource}) that the totals of these sources is zero:
\begin{equation}
\int_{\mathcal{S}} f(x,y)\,dx\,dy = 0, \qquad \int_0^1 \gamma(y)\,dy = 0. \label{eq:ts:zerototalsource}
\end{equation}

Physically-speaking, the total thermal energy of the solution is proportional to the integral of $u$ over $\mathcal{S}$.  (More precisely, for a material with density $\rho$ and heat capacity $c$ the energy (joules) contained in the material  $\rho c$ times the integral of $u$.)  On the other hand, the derivation of heat equation \eqref{eq:ts:heatequation} arises from the principles of conservation of internal (thermal) energy \citep{Ockendonetal2003}.  Thus equation \eqref{eq:ts:heatequation} determines the evolution of that total energy, namely
\begin{equation}
\frac{d}{dt} \left(\int_{\mathcal{S}} u(t,x,y)\,dx\,dy\right) = FIXME
\end{equation}

FIXME demo energy monitor
