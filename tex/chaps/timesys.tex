...
We now have the tools to return to solving \emph{partial} differential equations.  The well-known idea \citep{HundsdorferVerwer2003,LeVeque2007,MortonMayers2005} in this subsection is that a time-dependent PDE generates an arbitrarily-large system of ODEs when the space variables are discretized.
...
\end{equation}
We seek the temperature $u(t,x,y)$ for $(t,x,y) \in [0,T] \times \mathcal{S}$ where $\mathcal{S} =(0,1)\times (0,1)$ is the unit square.  In this particular case the \emph{diffusivity} $D_0>0$ is constant and the \emph{source term} $f(x,y)$ is time-independent.

We can convert a time-dependent PDE like \eqref{eq:ts:heatequation} into a system of ODEs by \emph{semi-discretization in space}, also known as the \emph{method of lines}.\sidenote{Visualize the continuous-time, discrete-space ODE domain as lines $(t,x_i,y_j)$.}  Our discretization of spatial derivatives in \eqref{eq:ts:heatequation} is the same as in Chapter \ref{chap:st}.  We use \pDMDA to put a grid of $m_x m_y$ points labeled $(x_i,y_j)$ on $\mathcal{S}$; see Figure \ref{fig:unitsquaregrid}.  Then we replace the spatial derivatives in \eqref{eq:ts:heatequation} by finite differences while keeping the time derivatives.  If $h_x,h_y$ is the grid spacing then
\begin{equation}
...
Equations \eqref{eq:ts:molheat} is a system of ODEs which we can solve explicitly or implicitly.  Our code will allow either, and there is no need to think about the time-integration method now.  We only need to put the equations into the form of an ODE system \eqref{eq:ts:ode}.  First, however, we need to fully-specify a well-posed problem, including a concrete source term, boundary conditions, and initial conditions.

For variety, and as a representative example of real-world applications, we construct a problem where the exact solution is not known,\sidenote{\emph{Knowable}, perhaps, but we do not bother.} but where a scalar quantity is conserved.  We choose\sidenote{Again, for variety.  Also we can demonstrate this feature of \pDMDA.} periodic boundary conditions along the $y=0,1$ top and bottom boundaries and Neumann boundary conditions along the other boundaries.  On the left ($x=0$) we have a non-homogeneous Neumann boundary condition, and on the right a homogeneous one.   See Figure \ref{fig:ts:heatsquare}.
...
In the interior we choose a particular formula for the source term, which takes both positive and negative values,
\begin{equation}
f(x,y) = FIXME \label{eq:ts:sourceformula}
\end{equation}
It is easy to verify (Exercise \ref{chap:ts}.\ref{exer:ts:zerototalsource}) that the total input from the source term is zero:
\begin{equation}
\int_{\mathcal{S}} f(x,y)\,dx\,dy = 0. \label{eq:ts:zerototalsource}
\end{equation}

Physically-speaking, the FIXME
