...
Recalling Table \ref{tab:ts:odebasictypes}, we now consider other ODE methods.  They are compared by number of steps $M$ and---this is very informal---by their relative wall-clock time $R$:\sidenote{Thus $R=1$ for a method which is the same speed as backward Euler, $R<1$ for a faster method, and $R>1$ for a slower method.}
