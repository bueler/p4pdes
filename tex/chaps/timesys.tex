...
A time-dependent PDE like \eqref{eq:ts:heatequation} becomes a system of ODEs by \emph{semi-discretization in space}, also known as the \emph{method of lines}.  (One visualizes the continuous-time, discrete-space ODE domain as lines $(t,x_i,y_j)$ in the three-dimensional space-time.)

For the example here we discretize spatial derivatives in \eqref{eq:ts:heatequation} the same way we did in Chapter \ref{chap:st}.  We use a \pDMDA to put a grid of $m_x m_y$ points labeled $(x_i,y_j)$ on $\mathcal{S}$; see Figure \ref{fig:unitsquaregrid} in Chapter \ref{chap:st}.  Then we replace the spatial derivatives in \eqref{eq:ts:heatequation} by centered finite differences while keeping the time derivatives unmodified.  If $h_x,h_y$ are the grid spacings then we get
\begin{equation}
...
Equations \eqref{eq:ts:molheat} form a system of ODEs in form \eqref{eq:ts:ode}, which we can solve explicitly or implicitly, but there is no need to think about numerical time-integration now.\sidenote{Time-stepping-method agnosticism is central to the method of lines.}  We do need, however, to fully-specify a well-posed problem, including concrete source term, boundary conditions, and initial conditions.

For variety, and as an example which is representative of real-world applications, we specify a problem where the exact solution is not known\sidenote{The exact solution is \emph{knowable}, perhaps, but we do not bother.} but where a scalar quantity is conserved.  We choose, arbitrarily but also for a bit of variety, to have periodic boundary conditions along the $y=0$ and $y=1$ (top and bottom) boundaries and to have Neumann boundary conditions along the other boundaries.  On the left ($x=0$) we have a non-homogeneous Neumann boundary condition, and on the right ($x=1$) a homogeneous one.   See Figure \ref{fig:ts:heatsquare}.
...
In the interior we choose a particular formula for the source term which takes both positive and negative values, $f(x,y) = 3 e^{-25(x-0.6)^2} \sin(2 \pi y)$.  For the non-homogeneous boundary condition,
	$$\frac{\partial u}{\partial n} = -\frac{\partial u}{\partial x} = \gamma(y),$$
where $n$ denotes the outward unit normal direction, we choose $\gamma(y) = \sin(6\pi y)$.  It is easy to verify (Exercise \ref{chap:ts}.\ref{exer:ts:zerototalsource}) that the totals of these particular sources are zero:
\begin{equation}
\int_{\mathcal{S}} f(x,y)\,dx\,dy = 0, \qquad \int_0^1 \gamma(y)\,dy = 0. \label{eq:ts:zerototalsource}
\end{equation}

Physically-speaking, total thermal energy is proportional to the integral of $u$ over $\mathcal{S}$.  (For a material with density $\rho$ and heat capacity $c$ the energy (joules) contained in the material is $\rho c$ times the integral of $u$.)  On the other hand, heat equation \eqref{eq:ts:heatequation} is derived from the principle of conservation of internal (thermal) energy \citep{Ockendonetal2003}.  In fact, equation \eqref{eq:ts:heatequation} determines the evolution of that total energy, and it turns out that it is independent of time because of our particular choices:
\begin{align}
\frac{d}{dt} \left(\int_{\mathcal{S}} u(t,x,y)\,dx\,dy\right) &= D_0 \int_{\partial \mathcal{S}} \frac{\partial u}{\partial n}\,ds + \int_{\mathcal{S}} f(x,y)\,dx\,dy \label{eq:ts:conservedscalar} \\
   &= D_0 \int_0^1 \gamma(y)\,dy + \int_{\mathcal{S}} f(x,y)\,dx\,dy \notag \\
   &= 0. \notag
\end{align}
See Exercise \ref{chap:ts}.\ref{exer:ts:zerototalsource}.

FIXME want discrete version to conserve too; demo energy monitor
...
\item \label{exer:ts:zerototalsource}  Show \eqref{eq:ts:zerototalsource}.  Then fill in the details to show \eqref{eq:ts:conservedscalar}, taking note that the periodic boundary condition amounts to treating those ``boundary'' locations as interior points of a (topologically) cylindrical surface.
