...
To the reader has become lost in the maze of issues which have built up around the examples so far---Krylov solvers, preconditioner choices, line searches, finite element details, or whatever causes confusion---some good news:  In \emph{this} Chapter we restart with an easy question and a small example.  Solving \emph{ordinary} differential equations (ODEs) with \PETSc should be easy.  It is, and so is making the transition, through spatial discretization, to numerical methods for time-dependent, parabolic PDEs like the heat equation.

In this Chapter we look at three examples:
\begin{itemize}
\item a linear system of two ODEs,
\item an arbitrarily-large system of ODEs arising from spatial finite-difference approximation of the classical heat equation,
\item and another large system arising from two coupled nonlinear PDEs which form a two-species reaction/diffusion model.
\end{itemize}

These time-dependent problems are solved by runtime-chosen time-stepping methods.  Such methods come from a so-far unused major \PETSc object class: \pTS time-steppers.  Along the way we will recall the basics of \emph{explicit} and \emph{implicit} time-stepping techniques.  In the process we will end up returning to the scientific computing issues in which we have been engaged in previous Chapters, such as the rapid solution of large systems of equations.


\section{ODEs and \PETSc \pTS objects}

Consider the ODE system in form
\begin{equation}
