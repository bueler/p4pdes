...
The last example, in which we solve the time-dependent heat equation \eqref{eq:ts:heatequation}, uses a method of lines discretization, a 2D \pDMDA, and centered finite differences to produce a stiff ODE system.  This next example does the same, but it is more interesting in three particular ways:
\begin{itemize}
\item it is a \emph{system} of two coupled PDEs
\item which are \emph{nonlinear} reaction-diffusion equations, and
\item we \emph{split} the stiff part of the equations from the non-stiff, so that implicit/explicit (IMEX) numerical methods give benefits from both strategies.
\end{itemize}

Consider the following PDEs from \citep{Pearson1993,HundsdorferVerwer2003}
\begin{align}
...
\end{align}
where $u(t,x,y),v(t,x,y)$ represent chemical concentrations defined for $t\in[0,T]$ and $(x,y)\in [0,L)\times [0,L)$, and where the boundary conditions are periodic.  Here $D_u>0,D_v>0$ are \emph{diffusion coefficients}, $\phi>0$ is the ``feed rate'' for the $u$ (chemical) species, and $\kappa>0$ is a rate constant for a chemical reaction that eliminates the $v$ species by converting it into an inert product \citep{Pearson1993}.  The interaction between the two species occurs at a rate $u v^2$ and this reaction effectively converts $u$ into $v$, so that ``$-u v^2$'' and ``$+u v^2$ appear in the two equations, respectively.

This simple reaction-diffusion model produces a ``surprising variety of irregular spatiotemporal patterns'' \citep{Pearson1993}, but of course one needs a computer to reveal them.  While it is less novel to solve this problem than in 1993, it is an excellent illustration of the value of implicit time-stepping in the presence of stiff diffusive terms.  Pearson used first-order forward Euler explicit time-stepping to do the numerical experiments, requiring a parallel supercomputer,\sidenote{Namely, a Connection Machine at the Los Alamos Advanced Computing Laboratory.} but will take time steps $1000$ times longer and do the computations easily at a small or large level of parallelism.

As explained for the heat equation, the Laplacian derivative $\grad^2$ generates a stiff ODE system when the method of lines is applied.  With this in mind, \PETSc allows us to write the discretized (i.e.~method of lines) system in the form
\begin{equation}
\bF(t,\by,\by') = \bG(t,\by). \label{eq:ts:pattern:imexform}
\end{equation}
This form can handle a larger class of problems than we need, including those \emph{differential-algebraic equations} in which $\partial \bF/\partial \by'$ is singular.  We do not need that capability, but the same form allows one to put the stiff terms in $\bF$ and the nonstiff in $\bG$, and have advanced ODE time-stepping methods apply different schemes to each part; such \emph{is} our purpose.

To be specific, and speaking conceptually, from discretizing \eqref{eq:ts:pattern:pde} we have a vector $\bu$ of the discrete values $u_{i,j}(t) \approx u(t,x_i,y_j)$ and a similar vector $\bv$.  Together these are the state variables for the ODE: 
\begin{equation}
\by = \begin{bmatrix} \bu \\ \bv \end{bmatrix}.  \label{eq:ts:pattern:blockvars}
\end{equation}
To implement the method-of-lines discretized form of \eqref{eq:ts:pattern:pde} we write functions \texttt{FormIFunction()} which computes
\begin{equation}
...
\end{equation}
That is, the components $u,v$ are interleaved.  (Regarding the particular order on the grid, which is intrinsic to \pDMDA, see Chapter \ref{chap:st}.)

However, we do \emph{not} need to be aware of the order in memory to write the code.  Indeed, use of \texttt{DMDAVecGetArray()} and \texttt{MatSetValuesStencil()} allow us to refer to values of $u$ and $v$ by the component name (i.e.~using C \texttt{struct} member notation ``\texttt{.u}'' or ``\texttt{.v}''; see below) and the grid indices $i,j$.

We do write some Jacobian code, but only for the stiff part $\bF$.  The function \texttt{FormIJacobianLocal()} computes
\begin{equation}
...
\cinputpart{pattern.c}{\CODELOC}{FIXME}{III}{//RHSFUNCTION}{//ENDRHSFUNCTION}{code:ts:pattern:rhsfunction}

\cinputpart{pattern.c}{\CODELOC}{FIXME}{IV}{//IFUNCTION}{//ENDIFUNCTION}{code:ts:pattern:ifunction}
