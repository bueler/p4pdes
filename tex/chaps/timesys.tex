...
The second command may be a surprise, but it turns out there is little reason to design \PETSc examples for the linear case only.  By setting the problem type to \texttt{TS\_NONLINEAR} we are saying that the  problem is in a general form \eqref{eq:ts:ode} with a function $\bg(t,\by)$ which will be supplied by user code.\sidenote{We will see in later examples that \pTS of type \texttt{TS\_NONLINEAR} can have even more general forms than \eqref{eq:ts:ode}.}
