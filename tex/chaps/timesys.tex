...
and initial step dt=0.01 (so D0 dt / (min{dx,dy}^2) = 0.16) ...
  energy = 0.00e+00
0 TS dt 0.01 time 0.
...
\section{Performance, stability, and stiffness}

It is time to recall some of the theory from earlier in this Chapter, and apply it to this heat equation example.  To summarize the example, the method of lines converts the heat equation \eqref{eq:ts:heatequation} into a large, stiff, and linear system of ODEs in form \eqref{eq:ts:ode}.  If we choose an implicit time-stepping method then \PETSc allows us to solve equations \eqref{eq:ts:implicitgenericsystemAGAIN}, which must be solved at each time step, in parallel using the stack of tools shown in Figure \ref{fig:ts:stack}.  The code \texttt{heat.c} sets the implicit, A-stable, and stiff-decay backward Euler method as the default method (\texttt{-ts\_type beuler}).
...
\end{cline}
%$
This run, which takes $M=10$ steps to reach the final time $t_f=0.1$ using an $O(\Delta t^1)$ method, gives reasonable results in a few seconds, on a reasonably-fine grid, using the default settings.

Recalling Table \ref{tab:ts:odebasictypes}, we consider some other ODE methods.  They are compared by relative time $R$---where $R>1$ is the factor by which another method is slower---and number of steps $M$.
\begin{itemize}
\item \texttt{-ts\_type rk}  \hfill $R=4.5$, $M=5201$
\begin{quote} The \RKthreebs adaptive method.  It detects the stiffness and takes many inappropriately-small time steps, with larger total run-time. \end{quote}
\item \texttt{-ts\_type rk -ts\_adapt\_type none}  \hfill $R=0.012$, $M=10$
\begin{quote} This is the same method but with adaptivity turned off.  The solution explodes to size $10^{72}$ after $M=10$ steps!  The fact that it is fast is irrelevant.\end{quote}
\item \texttt{-ts\_type cn}  \hfill $R=0.84$, $M=10$
\begin{quote} This is the $\theta=0.5$ method with $O(\Delta t^2)$ local truncation error.  Visualization shows a problem.  Instead of smoothly growing in size as in the first run, features oscillate unstably.  This is a well-known consequence of a method which is A-stable, so that the oscillations do eventually decay---but without the stiff-decay property. \end{quote}
\item FIXME
\end{itemize}

FIXME runs show $D_0 \Delta t / \Delta x^2$; explain

\begin{code}
suppress wobbles by weighting implicitness more: -ts_type theta -ts_theta_theta 0.7 -ts_theta_endpoint

better adaptive than rk: -ts_type bdf -ts_bdf_adapt -ts_bdf_order 3
\end{code}
