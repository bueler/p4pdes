...
Thus we learn about all the basic controls.  For example, to see numerical values of the solution at each time-step, do
\begin{cline}
...
We are free to adjust the start time, end time, and initial time step at run-time:
\begin{cline}
...
\end{cline}
%$
which generates 200 steps of length \texttt{dt}$=0.1$, exactly as we requested.  On the other hand, we can change the key tolerance used in adaptive time-stepping.  One way is to choose the relative tolerance to be much larger or much smaller than the default of $10^{-4}$,
\begin{cline}
$ ./ode -ts_monitor -ts_rtol 1.0e-1
$ ./ode -ts_monitor -ts_rtol 1.0e-10
\end{cline}
%$
These give 18 and 119 steps, and final numerical errors of $1.2$ and $3.4\times 10^{-3}$, respectively.
...
A careful study of the relative performance of \pTS methods on this first ODE example problem is now possible, of course.  We encourage the reader to pursue it, but we will only return to that after introducing implicit time-stepping and a fundamental concerns with ODE solution methods, namely stability and stiffness.

Before moving on we mention an additional mode of visualization of the solution trajectory.  Figure \ref{fig:ts:ode} was produced by using the script \texttt{plottrajectory.py} in the same directory \texttt{c/ch6/}, like this:
\begin{cline}
$ ./ode -ts_monitor binary:t.dat -ts_monitor_solution binary:y.dat
$ ./plottrajectory.py -o figure.png t.dat y.dat
\end{cline}
Running the Python script \texttt{plottrajectory.py} requires copies of, or sym-links to, the Python tools \texttt{PetscBinaryIO.py} and \texttt{petsc\_conf.py} in the \texttt{bin/} directory of your \texttt{\$PETSC\_DIR}.
...
\section{Time-stepping stability and implicitness}  With the exception of the backward Euler method \eqref{eq:ts:backwardeuler}, all of the methods mentioned above are \emph{explicit}.  They can ultimately be written as single formulas which compute $\bY_\ell$ from previous solution values, merely by composing the functions in the statement of the scheme.  For example, the explicit trapezoid rule \eqref{eq:ts:rk2a} can be written
\begin{equation*}
\bY_\ell = \bY_{\ell-1} + \frac{h}{2} \bg(t_{\ell-1},\bY_{\ell-1}) + \frac{h}{2} \bg\big(t_\ell,\bY_{\ell-1} + h\, \bg(t_{\ell-1},\bY_{\ell-1})\big).
\end{equation*}
In terms of their tableau, explicit Runge-Kutta schemes only have non-zero coefficients $a_{ij}$ strictly below the diagonal.

\emph{Implicit} methods require solving equations.  Backward Euler, for instance, can be written as the (generally) nonlinear system
    $$\bF(\bY_\ell)=0$$
for the $N$ real variables, the components of $\bY_\ell$, with $\bF(\bX) = \bX - \bY_{\ell-1} - h \bg(t_\ell,\bX)$.  We will solve such systems, one at each time-step, by Newton's method, naturally.

A family of implicit schemes are the \emph{theta methods}, so-named merely because of their usual parameter:
\begin{equation}
\bY_\ell = \bY_{\ell-1} + (1-\theta) h \bg(t_{\ell-1},\bY_{\ell-1}) + \theta h \bg(t_\ell,\bY_{\ell}).  \label{eq:ts:theta}
\end{equation}
These methods are one-step methods with tableau
\begin{center}
\begin{tabular}{c|cc}
$0$   \\
$1$ & $1-\theta$ & $\theta$ \\ \hline
...
\end{tabular}
\end{center}
We have already listed the $\theta=0$ and $\theta=1$ cases, namely the forward \eqref{eq:ts:forwardeulerupdate} and backward \eqref{eq:ts:backwardeuler} Euler methods; they are first-order accurate.  The $\theta=1/2$ method is the \emph{trapezoid} method, or the \emph{Crank-Nicolson} method to users familiar with PDE cases \citep{MortonMayers2005}, and it is second order $O(\Delta t^2)$.

But the explicit version \eqref{eq:ts:rk2a} is also second order, and so why would one go to the effort of solving a system of equations at each time step?  The answer is that \emph{stability} is obligatory in a numerical scheme while higher-order is only desireable.  Explicit schemes are stable with sufficiently-small time steps, but taking many small time steps is a computational burden.  Before we can address the trade-off between these computational burdens, we relate \emph{stiffness} of certain ODE problems and stability of schemes.
...
\item FIXME which \texttt{-ts\_type} work with \texttt{ode.c}?  do \texttt{-help |grep ts\_type} to find possibilities, and try.  what work with additional option \texttt{-snes\_fd}?  with \texttt{odejac.c}?
