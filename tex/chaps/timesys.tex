...
\section{Runs of \texttt{heat.c}; visualization}

A helpful first run, which serves as a reminder of the \pTS/\pSNES/\pKSP/\pPC/etc.~stack shown in Figure \ref{fig:ts:stack}, is
\begin{cline}
$ ./heat -ts_view
\end{cline}
%$
...
A view of the time-stepping action, including convergence of the backward Euler equations at each step and the output of the energy monitor, comes from
\begin{cline}
...
\end{cline}
%$
Because the initial thermal energy is zero, without rounding error or any implementation errors we should see ``\texttt{energy = 0.00e+00}'' at every time step.  Noting the coarseness of the grid, and the correspondingly large magnitude of any errors of size comparable to truncation error, the above result shows energy is conserved to within rounding error.

At each step we see two Newton iterations even though the system of equations to solve at each time step is linear; see \eqref{eq:ts:heatequation}.  The resolution of this situation is either to ask for a direct solve of the equations at each time step, or to tighten the tolerance for the Krylov iteration:
\begin{cline}
...
\end{cline}
The results (not shown) in both cases reveal one \pSNES iteration per time step.  The latter technique is generally more scalable to large grids.

FIXME parallel run showing \pDMDA layout
\begin{cline}
$ mpiexec -n 4 ./heat -da_refine 5 -dm_view -ts_monitor
\end{cline}
%$

FIXME run-time graphics
\begin{cline}
$ ./heat -da_refine 5 -ts_monitor_solution draw -draw_pause 0.1
\end{cline}
%$
For example, Figure \ref{fig:ts:heatfinaltime} shows a grayscale version of the last frame frame of the run-time movie
\begin{cline}
./heat -da_refine 5 -ts_final_time 0.02 -ts_dt 0.001 -ts_monitor -ts_monitor_solution draw
\end{cline}

\begin{figure}
\includegraphics[width=0.8\textwidth]{figs/heatfinaltime}
\caption{Temperature as grayscale from a \texttt{heat.c} run.}
...
The rest of the results FIXME no exact; visualize with \texttt{-ts\_monitor\_solution draw} to see claimed instability effects

FIXME stdout shows $D_0 \Delta t / \Delta x^2$; explain
