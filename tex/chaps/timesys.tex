...
It should be no surprise that there are methods with improved accuracy compared to the Euler methods.  Among these are the \emph{one-step multi-stage} methods, with the \emph{Runge-Kutta} family best known.  They use $\bY_{\ell-1}$ only, and not previous values, to build the next approximation $\bY_\ell$ from multiple evaluations of the right-hand side $\bg$.

\newcommand{\RKtwoa}{RK$2$a\xspace}
\newcommand{\RKfour}{RK$4$\xspace}

For example, for a time step denoted simply as $h=\Delta t_\ell$, the \emph{explicit trapezoidal} method \citep{AscherPetzold1998} is a second-order accurate rule which takes a forward Euler step but then ``goes back'' and recomputes the step by the average of values of $\bg$:
\begin{align}
...
The well-known fourth-order Runge-Kutta method ``\RKfour'' has four stages.  Instead of stating the scheme with formulas like those in \eqref{eq:ts:rk2a} above, we describe a one-step, $s$-stage method by $s+1$ formulas
\begin{align}
...
\end{tabular}
\end{center}
For example, the $s=2$ method \RKtwoa is the left tableau in Table \ref{tab:ts:exampletableau}, and the $s=4$ method \RKfour is the middle tableau.  Note that in both of these methods $\hat\bY_1=\bY_{\ell-1}$, so that the $s+1$ evaluations of $\bg$ suggested by the general formulas \eqref{eq:ts:rkgeneral} are, in fact, only $s$ evaluations.
...
$1$ & $1-\theta$ & $\theta$ \\ \hline
    & $1-\theta$ & $\theta$
\end{tabular}
\caption{Butcher tableau for the explicit trapezoidal rule (\RKtwoa; left), explicit fourth-order Runge-Kutta method (\RKfour; middle) and the implicit $\theta$ method (right) in which $0 < \theta \le 1$.} \label{tab:ts:exampletableau}
\end{table}

A one-step, multi-stage method described by \eqref{eq:ts:rkgeneral}, or the above tableau, computes a new value $\bY_\ell$ via intermediate results $\hat\bY_i$ along the way.  A primary purpose of introducing the tableau form is to suggest the large number of methods which are possible, and thus to suggest, by implication at least, that choosing the \emph{best} technique for a given ODE system is rarely a reasonable goal.  Our goal in this Chapter, in any case, is to suggest some reasonable default choices for broad classes of problems.  And we now invoke the usual \PETSc-user idea: many command-line, run-time options is \emph{not a bad thing}.

To further clarify the language for describing ODE schemes, we note that the \emph{second-order Adams-Bashforth} method
\begin{equation}
...
FIXME: mention adaptive via embedded methods, and show 3bs:
%       A[4][4] = {{0,0,0,0},
%159:                  {1.0/2.0,0,0,0},
%160:                  {0,3.0/4.0,0,0},
