...
\end{equation}
In short, a scheme has stiff decay if it can ``follow'' any nonhomogeneity $\gamma(t)$, that is, in the limit of sufficiently-negative eigenvalues (i.e.~$\Re \lambda < 0$) in the ODE.

The backward Euler has stiff decay, but the trapezoid rule does not \citep{AscherPetzold1998}.  For this reason the backward Euler scheme is often a ``stable method of last resort'' in solving PDE (or PDE-like) problems with diffusive or heat-like character and ``rough'' data in the problem.  The diffusivity implies that the high spatial frequencies are strongly damped, i.e.~$\Re(\lambda)\ll 0$.  See Chapter \ref{chap:co} for an example.
