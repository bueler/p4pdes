...
In summary, a one-step, multi-stage method, whether described by \eqref{eq:ts:rkgeneral} or a tableau, computes a new value $\bY_\ell$ via intermediate results $\hat\bY_i$.  For a more interesting example of this approach, we describe the default time-stepping scheme in \PETSc, an \emph{adaptive} Runge-Kutta scheme which is \emph{embedded}.  Let us give meaning to these terms.
