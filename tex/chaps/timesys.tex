...
\section{Methods and accuracy for ODE initial value problems}  The above example ODE system is straightforward to approximate by any number of time-stepping numerical methods.  They all start from the initial values and use the right-hand-side function $g(t,\by)$ to update an approximate solution.  That is, they generate sequences $\{\bY_\ell\}$ which approximate the solution at discrete times $t_\ell$, so that $\bY_\ell \approx \by(t_\ell)$.  Because these ideas are easy to find in the literature\sidenote{A good reference is \citep{AscherPetzold1998}, for example.}, and are likely to be in the reader's background in some form, we only supply a gloss here.
...
\end{equation}
but much more work is required to use it in practice.  It is an \emph{implicit} method, as explored below.  At each time step \eqref{eq:ts:backwardeuler} generates exactly the kind of (generally) nonlinear system, with unknown values $\bY_\ell$, which we solved in previous Chapters by Newton's method and \pSNES.
...
\end{align}
Though two stages must be computed at each time step, only the final result $\bY_\ell$ is used to inform the next time step (i.e.~which computes $\bY_{\ell+1}$).  In this sense the method is ``one-step'' despite being multi-stage.  Method \eqref{eq:ts:rk2a} is also called ``\RKtwoa'' because it is one of several possible second-order Runge-Kutta methods.

The well-known fourth-order Runge-Kutta method ``\RKfour'' has four stages.  Instead of stating the scheme with formulas like those in \eqref{eq:ts:rk2a} above, we define tabular notation for any one-step, $s$-stage method.  Namely, the $s+1$ formulas
\begin{align}
...
\end{align}
are completely described by a \emph{Butcher tableau} \citep{Butcher1987}
\begin{center}
...
\end{tabular}
\end{center}
For example, the $s=2$ method \RKtwoa is the left tableau, and the $s=4$ method \RKfour is the middle tableau, in Table \ref{tab:ts:exampletableau}.  Note that, in all of the methods in Table \ref{tab:ts:exampletableau}, in fact $\hat\bY_1=\bY_{\ell-1}$.
...
\end{tabular}
\medskip
\caption{Butcher tableau for the explicit trapezoidal rule (\RKtwoa; left), explicit fourth-order Runge-Kutta method (\RKfour; middle) and the implicit $\theta$ method ($0 < \theta \le 1$; right).} \label{tab:ts:exampletableau}
\end{table}

In summary, a one-step, multi-stage method, whether described by \eqref{eq:ts:rkgeneral} or a Butcher tableau, computes a new value $\bY_\ell$ via intermediate results $\hat\bY_i$.  It helps, in further clarifying the language for describing ODE schemes, to mention alternative families of schemes.  For example, the \emph{second-order Adams-Bashforth} method
\begin{equation}
\bY_\ell = \bY_{\ell-1} - \frac{h}{2} \bg(t_{\ell-2},\bY_{\ell-2}) + \frac{3 h}{2} \bg(t_{\ell-1},\bY_{\ell-1})  \label{eq:ts:ab2}
\end{equation}
is a \emph{multi-step} method.  It computes a new value $\bY_\ell$ with only one additional evaluation of $\bg$ (i.e.~one stage), but using two previous values $\bY_{\ell-2},\bY_{\ell-1}$.  Note this works in a straightforward way only if the step-size $h=\Delta t$ is independent of $\ell$.  A goal of such methods is to achieve order greater than one with only one evaluation of $\bg$ per time step, and this is often achieved in practice.  However, in our casual treatment of time-stepping, we will say no more about multi-step methods (but see Table \ref{tab:ts:tstypes} below).

We should mention the default Runge-Kutta scheme in \PETSc, which is not one of the above.  It is \emph{adaptive} by being an \emph{embedded} method, and it is worthwhile giving basic meaning to these terms.  An ODE numerical solution method is \emph{adaptive} if it adjusts the time step by a procedure which uses information the already-computed approximate solution.  A Runge-Kutta method is \emph{embedded} if it is in fact two methods which have the same ``$c_j$'' and ``$a_{ij}$'' parts of their tableau, but different ``$b_j$'' and different order of accuracy.  The Bogacki-Shampine Runge-Kutta scheme \RKthreebs, the \PETSc default, is a pair of schemes, each four stage, but one of third order and one of second order.  Their tableau are shown in Table \ref{tab:ts:tableaurk3bs}.
...
\medskip
\caption{Tableau for the embedded four-stage, third-order \RKthreebs scheme, the \PETSc default.} \label{tab:ts:tableaurk3bs}
\end{table}

The first idea behind this pair of schemes is that the difference of their computed values for the next time, that is, the difference of their $\bY_\ell$ values, provides a basis on which to decide if the time step was short enough to satisfy the error tolerance (goal).  In this case, one scheme makes $O(\Delta t^2)$ numerical error and the other one $O(\Delta t^3)$.  The difference can be turned into a bound on the error in from the lower order scheme; the details do not concern us here but see [CITE FIXME].  The second idea is that the work of computing two approximations is nearly the same as just one, because they share the same locations for evaluations of $\bg$; they are ``embedded.''  Indeed it is a general theme that the cost of an ODE numerical method can be understood by counting distinct evaluations of the right-hand side of the ODE, and not accounting for the additional arithmetic represented by the coefficients in the tableau.

Clearly there are a very large number of possible methods, whether one-step or multi-step, adaptive or not, and so on.  Choosing the ``best'' one for a given ODE system is rarely a reasonable goal.  Many different methods are available from the \PETSc command line, and our goal in this Chapter is only to suggest some reasonable default choices for broad classes of problems.  We also invoke the essential \PETSc mantra: a rich set of command-line, run-time options is \emph{not a bad thing}.

\section{Implicit time-stepping schemes, and scheme stability}
...
FIXME
\end{equation}
Shown as right-hand tableau in Table \ref{tab:ts:exampletableau}.
