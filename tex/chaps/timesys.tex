...
\end{equation}
We seek the temperature $u(t,x,y)$ at times $t \in [0,T]$ and points $(x,y)$ in the unit square $\mathcal{S} =(0,1)\times (0,1)$.  For simplicity we set the \emph{diffusivity} $D_0>0$ to be constant and the \emph{heat source} $f(x,y)$ to be time-independent.

Any time-dependent PDE like \eqref{eq:ts:heatequation} becomes a system of ODEs by \emph{semi-discretization in space}, also known as the \emph{method of lines}.  (The latter name imagines the continuous-time, discrete-space ODE domain as lines $(t,x_i,y_j)$ in the three-dimensional space-time.)

For the example here we discretize spatial derivatives the same way we did in Chapter \ref{chap:st}.  We use a \pDMDA to put a grid of $m_x m_y$ points $(x_i,y_j)$ on $\mathcal{S}$.  (Compare Figure \ref{fig:unitsquaregrid} in Chapter \ref{chap:st} and Figure \ref{fig:ts:heatdefaultgrid} below.)  Then we replace the spatial derivatives in \eqref{eq:ts:heatequation} by centered finite differences while keeping the time derivatives; they become ordinary derivatives.  If $h_x,h_y$ are the grid spacings then we get
\begin{equation}
...
Equations \eqref{eq:ts:molheat} are actually a system of ODEs in form \eqref{eq:ts:ode}.  We can solve it numerically, explicitly or implicitly, but in fact there is no need to think about time-stepping methods now.\sidenote{This agnosticism is central to the method of lines.}  We do, however, need to fully-specify a well-posed problem.  We need a concrete source term, boundary conditions, and initial conditions.

For variety, and as an example which is representative of real-world applications, we specify a problem without a known\sidenote{The exact solution is \emph{knowable}, perhaps, but we do not bother.} exact solution, but in which a scalar quantity is conserved.  We choose, somewhat arbitrarily but also for variety, to have periodic boundary conditions along the top and bottom boundaries; see Figure \ref{fig:ts:heatsquare}.  On the left ($x=0$) we choose a non-homogeneous Neumann condition, and on the right ($x=1$) a homogeneous Neumann one.
...
For the boundary condition along the left side we set
	$$-\frac{\partial u}{\partial x} = \gamma(y)$$
...
\frac{d}{dt} \left(\int_{\mathcal{S}} u(t,x,y)\,dx\,dy\right) &= D_0 \int_0^1 \gamma(y)\,dy + \int_{\mathcal{S}} f(x,y)\,dx\,dy \label{eq:ts:conservedscalar} \\
   &= 0. \notag
\end{align}
(Again, see Exercise \ref{chap:ts}.\ref{exer:ts:zerototalsource}.)  We want our discretized equations \eqref{eq:ts:molheat} to conserve energy too; see the ``energy monitor'' below.


\section{Implementation}

The program \texttt{c/ch5/heat.c} shown in Codes \ref{code:ts:heat:heatctx}--\ref{code:ts:heat:monitor} is straightforward to write based on our Chapter \ref{chap:st} and \ref{chap:nl} experience with \pDMDA and our experience with \pTS in this Chapter.  The overall structure of \texttt{heat.c}, in terms of \PETSc components, is shown in Figure \ref{fig:ts:stack}.
...
Code \ref{code:ts:heat:heatctx} shows the context \texttt{struct}, so that call-backs have access to the constant $D_0$ and functions $f$, $\gamma$.  Code \ref{code:ts:heat:dmdasetup}, which is an extract of \texttt{main()}, sets up the grid \pDMDA object.  The grid is shown in Figure \ref{fig:ts:heatdefaultgrid}; grid points along the lower boundary $y=0$ are also on the upper boundary $y=1$.  By specifying periodicity, ghost points are set up so that the $(x_i,y_{j\pm 1})$ neighbor of a grid point $(x_i,y_j)$ is always available.\sidenote{This applies both in serial and in parallel.}  The default grid of $m_x \times m_y = 5\times 4$ points, set by arguments to \texttt{DMDACreate2d()} as shown in Code \ref{code:ts:heat:dmdasetup}, actually has square cells.  In fact, the spacings of the grid are $1/(m_x-1)$ and $1/m_y$ respectively---see the helper function \texttt{Spacings()} in \texttt{heat.c} (not shown).

\begin{figure}
% usage: \standardTSstack{scale}{Jacobian}{DMDA}
\standardTSstack{0.775}{}{}
\caption{The \PETSc ``stack'' used for time-dependent, structured-grid PDE examples \texttt{heat.c,pattern.c}.}
\label{fig:ts:stack}
\end{figure}
...
The next part (Code \ref{code:ts:heat:tssetup}) creates and configures the \pTS.  A new call, compared to previous \pTS examples anyway, is \texttt{TSSetDM()}.  (The \pTS determines the size $N=m_x m_y$ of the system of ODEs through this association to a \pDMDA grid object, for example.)  The same extract also shows the calls, specific to this context with both \pDMDA and \pTS, for assigning ``local'' grid-aware functions \texttt{FormRHSFunctionLocal()} and \texttt{FormRHSJacobianLocal()}; these appear in Codes \ref{code:ts:heat:rhsfunction} and \ref{code:ts:heat:rhsjacobian}.  Also we set the default \pTS type to backward Euler, because the system \eqref{eq:ts:molheat} is stiff and that method has stiff decay, though of course we may override this choice.
...
\cinputpart{heat.c}{\CODELOC}{Evaluate the Jacobian of the ODE RHS.}{III}{//RHSJACOBIAN}{//ENDRHSJACOBIAN}{code:ts:heat:rhsjacobian}

A feature of this example (Code \ref{code:ts:heat:monitor}) is to add a energy ``monitor'' call-back to the \pTS object to check that the total energy is constant in the discretized system.
