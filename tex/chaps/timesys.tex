...
Any time-dependent PDE like \eqref{eq:ts:heatequation} becomes a system of ODEs by \emph{semi-discretization in space}, also known as the \emph{method of lines}.  (This name imagines the continuous-time, discrete-space ODE domain as lines $(t,x_i,y_j)$ in the three-dimensional space-time.)

For the example here we discretize spatial derivatives the same way we did in Chapter \ref{chap:st}.  We use a \pDMDA to put a grid of $N = m_x m_y$ points $(x_i,y_j)$ on $\mathcal{S}$.  (Compare Figure \ref{fig:unitsquaregrid} in Chapter \ref{chap:st} and Figure \ref{fig:ts:heatdefaultgrid} below.)  Then we replace the spatial derivatives in \eqref{eq:ts:heatequation} by centered finite differences while keeping the time derivatives as is; in fact they become ordinary derivatives.  If $h_x,h_y$ are the grid spacings then we get
\begin{equation}
...
Equations \eqref{eq:ts:molheat} form a system of ODEs, i.e.~equation \eqref{eq:ts:ode}.  We can solve the system numerically, explicitly or implicitly, but there is no need to think about time-stepping methods now.\sidenote{This deliberate ambiguity about time-stepping is central to the method of lines.}  We do, however, need to fully-specify a well-posed problem.  We need boundary conditions, an initial condition, and a concrete source term.

For variety, and as an example which is representative of real-world applications, we specify a problem \emph{without} a known\sidenote{The exact solution is \emph{knowable}, perhaps, but we do not bother.} exact solution, but in which a scalar quantity is conserved.  Somewhat arbitrarily, but also for variety, we have periodic boundary conditions along the top and bottom boundaries.  On the left ($x=0$) and right ($x=1$) we choose non-homogeneous and homogeneous Neumann conditions, respectively.  As a simple initial condition we choose zero temperature: $u(0,x,y)=0$.
...
The program \texttt{c/ch5/heat.c} shown in Codes \ref{code:ts:heat:heatctx}--\ref{code:ts:heat:monitor} is straightforward to write based on our experiences in Chapters \ref{chap:st} and \ref{chap:nl} with \pDMDA and with \pTS in this Chapter.  Note that Code \ref{code:ts:heat:heatctx} simply shows a context \texttt{struct}, from which call-backs have access to the constant $D_0$ and functions $f$, $\gamma$.   The overall structure of \texttt{heat.c}, in terms of \PETSc components, is shown in Figure \ref{fig:ts:stack}.
...
The next part (Code \ref{code:ts:heat:tssetup}) creates and configures the \pTS.  The \pTS determines the size $N=m_x m_y$ of the system of ODEs through an association to the \pDMDA grid object, set by calling \texttt{TSSetDM()}.  The same extract also assigns ``local'' grid-aware functions \texttt{FormRHSFunctionLocal()} and \texttt{FormRHSJacobianLocal()}  describing the right-hand side function $\bg(t,\by)$ defined by the finite difference formula \eqref{eq:ts:molheat}.  Also we set the default \pTS type to backward Euler, a stiff-decay method which can handle the stiff system \eqref{eq:ts:molheat}; of course we may override this choice.
...
Codes \ref{code:ts:heat:rhsfunction} evaluate the RHS $\bg(t,\by)$ of ODE system \eqref{eq:ts:molheat}, and Code \ref{code:ts:heat:rhsjacobian} evaluates its derivatives (Jacobian).  The finite difference formula \eqref{eq:ts:molheat} applies at most points in a straightforward way, both in computing the function and the derivative matrix; it is the same 5-point stencil shown in Figure \ref{fig:unitsquaregridstencil}.

At the left and right boundaries, however, a Neumann condition is applied.  For example, at a grid point $(x_0,y_j)=(0,y_j)$ we have \eqref{eq:ts:leftneumannbc}, which becomes
     $$- \frac{u_{+1,j} - u_{-1,j}}{2 h_x} = \gamma(y_j),$$
using a centered finite difference approximation and a notional value ``$u_{-1,j}$'' \citep{MortonMayers2005}, or equivalently $u_{-1,j} = u_{+1,j} + 2 h_x \gamma(y_j)$.  This replacement for ``$u_{-1,j}$'' is put into \eqref{eq:ts:molheat} so that it becomes meaningful.  Note that in parallel only some MPI processes own the $i=0$ and $i=m_x-1$ parts of the grid, and thus access to \pVec \texttt{u} must be protected by conditionals.
...
A feature of this example is an energy ``monitor'' call-back so that we can see if the total energy is constant in the discretized system.  Code \ref{code:ts:heat:tssetup} shows that we use \texttt{TSMonitorSet()} to set a call-back to our function \texttt{EnergyMonitor()}, and Code \ref{code:ts:heat:monitor} shows the function itself.  We apply the trapezoid rule for integration to do the integral.\sidenote{This quadrature can also be interpreted as the midpoint rule, in a grid-point-centered cell interpretation, with modifications at the boundaries.}  \texttt{EnergyMonitor()} is also one of those rare cases of a ``raw'' MPI call.  We put each process' contribution to the integral into \texttt{lenergy} and then do \texttt{MPI\_Allreduce()} to get the value \texttt{energy} for the whole (global) grid.
...
\section{Runs with \texttt{heat.c}}
