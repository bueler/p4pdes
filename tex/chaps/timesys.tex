...
The last example, in which we solve the time-dependent heat equation \eqref{eq:ts:heatequation}, uses a MOL discretization, a 2D \pDMDA, and centered finite differences to produce a stiff ODE system.  Our next example does the same, but it is more interesting in three particular ways:
\begin{itemize}
\item it is a \emph{system} of two coupled PDEs
\item which are \emph{nonlinear} reaction-diffusion equations, and
\item we \emph{separate the stiff part} so that implicit/explicit (\emph{IMEX}) numerical methods can give benefits from both strategies.
\end{itemize}

Consider the following system of two coupled, scalar PDEs \citep{Pearson1993,HundsdorferVerwer2003}
\begin{align}
...
\end{align}
Here $u(t,x,y)$ and $v(t,x,y)$ represent chemical concentrations defined for $t\in[0,T]$ and $(x,y)$ in the a square domain $\mathcal{S} = [0,L)\times [0,L)$.  The boundary conditions on $\mathcal{S}$ are periodic, so in effect there is no boundary.  The \emph{diffusion coefficients} $D_u>0,D_v>0$ are generally different for the two chemicals.  Constant $\phi>0$ describes the ``feed rate'' for $u$, while $\kappa>0$ gives the rate of a chemical reaction that eliminates $v$ by converting it into an inert product \citep{Pearson1993}.  The interaction between the two species, which converts $u$ into $v$, occurs at a rate $u v^2$.

This reaction-diffusion model produces a ``surprising variety of irregular spatiotemporal patterns'' \citep{Pearson1993}, but of course one needs a computer to reveal them.  This problem makes an excellent illustration of the value of implicit time-stepping in the presence of stiff diffusive terms.  While Pearson used explicit, first-order forward-Euler to do the numerical experiments, which required a parallel supercomputer,\sidenote{Namely, a Connection Machine at the Los Alamos Advanced Computing Laboratory.} we will take implicit or partially-implicit time steps $1000$ times longer.

As in the heat equation, the Laplacian $\grad^2$ in \eqref{eq:ts:pattern:pde} generates a stiff ODE system when MOL is applied.  With this in mind, \PETSc allows us to write the discretized system in the form
\begin{equation}
\bF(t,\by,\by') = \bG(t,\by). \label{eq:ts:pattern:imexform}
\end{equation}
This form is suitable for a larger class of problems than we need, including \emph{differential-algebraic equations} (DAEs) wherein $\partial \bF/\partial \by'$ is singular.  On the other hand the same form allows one to put the stiff terms in $\bF$ and the nonstiff in $\bG$, so that IMEX methods can apply different schemes to each part; such \emph{is} our purpose.  (Note that form \eqref{eq:ts:ode} is the case of \eqref{eq:ts:pattern:imexform} in which $\partial \bF/\partial \by'$ is the identity.  If $\partial \bF/\partial \by'$ is invertible then in theory \eqref{eq:ts:pattern:imexform} can be converted to \eqref{eq:ts:ode}, but this is often impractical.)  In the particular case of the MOL discretization of \eqref{eq:ts:pattern:pde}, either form would work but the IMEX methods need \eqref{eq:ts:pattern:imexform}; see Exercise \ref{chap:ts}.\ref{exer:ts:patternrhsonlyform}.

\cinputpart{pattern.c}{\CODELOC}{\texttt{struct}s: for point-wise values of the solution and a ``context'' for use by call-backs.}{I}{//FIELDCTX}{//ENDFIELDCTX}{code:ts:pattern:fieldctx}

To get started immediately with the implementation, Code \ref{code:ts:pattern:fieldctx} shows the definition of \texttt{struct}s named \texttt{Field} and \texttt{PatternCtx}.  The former is the point-wise value of the solution of PDE \eqref{eq:ts:pattern:pde}, that is, a pair $(u,v)$.  The latter is the standard ``context'' struct so that call-backs can get the values of the constants in \eqref{eq:ts:pattern:pde}.

Now consider the spatial discretization of \eqref{eq:ts:pattern:pde}.  Speaking conceptually, this results in a vector $\bu$ of the discrete values $u_{i,j}(t) \approx u(t,x_i,y_j)$ and a similar vector $\bv$.  Together these form the state vector of the ODE system \eqref{eq:ts:pattern:imexform}, 
\begin{equation}
\by = \begin{bmatrix} \bu \\ \bv \end{bmatrix}.  \label{eq:ts:pattern:blockvars}
\end{equation}
To implement this MOL-discretized form we will write \texttt{FormIFunction()} which computes
\begin{equation}
\bF(t,\by,\by') = \by' - \begin{bmatrix} D_u A \bu \\ D_v A \bv \end{bmatrix},  \label{eq:ts:pattern:blockifunction}
\end{equation}
where $A$ is the centered finite-difference approximation of the scalar Laplacian $\grad^2$, and \texttt{FormRHSFunction()} which computes
\begin{equation}
...
\end{equation}
(These functions appear in Codes \ref{code:ts:pattern:ifunction} and \ref{code:ts:pattern:rhsfunction} below.)

Forms \eqref{eq:ts:pattern:blockvars}--\eqref{eq:ts:pattern:blockrhsfunction} are, in fact, only notional.  The actual order of the grid values in memory is
\begin{equation}
\by = \begin{bmatrix} u_{0,0} \\ v_{0,0} \\ u_{1,0} \\ v_{1,0} \\ \vdots \end{bmatrix}. \label{eq:ts:pattern:interleaved}
\end{equation}
That is, the components $u,v$ are interleaved.  (The particular grid order comes from the \pDMDA; see Chapter \ref{chap:st}.)  However, we do \emph{not} need to be aware of the order in memory to write the code.  Indeed, use of \texttt{DMDAVecGetArray()} and \texttt{MatSetValuesStencil()}---see below in Code \ref{code:ts:pattern:tssetup}---allow us to refer to values of $u$ and $v$ by the component name (i.e.~using C \texttt{struct} member notation ``\texttt{.u}'' or ``\texttt{.v}''; see below) and the grid indices $i,j$.

We will also write some Jacobian code, but only for the stiff part $\bF$.  The function \texttt{FormIJacobianLocal()}---see Code \ref{code:ts:pattern:ijacobian}---computes
\begin{equation}
J = (\text{shift}) \frac{\partial \bF}{\partial \by'} + \frac{\partial \bF}{\partial \by}
\label{eq:ts:pattern:jacobianshift}
\end{equation}

\cinputpart{pattern.c}{\CODELOC}{FIXME}{II}{//TSSETUP}{//ENDTSSETUP}{code:ts:pattern:tssetup}

\cinputpart{pattern.c}{\CODELOC}{FIXME}{III}{//IFUNCTION}{//ENDIFUNCTION}{code:ts:pattern:ifunction}

\cinputpart{pattern.c}{\CODELOC}{FIXME}{IV}{//RHSFUNCTION}{//ENDRHSFUNCTION}{code:ts:pattern:rhsfunction}

\cinputpart{pattern.c}{\CODELOC}{FIXME}{V}{//IJACOBIAN}{//ENDIJACOBIAN}{code:ts:pattern:ijacobian}
...
\begin{figure}
%\includegraphics[width=0.45\textwidth]{figs/snakes}
\caption{FIXME}
...
\begin{cline}
$ mpiexec -n 4 ./pattern -ptn_noisyinitial -ptn_phi 0.05 -ptn_kappa 0.063 -ts_monitor -ts_monitor_solution draw -da_refine REF -ts_final_time TIME 
\end{cline}
%$
where REF=7 and TIME=5000(?)
