...
It should be no surprise that there are methods with improved accuracy compared to the Euler methods.  Among these are the \emph{one-step multi-stage} methods, with the \emph{Runge-Kutta} family best known.  One-step methods use $\bY_{\ell-1}$ only, and not previous values, to build the next approximation $\bY_\ell$, but generally from multiple evaluations of the right-hand side $\bg$.  For example, denoting the time-step simply as $h=\Delta t_\ell$, the \emph{explicit trapezoidal} method \citep{AscherPetzold1998} is a second-order rule which takes a forward Euler step but then ``goes back'' and recomputes the step by the average of values of $\bg$:
\begin{align}
...
For example, the $s=2$ method \RKtwoa is the left tableau in Table \ref{tab:ts:exampletableau}, and the $s=4$ method \RKfour is the middle one.  Spaces in the tableau correspond to zeros.  Note that, in all of the methods in Table \ref{tab:ts:exampletableau}, in fact $\hat\bY_1=\bY_{\ell-1}$, which is why the first rows are trivial.  Also, because these schemes are \emph{explicit} they have nonzero $a_{ij}$ only strictly below the diagonal; we return to this idea below.
...
In summary, a one-step, multi-stage method, whether described by \eqref{eq:ts:rkgeneral} or a tableau, computes a new value $\bY_\ell$ via intermediate results $\hat\bY_i$.  For a more interesting example of this approach, we should mention the default time-stepping scheme in \PETSc, not yet described.  It is an \emph{adaptive} Runge-Kutta scheme which is \emph{embedded}.  Let us give meaning to these terms.

An ODE numerical solution method is \emph{adaptive} if it adjusts the time step by a procedure which uses information from the already-computed values.  An adaptive scheme should take shorter steps when the values from $\bg$ are rapidly-changing or irregular, and longer steps when they are smoother and more predictable.  Successfully implementing such strategies, the \emph{error estimation and control} \citep{AscherPetzold1998} done internally by the \PETSc solvers, not visible in our user code,\sidenote{Thank goodness!} is nontrivial and outside our scope.

One strategy to make a Runge-Kutta method adaptive is to choose an \emph{embedded} method.  Such a scheme is in fact two methods which have the same $c_j$ and $a_{ij}$ parts of their tableau (see equation \eqref{eq:ts:rkgeneral}), but different $b_j$ \emph{and} different orders of accuracy.  The Bogacki-Shampine scheme \RKthreebs, the \PETSc default, is an embedded pair, each four stage, but one of third order and one of second order.  Their tableau are shown at right in Table \ref{tab:ts:exampletableau}, with the two different $b_j$ values listed on different rows below the horizontal line.  The difference of the computed values from these two schemes provides a basis on which to decide if the time step was short enough to satisfy the error tolerance/goal \citep{Butcher1987}.  In \RKthreebs one scheme (first $b_j$ row) makes $O(\Delta t^3)$ local truncation errors, while the other (second $b_j$ row) makes $O(\Delta t^2)$ errors.  The difference is turned into an estimate of the error committed by the lower order scheme.  Also the work of computing these two approximations is the same cost as just one, because they share the same locations for evaluations of $\bg$; this is the central meaning of ``embedded.''  Further details of the \RKthreebs scheme, including its stability and efficiency properties, are definitely beyond our scope, but see \citep{BogackiShampine1989}.

In any case, it is a general theme that the cost of an ODE numerical method arises from the (distinct) evaluations of the right-hand side $\bg$ of the ODE, especially if $\bg$ has significant nonlinearities.  The additional arithmetic in implementing the scheme, represented by the coefficients in the tableau, is usually less important.
...
\end{equation}
is a \emph{multi-step} method.  It computes a new value $\bY_\ell$ with only one additional evaluation of $\bg$ (i.e.~one stage), but using two previous values $\bY_{\ell-2},\bY_{\ell-1}$.  This works in a straightforward way only if the step-size $h=\Delta t$ is independent of $\ell$.  A goal of such methods is to achieve order greater than one with only one evaluation of $\bg$ per time step, and this is often achieved in practice.  Other schemes which combine the multiple stages and multi-step ideas include the \emph{general linear methods} \citep{Butcher1987}.  In our quick treatment of time-stepping, we will say no more about multi-step and general linear methods, but the latter appear as possible \PETSc options in Table \ref{tab:ts:tstypes} below.

Clearly there are a very large number of possible methods, whether one-step or multi-step, adaptive or not, and so on.  Many different methods are available from the \PETSc command line (below).  Choosing the ``best'' one for a given ODE system is rarely a reasonable goal.  Our goal in this Chapter is only to suggest some reasonable choices for broad classes of problems.  We also invoke an essential \PETSc mantra here, namely: a rich set of command-line, run-time options is \emph{not a bad thing}.
...
\cinputpart{ode.c}{\CODELOC}{FIXME}{III}{//JACOBIAN}{//ENDJACOBIAN}{code:ts:ode:jacobian}
