...
Solving \emph{ordinary} differential equations (ODEs) with \PETSc ought to be easy.  Given that we have practice with \PETSc objects, it is.  Furthermore, the transition to numerical methods for time-dependent, parabolic PDEs like the heat equation is straightforward.  In fact, in this Chapter we look at three examples:
\begin{itemize}
\item a linear system of two ODEs,
\item an arbitrarily-large system of ODEs arising from spatial finite-difference approximation of the time-dependent heat PDE in 2D,
...
The particular integration method used by the \pTS can be chosen at run-time.  We will recall the basics of both \emph{explicit} and \emph{implicit} time-stepping techniques.  Because implicit time-stepping implies the full stack of \pSNES/\pKSP/\pPC solvers for the (generally) non-linear equations at each time step, our experience with examples from Chapters \ref{chap:st}--\ref{chap:of} will all be useful.

We discretize the two PDE examples above in space only.  This \emph{method of lines} approach avoids any need to write code specific to a temporal discretization.

The third example above is the first \emph{system} of PDEs addressed so far.  Describing such a system using \PETSc calls requires some new constructs in the code.
...
This example is solved by \texttt{c/ch6/ode.c}, a program shown in Codes \ref{code:ts:ode:main} and \ref{code:ts:ode:callbacks}.  We will summarize user code actions on the \pTS object itself, and those parts of the user code which describe the right-hand-side $\bg(t,\by)$.
...
Regarding the problem type of the \pTS, it turns out there is little reason to design \PETSc examples for the linear case only.  By setting the problem type to \texttt{TS\_NONLINEAR} we are saying that the  problem is in form \eqref{eq:ts:ode} with a function $\bg(t,\by)$ which will be supplied by user code.\sidenote{In later examples, a \pTS of type \texttt{TS\_NONLINEAR} will have even more general forms than \eqref{eq:ts:ode}.}
...
Because an allocated \pMat must be supplied to \texttt{TSSetRHSJacobian()}, the above \pTS set-up lines are preceded, in Code \ref{code:ts:ode:main}, by a \texttt{Create/SetUp} sequence for \pMat \texttt{J}.
