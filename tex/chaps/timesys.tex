...
\end{cline}
%$
By contrast, for 200 steps of the implicit trapezoid (Crank-Nicolson) method we get two-digit accuracy:
\begin{cline}
...
\end{equation}
A proposal to use Newton's method to solve \eqref{eq:ts:implicitgenericsystemAGAIN} should not surprise the reader; see the previous Chapter.  An initial iterate is even obvious: $\bY_\ell^0 = \bY_{\ell-1}$, the last value of the solution.  Newton's method requires the Jacobian derivative of $\bF$, however.
...
\end{equation}
wherein $\bY_{\ell-1}$ is known from the previous time step.  The Jacobian of \eqref{eq:ts:thetasystem} is
   $$J(\bX) = \frac{\partial \bF}{\partial \bX} = I - \theta h \frac{\partial \bg}{\partial \by}(t_\ell,\bX).$$

There is no need, however, to write user code specific to the $\theta$ method or any other ODE method.  First, the \pTS contains a \pSNES inside, though we typically do not call it directly as it is used internally for implicit time-stepping.  Second, each \PETSc-implemented implicit time-stepping method in \pTS can convert user-supplied code for the Jacobian derivative of the right-hand side of \eqref{eq:ts:ode}, that is, code to compute
\begin{equation}
...
An example program \texttt{odejac.c} is shown in Codes \ref{code:ts:odejac:jacobian} and  \ref{code:ts:odejac:mat}.  More precisely, only the needed additions to \texttt{ode.c} are shown.

\cinputpart{odejac.c}{\CODELOC}{To do implicit time-stepping we add a C function \texttt{FormRHSJacobian()}, which computes the derivatives of the right-hand side of \eqref{eq:ts:ode}, namely $\partial \bg/\partial \by$, for the example ODE system \eqref{eq:ts:example}.}{I}{//JACOBIAN}{//ENDJACOBIAN}{code:ts:odejac:jacobian}

The new program calls \texttt{TSSetRHSJacobian()} to set a Jacobian call-back to our C function \texttt{FormRHSJacobian()}.  Our code merely sets the entries of the \pMat holding the derivative \eqref{eq:ts:rhsjacobian} of the right-hand side in Example ODE \eqref{eq:ts:example}.  An allocated \pMat, here called \texttt{J} for simplicity, is also supplied to \texttt{TSSetRHSJacobian()}, and we assign \texttt{J} to both the Jacobian and preconditioner-material arguments of \texttt{TSSetRHSJacobian()}; compare usage of \texttt{SNESSetJacobian()} and \texttt{DMDASNESSetJacobianLocal()} in programs in Chapter \ref{chap:nl}.

\cinputpart{odejac.c}{\CODELOC}{We also create a \pMat to hold $\partial \bg/\partial \by$, call \texttt{TSSetRHSJacobian()}, and reset the \pTS type to the implicit Crank-Nicolson ($\theta=1/2$) method.}{II}{//MATJ}{//ENDMATJ}{code:ts:odejac:mat}

The reader might note that \texttt{odejac.c} is quite similar in structure to \texttt{ecjac.c} in Chapter \ref{chap:nl}, but with calls to \pSNES replaced by corresponding calls to \pTS.

FIXME we assume the ``hard'' time dependent case of stiff + nonlinear

FIXME: table shows RK subtypes, and columns for order and adaptive; no ``common options''
...
$\theta>0$   FIXME                          & \texttt{theta}  & optional & FIXME \\
Crank-Nicolson                              & \texttt{cn}     & optional & 
\end{tabular}
\caption{Choosing some of the best-known ODE integration methods.  Explicit schemes are above the line and implicit below.} \label{tab:ts:odebasictypes}
\end{table}
...
\end{cline}
and the following is the adaptive form of Crank-Nicolson,
\begin{cline}
$ ./odejac -ts_type cn -ts_theta_adapt
\end{cline}
%$

If the reader has followed all of the above then it will also be clear that we could have used implicit time-stepping with the earlier, Jacobian-free program \texttt{ode.c}.  Though
\begin{cline}
$ ./ode -ts_type cn
\end{cline}
%$
...
\end{cline}
%$
all give the same result.\sidenote{Very nearly; see Exercise \ref{chap:ts}.\ref{exer:ts:fdjacobian}.}
