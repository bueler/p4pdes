...
We write Jacobian code only for the stiff part $\bF$, with the idea that only it needs to be treated implicitly for stability.  However, examples like \texttt{pattern.c}, which work with general ODE form \eqref{eq:ts:pattern:imexform}, require that the Jacobian function (\texttt{FormIJacobianLocal()} in Code \ref{code:ts:pattern:ijacobian}) computes a shifted combination of the derivatives of $\bF$:
\begin{equation}
