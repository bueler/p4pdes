...
By careful configuration, \PETSc can handle complex scalars, vectors, and matrices.  However, we use only real numbers in this book.

The unqualified word ``vector'' will be understood in this book to refer to a column vector of finite length, written $\RR^N$.  On the other hand we will also consider functions defined on domains in $\RR^d$, such as solutions to PDEs, and regard them as vectors in infinite-dimensional vector space.

We will use bold for vectors, but not for entries, thus for example $\bv\in\RR^N$ has entries\sidenote{We break tradition by numbering vector entries starting from zero.} $v_0,\dots,v_{N-1}\in \RR$.  The \emph{standard basis} for $\RR^N$ is the set of vectors $\{\be_i\}_{i=0}^{N-1}$ which have zero entries except for each vector having a one in the $i$th entry: $(\be_i)_j = \delta_{ij}$ (the Kronecker delta).
...
A \emph{vector norm} is a function $\|\cdot\|:\RR^N \to [0,\infty)$ that gives a well-defined length to a vector.  (Several axioms, including the triangle inequality, must be satisfied \citep{TrefethenBau1997}.)  For $1 \le p < \infty$ the following formula defines a norm,
\begin{equation}
\|\bv\|_p = \left(\sum_{i=0}^{N-1} |v_i|^p\right)^{1/p}.  \label{eq:ls:vectorpnorm}
\end{equation}
The $p=1,2$ cases are especially common in practical uses, but all vector $p$-norms can be computed with $O(N)$ work using \eqref{eq:ls:vectorpnorm}.  The maximum norm
\begin{equation}
