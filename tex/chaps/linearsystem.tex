...
At the core of many numerical PDE solution methods is the solution of a finite-dimensional linear system.  Because such linear systems become, generally, more-accurate representations of the PDE as their size goes to infinity, it has always been the case that one solves the largest linear systems which the available computing technology can handle.  In any case, solving such linear systems, using algorithms that have the potential to scale to very large sizes, for instance so that the vector solution of the system must be distributed across many processors to even fit in memory, represents the core technology in \PETSc.

In this Chapter we do review the basic ideas of numerical linear algebra, but we move relatively fast; we are done reviewing by page \pageref{subsec:ls:vecmatintro}!\sidenote{\citet{TrefethenBau1997} is thus recommended as a textbook-length treatment.}  Besides stating elementary definitions, our goal is to identify the fundamental constraints which makes solving large systems challenging.  Thus we assert ``facts of life'' about numerically-solving linear systems.  Then the central threads of iterative linear algebra are laid out, namely 1.~iteration to reduce the residual, 2.~preconditioning of systems to improve the convergence of the iteration, and 3.~the ``Krylov space'' idea, and its connection to polynomial approximation, that unifies iterative schemes under one (generous) umbrella.

We then introduce the \PETSc types for representing linear system objects, namely vectors and matrices.  We apply \PETSc to solve a small, fixed-size linear system.  This is followed by a somewhat more-representative example of a sparse, arbitrary-size, tridiagonal system.  The latter example is exploited for a first look at the comparative effectiveness of run-time options, namely Krylov-space and preconditioner methods.
...
A \emph{real vector space} is a set that is closed under addition and multiplication by \emph{scalars} (elements of $\RR$) and to which certain algebraic axioms apply.\sidenote{Namely, the existence of a zero vector plus the appropriate associative, distributive, and commutative axioms \citep{Strang2009}.}  The elements of the set are \emph{vectors}.

The unqualified word ``vector'' will be understood in this book to refer to a column vector of finite length, written $\RR^N$.  (On the other hand we will also consider functions defined on domains in $\RR^d$, such as solutions to PDEs, and regard them as vectors in infinite-dimensional vector space.)  We will use bold for vectors, but not for entries, thus for example $\bv\in\RR^N$ has entries $v_0,\dots,v_{N-1}\in \RR$; note that we break tradition by numbering vector entries starting from zero.  The \emph{standard basis} for $\RR^N$ is the set of vectors $\{\be_i\}_{i=0}^{N-1}$ which have zero entries except for each vector having a one in the $i$th entry: $(\be_i)_j = \delta_{ij}$ (the Kronecker delta).
...
A \emph{vector norm} is a function $\|\cdot\|:\RR^N \to [0,\infty)$ which assigns a well-defined length to a vector.  (Several axioms, including the triangle inequality, must be satisfied \citep{TrefethenBau1997}.)  For $1 \le p < \infty$ the following formula defines a norm,
\begin{equation}
\|\bv\|_p = \left(\sum_{i=0}^{N-1} |v_i|^p\right)^{1/p}.  \label{eq:ls:vectorpnorm}
\end{equation}
The $p=1,2$ cases are especially common in practical use, but all vector $p$-norms can be computed with $O(N)$ work using \eqref{eq:ls:vectorpnorm}.  The maximum norm also gets frequent application:
\begin{equation}
\|\bv\|_\infty = \max_{i=0,\dots,N-1} |v_i|.  \label{eq:ls:vectorinfnorm}
\end{equation}

(An interruption, regarding the way \PETSc represents the above ideas, is appropriate.   First, by careful configuration, \PETSc can handle complex scalars, vectors, and matrices, but we use only real numbers in this book.  Second, \PETSc computes any of the $p=1,2,\infty$ cases of the vector norms \eqref{eq:ls:vectorpnorm} or \eqref{eq:ls:vectorinfnorm} by a call to \texttt{VecNorm()} on a \PETSc \pVec object; see below for an introduction to \pVec objects.)
...
Generally, induced matrix norms are expensive to compute.  In most of the cases, as they are optimization problems, they take more work than the $O(MN)$ operations needed to examine all the entries of a (generic, dense) matrix $A\in \RR^{M\times N}$.  However, those matrix norms induced by the $p=1$ and $p=\infty$ vector norms are easy to compute by $O(MN)$ formulas \citep{TrefethenBau1997}.  The \emph{Frobenius} matrix norm can also be computed by a $O(MN)$ formula, though the norm itself is not induced from a vector norm \citep{TrefethenBau1997}.  Thus it is that \PETSc can compute $p=1,\infty$ and Frobenius matrix norms by a call to \texttt{MatNorm()} on a \PETSc \pMat object (see below) but not other induced norms.
