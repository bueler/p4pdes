...
\PETSc objects are generically distributed across, and accessible from, multiple MPI processes.  Therefore the first argument of \texttt{ObjectCreate()}, and of \texttt{MatCreate()} above, is an MPI communicator (``\texttt{COMM}'').  All processes in \texttt{COMM} must call the \texttt{ObjectCreate()} method.
...
\end{code}
That is, we can use $3$ or $1$, for the two ranks respectively, as the ``local size'' argument to \texttt{VecSetSizes()}, instead of \texttt{PETSC\_DECIDE}.\sidenote{Such inflexible coding is neither necessary, nor recommended.}  The resulting layout is shown in Figure \ref{fig:artificialmpitwoveclayout}.
...
% e1balanced.c
\item Modify \texttt{c1matvec.c} to read an integer option \texttt{-N}\sidenote{Use methods \texttt{PetscOptionsBegin()}, \texttt{PetscOptionsInt()}, and \texttt{PetscOptionsEnd()}.} and form an $N\times N$ matrix, with $-2$ on the diagonal and $1$ on the super- and sub-diagonals as before.  Choose any desired right-hand-side.  Use \PETSc runtime options to estimate the condition number of the matrix for sample $N$ values; the online \PETSc FAQ page will help with these options.
