...
\end{cline}
The former view shows the compressed sparse storage, with values as pairs with column index and value.  The latter view is a traditional (``dense'') display where zero values are shown.  In these cases the matrix was stored in \emph{serial} compressed sparse row format, the \texttt{MATSEQAIJ} type, because of the one-process execution.

\section{A bit more numerical linear algebra}

Gauss elimination \citep{TrefethenBau} is one way to solve a system, but many other methods are iterative, and they often use the \emph{residual}.  By definition, the residual of vector $\bu_0$ in linear system \eqref{introsystem} is the vector
\begin{equation}
...
\medskip
The above introduction of the residual, Richardson iteration, and preconditioning is hardly adequate.  But let us return to \PETSc codes and solve linear systems.  The reader can treat \PETSc's linear solvers as black boxes for now.

\section{Solve a small linear system in \PETSc}

We have already know how to create, fill, and destroy a \pVec object and a \pMat object.  The next code, \texttt{c1vecmatksp.c} shown in Figure \ref{code:vecmatksp}, does all these steps.  To solve the system, however, it adds one more \PETSc object, namely a Krylov space method object ``\pKSP'' which holds the linear system.

The \pKSP has the ability to solve a linear system by a method chosen at runtime.  Besides the promised \texttt{Create/SetFromOptions/Destroy} sequence for \pKSP, there are two more important commands.  The system is actually solved by
\begin{code}
KSPSolve(ksp,b,x);
\end{code}
which supplies the right-hand side of the system (i.e.~\pVec \texttt{b}) and space for the solution (\pVec \texttt{x}).

But first we must tell \pKSP about the matrix in the linear system.  For that we have the command
\begin{code}
KSPSetOperators(ksp,A,A);
\end{code}
Why do we list \texttt{A} twice, you say?  As in our brief introduction above, the linear system 


\inputwhole{../c/c1vecmatksp.c}{\texttt{p4pdes/c/c1vecmatksp.c}}{Solve a small linear system.}{code:vecmatksp}


FIXME: one and 2 processor

FIXME: with \texttt{-mat\_view} and \texttt{-vec\_view}

FIXME: with \texttt{-ksp\_view}

FIXME: serial default is \texttt{-ksp\_type gmres -pc\_type ilu}

FIXME: with \texttt{-pc\_type jacobi}
