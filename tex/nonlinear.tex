...
is of form \eqref{eq:nl:diffusionreaction} with $R(u) = - \rho \sqrt{u}$ and $f(x)=0$.  Because $R$ is non-increasing\sidenote{It is non-increasing for the $u\ge 0$ values for which it is defined.  If desired it can be extended as $R(u)=0$ for $u<0$, thereby becoming non-increasing and continuous for all of $\RR$.} and continuous if $\rho>0$, the corresponding Dirichlet problem is well-posed.  We are actually using \eqref{eq:nl:drsqrt} in a first numerical example, however, because we want to verify our numerical solution using an exact solution, and its particular form makes this easy.  The exact solution can be found by noting that both second-derivative and square-root operations will convert certain 4th degree polynomials into quadratic polynomials.
% this idea comes from an example in Ockendon
For verification purposes we also get the boundary conditions from the exact solution.  Therefore we substitute
\begin{equation}
u(x)=M(x+1)^4 \label{eq:nl:exactform}
\end{equation}
into \eqref{eq:nl:drsqrt} and find that if $M=(\rho/12)^2$, $\alpha=M$, and $\beta=16 M$ then \eqref{eq:nl:exactform} is an exact solution to the boundary value problem.

\cinputpart{reaction.c}{\CODELOC}{Call-back functions for \texttt{reaction.c}.}{I}{//CALLBACK}{//ENDCALLBACK}{code:reactionI}

Code \texttt{reaction.c} in Figures \eqref{code:reactionI} and \eqref{code:reactionII} solves this problem using a \pSNES object for the Newton iteration and a \pDMDA object for the finite-difference grid.  In more detail, the first part of the code in Figure \ref{code:reactionI} has three call-back methods which compute the initial iterate and exact solution (\texttt{InitialAndExactLocal()}), the nonlinear residual (\texttt{FormFunctionLocal()}), and the Jacobian (\texttt{FormJacobianLocal()}).  All of these are ``\texttt{Local}'' methods in the sense that the input to the call-back includes C pointers for the arrays instead of the \pVecs themselves.  Our implementation of \texttt{InitialAndExactLocal()} explains how this works for the \pSNES call-backs.  That is, as seen in Figure \ref{code:reactionII}, before this method is called we use \texttt{DMDAVecGetArray()} on the \pVecs which need reading and setting, namely \texttt{u} and \texttt{uexact}, respectively.  This gives pointers of type \texttt{PetscReal*} which are then handed to \texttt{InitialAndExactLocal()}.  The call-back done inside a \pSNES object treats the calls to \texttt{FormFunctionLocal()} and \texttt{FormJacobianLocal()} the same way.

Another aspect of these ``\texttt{Local}'' call-back methods is that we extract a \texttt{DMDALocalInfo} struct for passing into the call-back functions.  This means that the local part of the grid (i.e.~\texttt{info.xs} and etc.) and the global grid size (i.e.~\texttt{info.mx}) can be accessed without accessing the \pDMDA object itself inside the call-back functions.
...
\cinputpart{reaction.c}{\CODELOC}{In \texttt{main()} we create \pDMDA, then \pVecs, and then a \pSNES, solve the equation, and clean up.}{II}{//MAIN}{//ENDMAIN}{code:reactionII}
