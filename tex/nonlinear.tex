...
Observe that in \texttt{main()} in Figure \ref{code:reactionII} we use \texttt{DMDASNESSetFunctionLocal()} instead of \texttt{SNESSetFunction()} as in \texttt{ecjacobian.c} earlier.  There is a similar method for assigning the local Jacobian routine.  This is because the signature of the call-back is different and uses \pDMDA-specific pointers.  Also, though we implement a Jacobian, we do not allocate a \pMat for it because the \pDMDA has information about the grid and stencil that suffice to pre-allocate a \pMat internally.
