...
\end{equation}
models a time-independent combination of \emph{diffusion} and \emph{reaction} processes by the terms on the left, respectively.  The right-hand side $f(x)$ models an additional location-dependent \emph{source}.  If $R(u)+f(x)>0$ then the reaction produces $u$, while otherwise it destroys $u$.  Note that our sign convention is clearest in the time-dependent model ``$u_t = D u_{xx} + R(u) + f(x)$,'' a typical generalization of the one-dimensional time-evolving heat equation.  In such a case, where $u$ represents temperature and $R(u)$ represents a heat-producing or -absorbing temperature-dependent reaction, according to its sign, \eqref{eq:nl:diffusionreaction} is the model of steady state.

We should be concerned about solvability of \eqref{eq:nl:diffusionreaction} in the case where $R$ is an \emph{increasing} function of $u$.  If $R$ positive and increasing then the ability of the $-D u''$ term to ``damp-out'' maxima may be exceeded by the increasing production from large values of $u$.  If $R$ is negative and increasing then the analogous concern applies to minima of $u$.  These concerns are demonstrated by the example $R(u) = \lambda e^u$ with $\lambda>0$ in Exercise 4.\ref{exer:nl:bratu}, in which the problem is apparently not numerically-solvable for sufficiently-large $\lambda$.  

On the other hand, it is also fair to say that equation \eqref{eq:nl:diffusionreaction} is a nonlinear elliptic PDE,\sidenote{By mild abuse of the letter ``P''.} but in one-dimension.  From this point of view we can get the converse of our concern, namely that a non-increasing $R$ function generates unique existence.  Consider the Dirichlet boundary conditions case, for instance
\begin{equation}
u(0)=\alpha \quad \text{and} \quad u(1)=\beta,  \label{eq:nl:drbcs}
\end{equation}
where we have chosen a convenient interval $x\in[0,1]$ by shifting and scaling $x$ as needed.  In the case where $R(u)$ is continuous and \emph{non}-increasing, the nonlinear operator in \eqref{eq:nl:diffusionreaction} is strictly monotone \citep{KinderlehrerStampacchia1980}.  Because it is also coercive on the appropriate space, which says intuitively that the leading-order term is effective at damping out variations which generate a large norm of the first derivative $u'$, abstract arguments show well-posedness \citep[pages 93-94]{KinderlehrerStampacchia1980}.
% analysis in 1D, thinking about well-posedness, recall first integral:
...
\end{equation}
% R(u) = - rho sqrt(|u|) decreasing (so "-u''+F(u)=0", for K&S p 94, with F increasing)
is a particular case of problem \eqref{eq:nl:diffusionreaction} with $R(u) = - \rho \sqrt{u}$, which is non-increasing for the $u>0$ values for which it is defined, and $f(x)=0$.  We use this in a first numerical example because we want to verify our numerical solution using an exact solution; the  particular form of \eqref{eq:nl:drsqrt} makes this easy.

\cinputpart{reaction.c}{\CODELOC}{FIXME}{I}{//CALLBACK}{//ENDCALLBACK}{code:reactionI}

In fact, because both second-derivative and square-root operations can convert certain 4th degree polynomials into quadratic polynomials, an exact solution can be found by substituting a polynomial.  For verification purposes we can also get the boundary conditions from the exact solution.  Thus we substitute
\begin{equation}
u(x)=M(x+1)^4 \label{eq:nl:exactform}
\end{equation}
into \eqref{eq:nl:drsqrt} and find
\begin{equation}
M=(\rho/12)^2, \quad u_L=M, \quad \text{and} \quad u_R=16 M. \label{eq:nl:exactconds}
\end{equation}

Code \texttt{reaction.c} in Figures \eqref{code:reactionI} and \eqref{code:reactionII} solves this problem using a \pSNES object to manage the Newton iteration and a \pDMDA object for the finite-difference grid.  In more detail, FIXME: we have \texttt{Local} forms of \texttt{FormFunction/Jacobian()}, and in setting initial condition and exact solution we extract \texttt{DMDALocalInfo}
