...
\node at (-2.4,6.5) {colored columns of $J$:};
  ...
  \node at (6.5,0.5) {$0$};
\end{tikzpicture}
\caption{Graph $G(J)$ is built from the stencil and the grid, with an edge for every pair of unknowns that appears in an equation in system \eqref{eq:nl:equation}.  Coloring this graph---$c=3$ colors suffice in this case---also assigns colors to the columns of $J$ so that }
\label{fig:fdcolorreaction}
\end{figure}

Now suppose we (``properly'') color the vertices so that any two vertices which share an edge have distinct colors.  In this case the coloring uses $c=3$ colors, which in this simple case is equal to the minimum number needed, which is to say that the \emph{chromatic number} $\chi(G)=c=3$.  We have now also colored the columns of $J$, as shown in the bottom of the Figure, so that each row involves distinct colors.

We now generate $c$ vectors $v_k \in \RR^N$.  Each vector has a $1$ in entry $j$ if $k$ is the color of column $j$ in $J$.  In this $c=3$ case,
\begin{equation}
