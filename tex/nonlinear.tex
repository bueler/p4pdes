...
Actual practice for Newton's method is also not that complicated with \PETSc in hand, because it has a mature implementation of this core technology.  Despite the reputation of Newton iteration as fragile or scary, it works just fine on many nonlinear systems once one adds some protections.  Primarily, a line search may ``move'' a shorter distance to the new iterate than stated in \eqref{eq:nl:newtonupdate}; see page \pageref{sec:linesearch}.
...
\bF(\bx) = \begin{bmatrix}
           ...
           \end{bmatrix}
\end{equation}
for $\bx\in \RR^2$.\sidenote{Note $\bx$ is a column vector with components $x_0$, $x_1$: $\bx = [x_0 \,\, x_1]^\top$.}  Thus
\begin{equation}
...
We will compute the Newton iterates in the above example by using a nonlinear solver object of type \pSNES\sidenote{This acronym stands for ``scalable nonlinear equation solver.''} from \PETSc.  Note \pSNES has the usual \texttt{Create/SetFromOptions/Destroy} sequence.  Our code provides a function $\bF$ which is a ``call-back'' in the sense that we supply it to the \pSNES.  The \pSNES calls it with argument $\bx$ when it needs $\bF(\bx)$ during the Newton iteration.
...
To avoid conflicts with other methods which might want to read or write to the same memory, calls to \texttt{VecGetArray()} and \texttt{VecGetArrayRead()} are matched by \texttt{VecRestoreArray()} and \texttt{VecRestoreArrayRead()}.  These methods ``free-up''  the \pVecs, but they do not deallocate anything.\sidenote{\texttt{VecDestroy()} does that.}  In general:
\begin{quote}
...
To get started on nonlinear PDE problems we look at a simple two-point boundary value problem.  Suppose $u(x)$ is the density of some substance.  An equation of the form
\begin{equation}
...
On an $N$ point grid, the finite-difference scheme we propose for \eqref{eq:nl:diffusionreaction} is
\begin{equation}
...
Code \texttt{reaction.c} shown in Codes \ref{code:reactionI} and \ref{code:reactionII} solves this problem using \eqref{eq:nl:drfdscheme}, a \pSNES object for the Newton iteration, and a \pDMDA object for the grid.  Code \ref{code:reactionI} shows three call-back methods.  First we compute the initial iterate---a linear function connecting the boundary conditions---and the exact solution in the method \texttt{InitialAndExactLocal()}.  Then we have the residual (\texttt{FormFunctionLocal()}) and Jacobian (\texttt{FormJacobianLocal()}) evaluation functions.

These are ``\texttt{Local}'' methods in the sense that their inputs are C pointers for arrays instead of \pVecs.  Our implementation of \texttt{InitialAndExactLocal()} explains how this works.  As seen in Code \ref{code:reactionII}, before this method is called we use \texttt{DMDAVecGetArray()} on the \pVecs which need reading or setting, namely \texttt{u} and \texttt{uexact}.  This gives pointers of type \texttt{PetscReal*} which are then handed to \texttt{InitialAndExactLocal()}.  The call-back done by \pSNES on \texttt{FormFunctionLocal()} and \texttt{FormJacobianLocal()} works the same way.

Another aspect of these ``\texttt{Local}'' call-back methods is that a \texttt{DMDALocalInfo} struct is passed into the call-back functions.  The local part of the grid (i.e.~\texttt{info.xs} and etc.) and the global grid size (i.e.~\texttt{info.mx}) can be accessed through this struct without needing the \pDMDA object itself.\sidenote{Recall Figure \ref{fig:localpartofgrid}.}
...
However, when we look closer at the results we get from finite-difference evaluation of Jacobians we see that it is not really working.  For the  \texttt{reaction.c} example there are far too many function evaluations per Newton step to be practical.  This is because, in contrast to the earlier fixed-dimension examples, discretizing a PDE generates an arbitrarily-large number of unknowns.

First, look at the performance of the analytical Jacobian on a grid of about 8000 nodes:
...
\end{cline}
%$
There are four $\bF$ evaluations and three $J_{\bF}$ evaluations in this $0.04$ second run.

Now compare the finite-difference-Jacobian case.  At first it simply fails:
\begin{cline}
...
\end{cline}
%$
The problem is that a complete Jacobian evaluation using finite-difference formula \eqref{eq:nl:fdjac} is requiring about $N+1=8194$ evaluations of $\bF$ per Newton iteration, one for each column of $J_{\bF}$ plus one for the right-side evaluation in \eqref{eq:nl:newtoneq}.  Thus the default maximum allowed $\bF$ evaluation count for the \pSNES is exceeded with just two iterations.\sidenote{Again, ``\texttt{./reaction -help |grep snes\_}'' gets the default value of 10000.}  If we raise the function-count limit then we do get convergence, but at more than 100 times increase in run time compared to the analytical Jacobian run:
\begin{cline}
...
\end{cline}
%$
In this run we have evaluated $\bF$ about $3N \approx 25000$ times.  

Fortunately, given that it is desirable for initial prototyping and saving programmer effort, there is no need to abandon the finite-difference-Jacobian idea.  The additional tool needed to make it work is applicable to many PDE numerical schemes, especially on structured grids.  Namely, we can exploit the fact that a small set of grid values---a small stencil---is used in approximating the PDE at each location.\sidenote{Finite difference, element, and volume schemes for discretizing PDES all normally have this property.  Spectral methods, by contrast, have a large, sometimes global, stencil, and would not benefit.}

The idea is to ``color'' the nodes of the grid, and thus also the columns of the Jacobian, with a small set of colors so that each discrete equation uses nodes with distinct colors.  Code in \PETSc is already present to do such a coloring---adequately but not optimally---for structured grids.  From the coloring, evaluations of $\bF$ can be redesigned, relative to the basic formula \eqref{eq:nl:fdjac}, to allow the computation of many columns of $J_{\bF}(\bx)$ simultaneously; again this is already available in \PETSc.

The fact that minimizing the number of function evaluations in this context is equivalent to a graph coloring was first seen by \citet{ColemanMore1983}; see also the  more recent comprehensive review by \citet{Gebremedhinetal2005}.  Note that the ``colors'' are in fact just integers that we attach to each node/column.

For an example, consider the seven-point grid that comes from this command:\sidenote{Add \texttt{-mat\_view ::ascii\_dense} to see the successive Jacobian matrices from the Newton iteration.}
\begin{cline}
$ ./reaction -da_grid_x 7
\end{cline}
%$
Figure \ref{fig:stencilreaction} shows this grid and the three-point stencil for the numerical discretization scheme \eqref{eq:nl:drfdscheme}.   The stencil at $x_j$ shows that (generically) the equation involves three unknowns, namely $u_{j-1}$, $u_j$, and $u_{j+1}$.  Exceptions occur at the boundary nodes, where only two unknowns are involved.
...
We take the nodes to be the vertices of a graph.  We draw an edge between two vertices if the corresponding unknowns both appear in at least one equation, i.e.~in one instance of equation \eqref{eq:nl:drfdscheme}.  The result is the graph shown in the top of Figure \ref{fig:fdcolorreaction}.  Now suppose we \emph{color} the graph.  That is, integers are assigned to vertices so that any two vertices which share an edge have distinct values (colors).  Recalling that in finite-difference approximation \eqref{eq:nl:fdjac} each node corresponds to a column of the Jacobian, we have also colored the columns of $J$ (same Figure).
...
\node at (-2.4,6.5) {colored columns of $J$:};
  ...
  \node at (6.5,0.5) {$1$};
\end{tikzpicture}
\caption{We can build a graph from the grid, with an edge for every pair of unknowns that appears in an equation.  Coloring this graph also assigns colors to the columns of the Jacobian.}
\label{fig:fdcolorreaction}
\end{figure}

FIXME: generate $\bv_k$ by having a $1$ in every entry where $k$ is the color of that column of $J$: 
\begin{equation}
...
FIXME: we have a function that maps from node index $j$ to color $k$:
\begin{equation}
k = k(j)  \label{eq:nl:colorfunc}
\end{equation}
replace \eqref{eq:nl:fdjac} with
\begin{equation}
...
By the way, to actually \emph{count} the number of function evaluations, there is no need to alter \texttt{reaction.c}.\sidenote{No reason to add print statements!}  Use \texttt{-log\_summary} and look at the output.  Because we want the ``\texttt{SNESFunctionEval}'' line, these lines extract the count for the runs above:
\begin{cline}
...
\end{cline}
The exact number of $\bF$ evaluations was 24586 and 13 in the two runs, respectively.
