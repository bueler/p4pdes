...
At this point we risk overwhelming the reader with options, so we pause to review the possibilities before returning to testing them on the \texttt{reaction.c} problem.  Table \ref{tab:snesjacobianoptions} summarizes options relating to residual and Jacobian call-backs in \pSNES-using codes.
...
\end{tabular}
\caption{Jacobian options when using \pSNES, according to which functions are implemented.  Symbol ``$P$'' denotes a routine which computes an easy-to-invert approximate Jacobian while ``$J$'' denotes an implementation of the actual Jacobian function $J_{\bF}(\bx)$.  A big check mark shows recommended usage.} \label{tab:snesjacobianoptions}
\end{table}
