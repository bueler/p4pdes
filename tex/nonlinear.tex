...
\end{cline}
%$
The problem is that finite-difference Jacobian evaluation \eqref{eq:nl:examplefdjac} is requiring about 8000 evaluations of $\bF$ per Newton iteration, and this exceeds the default maximum count for the \pSNES.\sidenote{Again, ``\texttt{./reaction -help |grep snes\_}'' gets the default value of 10000.}  If we raise the limit by a factor of ten then we do get convergence, but at a huge performance cost relative to the use of an analytical Jacobian:
\begin{cline}
