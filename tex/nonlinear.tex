...
\end{cline}
%$
This uses a new option \texttt{-snes\_fd}, the purpose of which the reader may already see.  Clearly the iteration \eqref{eq:nl:newtoneq} requires the Jacobian!  Here we have only supplied the \pSNES object with an implementation of function $\bF(\bx)$, but not with $J_{\bF}(\bx)$.  The entries of the latter matrix are derivatives, however, and so we can approximate by finite differences.  For example, the upper-left entry in $J_{\bF}(\bx)$ is approximated
\begin{equation}
\frac{\partial F_0}{\partial x_0} = \frac{F_0(x_0+\delta,x_1) - F_0(x_0,x_1)}{\delta}
\end{equation}
if $\delta>0$.  It turns out that choosing $\delta \approx \sqrt{\eps}$ gives a reasonably accurate approximation to the derivative if the inputs to $\bF$ are all of order approximately one \cite{Kelley2003}.

Thus with option \texttt{-snes\_fd}, this is what is then done by \pSNES internally, at least in outline, to implement the Newton method \eqref{eq:nl:newtoneq} and \eqref{eq:nl:newtonupdate}:
\begin{itemize}
\item FIXME
\end{itemize}
