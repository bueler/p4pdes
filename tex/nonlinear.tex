...
To get started on nonlinear PDE problems we look at simple two-point boundary value problems.  Equations of the form
\begin{equation}
-u'' + R(u) = f(x)  \label{eq:nl:diffusionreaction}
\end{equation}
are called \emph{diffusion-reaction} equations because the second derivative term models the diffusion of some quantity with density $u$, while the term $R(u)$ can model the production or destruction of $u$.  It is also fair to say that equation \eqref{eq:nl:diffusionreaction} is a nonlinear elliptic PDE in one-dimension.  

FIXME: discuss well-posedness of \eqref{eq:nl:diffusionreaction} with Dirichlet boundary conditions $u(0)=u_L$ and $u(1)=u_R$
% analysis in 1D, thinking about well-posedness:
% - u' u'' + R(u) u' = 0
% first integral:   (u')^2 - 2 S(u) = C   (where S'(u) = R(u))
% R(u) = rho sqrt(|u|)  ==>   S(u) = (2/3) rho u^(3/2)
% R(u) = - lambda e^u  ==>   S(u) = - lambda e^u

For example,
\begin{equation}
-u'' + \rho \sqrt{u} = 0 \label{eq:nl:drsqrt}
\end{equation}
is a particular case of problem \eqref{eq:nl:diffusionreaction} with $R(u) = \rho \sqrt{u}$ and $f(x)=0$.  To use this in a first numerical example we want to verify our numerical solution using an exact solution, but its particular form makes this easy.

Specifically, because both second-derivative and square-root operations can convert certain 4th degree polynomials into quadratic polynomials, an exact solution can be found by substituting a polynomial.  For verification purposes we can also get the boundary conditions from the exact solution.  Thus we substitute
\begin{equation}
