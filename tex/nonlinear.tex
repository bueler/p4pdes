...
We have yet to exploit two critical possibilities when using a \pSNES, namely the ability to provide a method which computes the Jacobian function $J_{\bF}(\bx)$, and the ability to pass parameters through the call-back mechanism so that they can be used inside the residual- and Jacobian-evaluation functions.  The next code \texttt{ecjacobian.c}, in Figures \ref{code:ecjacobianI} and \ref{code:ecjacobianII}, uses these abilities.  For later reference, this code is a ``model use'' of \pSNES.\sidenote{In cases without a structured grid of the \pDMDA type (Chapter \ref{chap:structured}).}

\medskip
\cinputpart{ecjacobian.c}{\CODELOC}{Solves the same nonlinear system as does \texttt{expcircle.c}, but this version includes an exact Jacobian and has the \pSNES pass a parameter into the call-back functions.}{I}{//START}{//END}{code:ecjacobianI}
...
\cinputpart{ecjacobian.c}{\CODELOC}{The \texttt{main()} method is also similar to that in \texttt{expcircle.c}, but with a bit more code to allocate the \pMat which stores the Jacobian.}{II}{//STARTMAIN}{//ENDMAIN}{code:ecjacobianII}
...
FIXME: \PETSc has options: no option, \texttt{-snes\_fd}, \texttt{-snes\_mf}, and \texttt{-snes\_mf\_operator}

\def\checkmark{\tikz\fill[scale=0.4](0,.35) -- (.25,0) -- (.7,.8) -- (.25,.15) -- cycle;}
\def\bigcheckmark{\tikz\fill[scale=0.6](0,.35) -- (.25,0) -- (.7,.8) -- (.25,.15) -- cycle;}

The \PETSc paradigm for implementing residual and Jacobian call-backs in \pSNES-using codes is summarized in Table \ref{tab:snesjacobianoptions}.  In the Table, symbol ``$P$'' denotes an approximate Jacobian while ``$J$'' denotes an exact one.  The large check mark indicates recommended usage.  As practical \pSNES-using advice, you know you have correctly- and fully-implemented the Jacobian, and you have found an adequate initial iterate, if all four of these modes work and give quadratic convergence, as in the last line of the table.  This is the situation for the last two codes above, \texttt{ecjacobian.c} and \texttt{reaction.c}.
...
$\bF$ and $P$:   & \,?           & $\checkmark$    & $\checkmark$    & $\bigcheckmark$ \\
$\bF$ and $J$:   & $\bigcheckmark$ & $\checkmark$    & $\checkmark$    & $\checkmark$
\end{tabular}
\caption{For these three Jacobian options when using \pSNES, either the run-time should work and provide quadratic convergence on well-behaved examples (check mark), it gives uncertain convergence rate (?), or it gives a run-time error.  The big check marks recommended usage.} \label{tab:snesjacobianoptions}
\end{table}

From the code side we have these cases:
\renewcommand{\labelenumi}{(\roman{enumi})}
\begin{enumerate}
\item If only code for $\bF$ is implemented, provide it through \texttt{SNESSetFunction()}.  (If the problem is based on a structured grid, as in the \texttt{reaction.c} example above, use \texttt{DMDASNESSetFunctionLocal()}.)  Do not create or preallocate a \pMat for the Jacobian, as this is done internally by the \pSNES for \texttt{-snes\_fd} runs, while no assembled Jacobian \pMat exists for \texttt{-snes\_mf} runs.
\item If an exact or approximate Jacobian is implemented in code, then there are two cases:
   \renewcommand{\labelenumii}{\alph{enumii}.}
   \begin{enumerate}
   \item In general one must declare a \pMat variable, say ``\texttt{J}'', and then call \texttt{MatCreate()}, \texttt{MatSetSizes()}, and \texttt{MatSetFromOptions()} on it.  Then call either \texttt{MatSetUp()} or \texttt{MatXAIJPreallocate()} on \texttt{J}, to allocate space for the assembled Jacobian matrix.  Then provide \texttt{J} for both \pMat arguments to \texttt{SNESSetJacobian()}.  One also provides the call-back code ``\texttt{FormJacobian()}'' at the same time.
...
With this change, convergence is slowed for no option, i.e.~when using the approximate Jacobian as though it were exact.  On a coarse grid the number of iterations goes from 4 to 15, and the reported residual norms---here mostly suppressed---suggest that the convergence is no longer quadratic:
\begin{cline}
...
\end{cline}
converge?  Explain why this case, with this way of choosing the boundary conditions and initial iterate, in particular, and in contrast to the next exercise, is so insensitive to the value of the major parameter $\rho$.
...
\item To the author's knowledge an exact solution of \eqref{eq:nl:bratuoned} is unknown for $\lambda>0$.  While writing the code in the last exercise is doable, based on the simplicity of the problem and the availability of a ``nearby'' correct example code,\sidenote{I.e.~we started from \texttt{reaction.c}.} one should not walk over high wires too often without a net.  Fortunately, because we can ``manufacture'' \citep{Wesseling2001} an exact solution to a generalized form on \eqref{eq:nl:bratuoned}, one does not need to take big risks.

Consider the equation
\begin{equation}
    - u'' - \lambda e^u = f(x). \label{eq:nl:bratuonedwithf}
\end{equation}
We can make $u(x) = \sin(\pi x)$ the exact solution of \eqref{eq:nl:bratuonedwithf} by determining $f(x)$ from the equation by differentiation alone.  Add options and additional exact solution code to \texttt{bratu.c} in the previous exercise so as to optionally check the result on this exact solution.  For instance, the \texttt{struct} of type \texttt{AppCtx} might have a user-option-settable boolean variable \texttt{manufactured}.  When it is false then $f(x)=0$ as in the previous example; here no exact solution is available and no numerical error is reported.  If the variable is true then we change to the appropriate $f(x)$ and we can report a numerical error.  Confirm that for sub-critical values of $\lambda$ we have both quadratic convergence of the Newton iteration \emph{and} $O(h^2)$ convergence of the numerical error.
\end{enumerate}