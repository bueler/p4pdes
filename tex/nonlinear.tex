...
Three call-back methods appear in Code \ref{code:reactionI}.  First we compute the initial iterate---a linear function connecting the boundary conditions---and the exact solution in the method \texttt{InitialAndExactLocal()}.  Then we have the residual (\texttt{FormFunctionLocal()}) and Jacobian (\texttt{FormJacobianLocal()}) evaluation functions.  These are ``\texttt{Local}'' methods in the sense that their inputs are C pointers for arrays instead of \pVecs.  Our implementation of \texttt{InitialAndExactLocal()} explains how this works.  As seen in Code \ref{code:reactionII}, before this method is called we use \texttt{DMDAVecGetArray()} on the \pVecs \texttt{u} and \texttt{uexact}.  This gives \texttt{PetscReal*} pointers which are then handed to \texttt{InitialAndExactLocal()}.  The call-back done by \pSNES on \texttt{FormFunctionLocal()} and \texttt{FormJacobianLocal()} works the same way.
...
% mesh
  \draw[xstep=0.25,ystep=0.166667,black,thin,dashed] (0.0,0.0) grid (1.25,1.0);
% stencil
...
Note that for merit function \eqref{eq:nl:linesearchmeritfunction}, and by using \eqref{eq:nl:newton:eq}, we have
\begin{align}
