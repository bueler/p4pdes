...
\end{equation}
if $\delta\ne 0$ and $\be_j\in \RR^N$ denotes a vector with entry one in the $j$th position (and zero otherwise).  It turns out that choosing $\delta = \sqrt{\eps}$, where $\eps$ is machine precision, gives a reasonably accurate approximation if the inputs to $\bF$ are all of order approximately one \citep{Kelley2003}.
...
\begin{quote}
	...
	\item the Jacobian $J_{\bF}(\bx_k)$ is
	    ...
	    \item computed and assembled by using formula \eqref{eq:nl:fdjac} $N^2$ times, which requires calling \texttt{FormFunction()} $N$ times to evaluate $\bF(\bx_k+\delta \be_j)$ for $j=0,\dots,N-1$, \emph{or}
...
Option (\emph{ii})b is what happens if we run with option \texttt{-snes\_fd}.  One possible disadvantage of this option is that formula \eqref{eq:nl:fdjac} is only an approximation, but in many cases using an approximate Jacobian in the Newton step is not problematic \citep{Kelley2003}.
...
Besides the finite-difference Jacobian approach we have already seen, which uses equation \eqref{eq:nl:fdjac} to compute the entries of an assembled Jacobian matrix, there is another finite-difference approach which approximates the Jacobian-times-vector product by a finite difference.  This approach, which must be used with a Krylov-type method so as to avoid assembling the matrix, goes by the name ``Jacobian-free Newton-Krylov'' or JFNK \citep{KnollKeyes2004}.

At the simplest level of introduction of this idea we need only recall the Newton equation for the current step $\bs$ based on the current iterate $\bx=\bx_k$, namely \eqref{eq:nl:newtoneq}
\begin{equation}
    J \bs = - \bF(\bx) \label{eq:nl:newtoneqforJFNK}
\end{equation}
where $J = J_\bF(\bx)$, and the Krylov space definition \eqref{eq:li:krylovdefn} given matrix $J$ and some vector $\br$:
\begin{equation}
    \mathcal{K} = \operatorname{span}\{\br,J\br,J^2\br,\dots,J^{n-1}\br\}.  \label{eq:nl:krylovagain}
\end{equation}
In a Krylov space method (e.g.~CG or GMRES) we would might start with initial estimate $\bs\approx 0$ of the solution to linear equation \eqref{eq:nl:newtoneqforJFNK} \citep{KnollKeyes2004} and compute the residual $\br = - \bF(\bx) - J 0 = - \bF(\bx)$.  From this vector we need to compute $J\br$, $J^2\br=J(J\br)$, and so on, to construct a solution to \eqref{eq:nl:newtoneqforJFNK} from the Krylov space $\mathcal{K}$.

By definition of the derivative of $\bF$, i.e.~by the same idea as in \eqref{eq:nl:expandF},
\begin{equation}
J \br \approx \frac{\bF(\bx+\delta \br) - \bF(\bx)}{\delta} \label{eq:nl:JFNKbasic}
\end{equation}
if $\delta \ne 0$ is small.  Noting that $\br = - \bF(\bx)$ comes from evaluating $\bF$, each additional vector in the above Krylov basis of $\mathcal{K}$ can be approximated using one additional evaluation of $\bF$, namely by the formula
\begin{equation}
J^{\ell} \br \approx \frac{\bF(\bx+\delta J^{\ell-1}\br) - \bF(\bx)}{\delta} \label{eq:nl:JFNKiteration}
\end{equation}
for $\ell=1,\dots,n$.  This elegant idea is implemented in \PETSc and is invoked by the option \texttt{-snes\_mf}:\sidenote{``\texttt{mf}'' stands for ``matrix-free''.}
\begin{cline}
...
Our enthusiasm may be quickly damped, however, by trying refined grids.  Observe we are using an \emph{un}preconditioned Krylov method, when using JFNK as stated in \eqref{eq:nl:JFNKiteration}.  For example, because $J$ is symmetric in our case, we might try an appropriate Krylov method on refining grids, namely CG (Chapter \ref{chap:linearsystem}), and ask \PETSc for both the number of Newton and Krylov iterations:
\begin{cline}
...
\end{cline}
These large iteration counts, specifically the well-known doubling of CG iterations with each doubling of dimension, remind us of the flaws in unpreconditioned Krylov methods which were addressed in Chapter \ref{chap:linearsystem}.  Note that the default GMRES solver is no better; we simply get a linear solve failure:
\begin{cline}
...
\end{cline}
%$
In other words, we see an excellent reminder to apply the Krylov solver to a \emph{preconditioned} form of system \eqref{eq:nl:newtoneqforJFNK}.  However, this requires returning to, and modifying, the central idea in equation  \eqref{eq:nl:JFNKbasic}.

FIXME: preconditioned JFNK
