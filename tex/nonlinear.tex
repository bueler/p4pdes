...
Now suppose we (``properly'') color the vertices so that any two vertices which share an edge have distinct colors.  The coloring in the Figure uses $c=3$ colors which is equal to the minimum number needed, which is to say $\chi(G)=c$ where $\chi(G)$ is the \emph{chromatic number} of $G$ \citep{ChartrandLesniakZhang2011}.

Optimally-coloring a graph is a hard problem which \PETSc does not attempt to solve.  The \PETSc ``incidence-degree ordering'' and ``smallest-last ordering'' graph-coloring algorithms are generally near-optimal \citep{ColemanMore1983}, so we get $c$ for which $c \gtrsim \chi(G)$, but they run in a time proportional to the sum of squares of the number of nonzeros in each rows of $J$.  Thus we get a $c$-coloring in not just polynomial time in $N$, the number of unknowns, but in substantially-less time than $O(N^2)$ and often in $O(N)$ time.  (Recall we are trying to improve on the $O(N^2)$ cost of the \texttt{-snes\_fd} method.)
...
We now generate vectors $\bv_0, \bv_1, \dots, \bv_{c-1} \in \RR^N$ by the rule that $\bv_k$ has a $1$ in entry $j$ if $k$ is the color of column $j$ in $J$, and is otherwise zero.  In this $c=3$ case,
\begin{equation}
