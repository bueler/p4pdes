...
\end{equation}
In a Krylov space method (e.g.~CG or GMRES) we would might start with initial iterate $0$, as a solution to linear equation \eqref{eq:nl:newtoneqforJFNK}, and compute the residual $\br = - \bF(\bx)$.  From this vector we need to compute $J\br$, $J^2\br=J(J\br)$, and so on, to solve the linear equation by looking in the Krylov space $\mathcal{K}$.
...
\end{equation}
if $\delta \ne 0$ is small.  That is, noting that $\br = - \bF(\bx)$ itself is computed by an evaluation of $\bF$, each additional vector in the above Krylov basis of $\mathcal{K}$ can be computed by one additional evaluation of $\bF$, namely by the formula
\begin{equation}
J^{\ell} \br \approx \frac{\bF(\bx+\delta J^{\ell-1}\br) - \bF(\bx)}{\delta}
\end{equation}
for $\ell=1,\dots,n$.

This elegant idea is implemented by the simple option \texttt{-snes\_mf} in \PETSc:
\begin{cline}
...
Caution should quickly damp our enthusiasm, however.  Noting we are using an \emph{un}preconditioned Krylov solve when using JFNK as stated.  Because $J$ is symmetric in our case, we try an appropriate Krylov method on refining grids, and we ask \PETSc for both the number of Newton iterations and the Krylov method iterations on each Newton step:
\begin{cline}
...
These large iteration counts remind us of the flaw in unprecondition Krylov methods, addressed in Chapter \ref{chap:linearsystem}.  In the above CG case we see the well-known doubling of iterations with each doubling of dimension, while with the default GMRES solver things are no better as we simply get a linear solve failure:
\begin{cline}
...
\end{cline}
%$
In other words, we have an excellent reminder to apply the Krylov solver to a \emph{preconditioned} form of system \eqref{eq:nl:newtoneqforJFNK}, but this requires returning to the central idea in equation  \eqref{eq:nl:JFNKbasic} and modifying it.
