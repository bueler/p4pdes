...
\end{equation}
It turns out that choosing $\delta = \sqrt{\eps}$, where $\eps$ is machine precision, gives a reasonably accurate approximation if the inputs to $\bF$ are all of order approximately one and the function $\bF$ can be accurately evaluated. \citep{Kelley2003}.
...
\end{cline}
%$
This message is somewhat opaque unless you are conscious of the need to form the Jacobian matrix at each Newton iteration.  That is, \emph{something} must happen at step (\emph{ii}), and actions (\emph{ii})b and (\emph{ii})c require runtime options.
...
\section{The \PETSc Jacobian cases} \label{sec:jacobiancases}
