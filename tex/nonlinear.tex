...
\end{cline}
run without errors, and why?  Same question for \texttt{ecjacobian.c}.
...
\item FIXME: write variation \texttt{bratu.c} of \texttt{reaction.c} to solve
    $$-u'' + \lambda e^u = f(x)$$
with same Dirichlet conditons as for \eqref{eq:nl:diffusionreaction}.  while exact solution of \eqref{eq:nl:diffusionreaction} uses structure of equation, we can ``manufacture'' solution.  do so, say using $u_{\texttt{exact}}(x)=x + sin(3\pi x)$ and getting $u_L,u_R$ as the end-point $x=0,1$ values of $u_{\text{exact}}(x)$.  FIXME implement \texttt{bratu.c} without \texttt{FormJacobianLocal()} first, and check convergence, before adding Jacobian and then checking JFNK options work too  FIXME nontrivial linesearch needed once $\lambda$ is close to critical
\end{enumerate}