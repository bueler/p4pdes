...
Equations \eqref{poissonsquareFDearly} and \eqref{poissonsquareFDbcs} form a linear system of $d=MN$ equations in which we choose to treat all $d$ locations on the grid as unknowns.  To shown this linear system in traditional form
\begin{equation}
A \bu = \bb,
\end{equation}
where $A$ is a $d\times d$ matrix and $\bu,\bb$ are $d\times 1$ column vectors, we must order the unknowns.  This ordering will be used internally by \PETSc in a code that uses a \pDMDA, as in the current example, and we use it here for displaying the system, but the code we actually write (below) sticks to the grid-wise coordinates $(i,j)$.  The ordering is
\begin{align}
&u_{0,0}, \;u_{1,0}, \;\dots, \;u_{M-1,0}, \;\;u_{0,1}, \;u_{1,1}, \;\dots, \;u_{M-1,1} \label{orderingfd} \\
&\qquad \dots \;\;u_{0,N-1}, \;u_{1,N-1}, \;\dots, \;u_{M-1,N-1}. \notag
\end{align}
Now would be a good time to show the linear system itself, and the ordering, in a small case.
...
\end{tikzpicture}
\caption{Ordering of unknowns \eqref{orderingfd} on a $M=4$ and $N=3$ grid.}
\label{fig:unitsquaregridordering}
\end{marginfigure}

\medskip\noindent\hrulefill
\begin{example} In the $M=4$ and $N=3$ case we have the grid and ordering shown in Figure \ref{fig:unitsquaregridordering}.  , equations \eqref{poissonsquareFDbcs} and \eqref{poissonsquareFD} become a system of 12 equations in 12 unknowns of form $A \bu = \bb$:
...
\end{align}
Using \eqref{poissonsquareFD}, as long as the cell aspect ratio $\max\{h_y/h_x,h_x/h_y\}$ is not too large, all the equations in the system, including the boundary conditions, will have coefficients of comparable size.

Second, our equations can be interpreted as a \emph{symmetric} matrix, and that opens up a larger range of linear algebra methods for solving the system efficiently.  For example, in the $i=1$ case of \eqref{poissonsquareFD}, i.e.~at a grid point adjacent to the left-hand boundary of the square, the boundary location value $U_{0,j}$ appears in the equation.  The matrix in the linear system will be symmetric if we systematically ``move'' such values to the right-hand side as known (and zero) instead of inserting a ``$1$'' into the corresponding matrix locations.  That is, we make off-diagonal zero entries in the columns corresponding to known boundary values, in addition to the off-diagonal zeros in rows corresponding to known boundary values.



[FIXME show stencils and comment on coeffs]
%In the $h_x=h_y$ case, regardless of whether the grid is fine or coarse, the equation is
