...
Of course, this is un-preconditioned CG.  While it is not surprising that \texttt{-pc\_type jacobi} does no better (not shown; see exercises), it is easy to check that our favorite method so far has the same poor scaling.  Figure \ref{fig:poisson-cg-scale} shows that though IC($0$) gives somewhat faster solves and lower iteration counts, the \emph{scaling} of those counts is the same as \texttt{-pc\_type none}: the number of iterations doubles with each factor-of-two grid refinement.
...
Some theory of the CG iteration helps to understand these results, but stating a theorem requires preparation.  Suppose that $A$ is an $N\times N$, symmetric, and positive-definite matrix.  We can define a new norm, the \emph{$A$-norm}, on $v\in\RR^N$,
\begin{equation}
\|v\|_A = \ip{v}{Av}^{1/2} = (v^\top Av)^{1/2}.  \label{normAdefn}
\end{equation}
Recall the notation that $\be_k = \bu_k - \bu$ is the error of $\bu_k$ as a solution to $A\bu=\bb$.  Also, denote by $\sigma_j(A)$ the ordered singular values with subscript $0$ indicating the largest value.  Recall that $\sigma_j(A)=\lambda_j(A)$, where $\lambda_j(A)$ are the ordered eigenvalues of $A$, because $A$ is symmetric and positive-definite.  Thus the 2-norm condition of a symmetric positive-definite matrix $A$ is also the ratio of eigenvalues,
	$$\kappa_2(A) = \lambda_0(A)/\lambda_{N-1}(A) \ge 1.$$
(Note $\lambda_{N-1}(A)>0$ because $A$ is positive-definite.)

Now we can state a conceptually-useful theorem giving the sense in which CG is optimal and an error bound.  Part (\emph{ii}) of the theorem follows from (\emph{i}) by using Chebyshev polynomials to construct a polynomial which is small on the interval $[\lambda_{N-1}(A),\lambda_0(A)]$.  Part (\emph{iii}) follows from (\emph{ii}) by understanding the norm $\|\cdot\|_A$ in terms of the 2-norm and the matrix $\sqrt{A}$; see the Exercises.  We state part (\emph{iii}) for the simple reason that $\|\br_k\|_2$ is a quantity computable by \PETSc, given the information available, while $\|\be_k\|_A$ is not.  Parts (\emph{ii}) and (\emph{iii}) are not optimal bounds, though it is easy to state that for part (\emph{ii}) \citep[p.~51]{Greenbaum1997}.

\renewcommand{\labelenumi}{(\roman{enumi})}
\begin{theorem}Suppose $\bu_k$ are the iterates from CG.  Then:\begin{enumerate}
\item \citep[p.~50]{Greenbaum1997}  Considering all real polynomials $p_k(x)$ of degree $k$ such that $p_k(0)=1$,
    $$\|\be_k\|_A = \min_{p_k} \|p_k(A) \be_0\|_A.$$
\item \citep[p.~51]{Greenbaum1997}  The relative $A$-norm of the error at the $k$th iteration is bounded by an amount which is computable from the 2-norm condition number $\kappa=\kappa_2(A)$, namely
...
Consider the number of iterations needed to reduce $\|\be_k\|_A$ by a factor of $\eps<0$.  Note $(x-1)/(x+1) = 1 - 2/(x+1)$, and recall that $\ln(1-x) \approx -x$ for small $x$.  If $\sqrt{\kappa} = \sqrt{\kappa_2(A)}$ is large then part (\emph{ii}) says that $\|\be_k\|_A$ is reduced from its initial value $\|\be_0\|_A$ by a factor of $\eps$ if
    $$k \le \frac{\ln(\eps/2)}{\ln\left(1 - \frac{2}{\sqrt{\kappa}+1}\right)} = -\frac{\ln(\eps/2)}{2} (\sqrt{\kappa}+1).$$
In abbreviated form,
\begin{equation}
\frac{\|\be_k\|_A}{\|\be_0\|_A} \le \eps \quad \text{if} \quad k = O(\kappa_2(A)^{1/2}).  \label{cgkappaiter}
\end{equation}
...
Though the theorem states an important-to-understand connection between $\kappa_2(A)$ and iteration count, the precise condition number is not typically available.  Nonetheless, we can ask \PETSc to approximate eigenvalues of $A$ as the Krylov iteration proceeds.  Note that we need the iteration to use the \emph{un}-preconditioned operator when computing eigenvalues, so we use either no preconditioner or right-side-preconditioning (\texttt{-ksp\_pc\_side RIGHT}).  Doing this largely explains the poor scaling of CG iteration count for our case (i.e.~the symmetric-matrix, structured-grid Poisson problem case).
...
\end{cline}
%$
In fact $17$ eigenvalues\sidenote{There were $17$ \pKSP iterations---confirm this with option \texttt{-ksp\_converged\_reason}---and the method computes one additional eigenvalue per iteration.} are shown in this run, but we suppress all but the first and last.  Only these are needed to approximate $\kappa_2(A)$.
...
When we redo this with option \texttt{-da\_refine N} for \texttt{N}$=1,2$ we get
\begin{align*}
...
\end{equation}
exactly as seen experimentally above.  This can be shown through either exact eigen-analysis \citep{Briggsetal2000,Elmanetal2005}, though such analysis is special to the uniform rectangular case, or through a more robust and general FEM error analysis which applies to unstructured grids having a mesh-uniformity bound \citep{Elmanetal2005}.  Combining \eqref{cgkappaiter} and \eqref{kappaofh} shows that we should expect
\begin{equation}
    k = O(h^{-1}), \label{kofh}
\end{equation}
iterations to solve our discrete Poisson problem with unpreconditioned CG to a given relative tolerance.

The above analysis applies to the un-preconditioned CG iteration.  We have also seen that IC($0$) preconditioning speeds the iteration and reduces the iteration count, but that it does not improve scaling \eqref{kofh}.  Though detailed analysis of the effects of incomplete Cholesky precondition, we certainly have the correct context to understand this comment by \citet[][p.~82]{Elmanetal2005}:
\begin{quote}
...
\end{quote}
So apparently IC($0$) is just not that promising as a preconditioner.


\section{Krylov iteration is not enough \dots \emph{good} preconditioning is needed}

CG iterations and the preconditioning methods above, especially IC($0$) and ILU($0$), are effective in providing substantial improvements on naively-applied direct linear algebra methods.  Besides the bad news of the last section, namely that CG iterations scale badly with refining grids, there is more bad news we could bring up.  Namely, that smartly-applied direct linear algebra techniques can actually beat CG+IC($0$) anyway.

Specifically, suppose we run \texttt{poisson} with option \texttt{-da\_refine 7}, to give a finer $1025 \times 1025$ grid and about $10^6$ degrees of freedom.  Suppose that, in addition to the CG iteration with IC($0$) preconditioning we compare two other methods,
\begin{cline}
...
\end{cline}
%$
To be precise, we also added option \texttt{-ksp\_converged\_reason} to the CG run, so as to count iterations, and \texttt{-ksp\_rtol 1.0e-4} so as to be ``fair'' to the direct solvers.

The first of the above runs requires no explanation; it is the best-so-far way of using CG.  The second and third runs are clearly direct methods, but option \texttt{-pc\_factor\_mat\_ordering\_type nd} asks that the direct method be applied using the \emph{nested dissection} ordering of the unknowns \citep{George1973}.  While we will give no details for such an algorithm, one can say that, in outline, the unknowns and equations are re-ordered to reduce the cost of the factorization.  We get the first three rows of Table \ref{tab:cgdirectmg}.
...
poisson & cg         & icc           & 127.63 & 1262 \\
        & preonly    & cholesky + nd &  80.00 &    1 \\
        &            & lu + nd       &  56.45 &    1 \\
fish2   & cg         & mg            &  10.43 &    7 \\
\end{tabular}
}
\caption{Time and iteration count for \texttt{-da\_refine 7} runs, a $1025 \times 1025$ grid.  The \texttt{fish2} code appears in Chapter \ref{chap:multigrid}.} \label{tab:cgdirectmg}
\end{table}

Thus nested-dissection application of direct solvers is competitive with, indeed better than, IC($0$)-preconditioned CG.  Regarding the option for \pPC factorization reordering and \PETSc defaults, for LU factorization the nested dissection ordering is actually the default for single-processor runs.  % FIXME: check on parallel case
For Cholesky preconditioning, which does not require pivoting for stability on the symmetric positive-definite matrix problems to which it applies, the nested-dissection reordering is not the default.  % FIXME: what to say here?

At this point the impulsive reader might give up on Krylov methods, but that also would be premature.  However, it seems we need to find a more-effective preconditioner if we are going to have scalable solutions of this Poisson PDE problem.  Furthermore, quite a bit of exploration will eventually show that properly-preconditioned Krylov iterations are indeed a robust tool for \emph{parallel-scalable} solution of large nonlinear and variable-coefficient elliptic PDE problems in 2D and 3D; in Chapter \ref{chap:nonlinear} we will start to fulfill this promise.

Thus, so that the reader does not despair, note that we are headed toward geometric multigrid solution of PDEs (Chapter \ref{chap:multigrid}).  These multigrid methods can act as preconditioners on Krylov iterations, a combination which will provide an excellent solution method both for the structured, rectangular-grid Poisson problem of the current Chapter, and later for a broader range of elliptic PDE problems on unstructured grids (Chapter \ref{chap:dmplex}).  Thus we add one more run to the results in Table \ref{tab:cgdirectmg}, namely\sidenote{Again we add options \texttt{-ksp\_converged\_reason -ksp\_rtol 1.0e-4}, for iterations and fairness.}
\begin{cline}
./fish2 -da_refine 7 -ksp_type cg -pc_type mg
\end{cline}
where \texttt{ch5/fish2.c} is a program we describe in Chapter \ref{chap:multigrid}.  The result shows further improvement over the nested-dissection direct methods.  However, it turns out that if we are to exploit the power of geometric multigrid then we need to do a better job using \PETSc's tools for assembling the finite linear system.  Specifically, we need to allow the \pDM object to ``tell us'' the grid on which we are assembling the linear system, something which is done in \texttt{ch5/fish2.c}.
...
% ANSWER:   FD is:  - U_{i-1} + 2 U_i - U_{i+1} = h^2 f_i
