...
\section{A first look at efficiency: CG, for better or worse}

FIXME: the next several timing results had better go in a table

On a modestly-fine grid, the \PETSc defaults for \pKSP and \pPC give a certain timing and a certain number of iterations:
\begin{cline}
$ timer ./poisson -da_refine 5 -ksp_converged_reason -ksp_type gmres -pc_type ilu
Linear solve converged due to CONVERGED_RTOL iterations 506
on 257 x 257 grid:  error |u-uexact|_inf = 1.73086e-06
real 3.77
\end{cline}
%$
That is a lot of iterations, and it is not clear if about 4 seconds is fast or slow, but let us suppose this is some kind of base performance.  Without thinking too hard, experimentation should show us what goes faster or slower.

For example, having a preconditioner is important and desireable:
\begin{cline}
...
\end{cline}
%$
Noting that no improvement is theoretically-expected, as CG applies to our positive-definite problem while MINRES only requires symmetry, we will not pursue it further for now.\sidenote{In fact, \cite[][p.~88]{Elmanetal2005} says ``when solving discrete Poisson problems the convergence of MINRES is almost identical to that of CG.''}
...
Unfortunately, CG remain seriously flawed, at least from the 21st century point of view.\sidenote{The above timing table would be quite satisfying in 1975.}  The flaw is that the iteration count grows with refined grids.  In fact, \citet[][p.~76]{Elmanetal2005} summarises the situation this way:
\begin{quote}
