...
\cinputraw{dmdacreate2d.frag}{extract from c/ch3/poisson.c}{An example of creating a 2D \pDMDA.  See Figures \ref{code:ctwopoissoncreate} and \ref{code:ctwopoissonsolve} for the rest of this code.}{}{//START}{//STOP}{code:dmdacreatetwod}
...
\end{cline}
We see the default $10\times 10$ grid chosen in our call to \texttt{DMDACreate2d()}. There were seven iterations of the KSP method and a final residual norm and numerical error.  We have solved a first PDE problem.
...
We can set the grid at runtime by options \texttt{-da\_grid\_x M -da\_grid\_y N}.  Furthermore, recalling the $M=5$, $N=7$ grid in Figure \ref{fig:unitsquaregrid}, we can examine both the grid (i.e.~the \pDM object) and the generated matrix graphically at runtime.  That is, if X11 or other windowing is correctly linked in your \PETSc installation then
\begin{cline}
$ ./poisson -da_grid_x 5 -da_grid_y 7 -a_mat_view draw -draw_pause 5
\end{cline}
%$
shows a graphic similar to Figure \ref{fig:matM5N7}.  Options
\begin{cline}
$ ./poisson -da_grid_x 5 -da_grid_y 7 -dm_view draw -draw_pause 5
\end{cline}
%$
shows Figure \ref{fig:dmM5N7}.  Finally, a ``movie'' of the \pKSP iterates $\{\bu_k\}$ comes from running
\begin{cline}
...
In fact there are two ways to specify a finer grid, both of which address the \pDM object through options.  One is to specify the grid directly as above, but the other is to have the \pDM refine the grid by factors of two.  More precisely, the number of \emph{subintervals} is increased by a factor of two.  For example, this replaces our default grid of 9 by 9 subintervals (i.e.~10 by 10 grid \emph{points}) by 18 subintervals in each direction (i.e.~19 grid points):
\begin{cline}
$ ./poisson -da_refine 1
...
\end{itemize}
In \PETSc the choices of solver and parameters are controllable and exposable at runtime, once we have adequately-described the problem to \PETSc.  Thus we can, and we have until now, delayed questions of convergence, scaling, and exposure until runtime.  We can make our choice empirical, i.e.~based on actual performance.

Runtime control is a topic that we will return to frequently.  But we start right now with measuring convergence of \texttt{poisson.c}.
...
Is the reduced apparent rate of convergence a sign of an implementation error?  In fact not, because simply asking for the \pKSP object to solve the linear system better is effective.  On a particular grid we can see the difference using option \texttt{-ksp\_monitor} to watch the residuals and setting a stronger value for the \pKSP relative tolerance for residual norm size (\texttt{-ksp\_rtol}):
\begin{cline}
$ ./poisson -ksp_monitor
...
Regarding i), even knowing what are the possible runtime options is nontrivial in \PETSc because there are so many.  But the first step for finding allowed options is simple: pipe the output from option \texttt{-help} into a pager like \texttt{more} or \texttt{less}, like this
\begin{cline}
$ ./poisson -help | less
\end{cline}
%$
This gives a view of the options available for those parts of \PETSc which are actually used in \texttt{poisson}.  For instance, the many options for controlling nonlinear solvers are \emph{not} included in \texttt{-help} output because we have not used \pSNES; compare Chapter \ref{chap:nonlinear}.  This \texttt{-help} output also gives default values for parameters; for example we see that the default for \texttt{-ksp\_rtol} is \texttt{1e-05}, which explains why convergence ``leveled out'' on fine grids.

Alternatively, one might want to know what are options for controlling particular objects inside \texttt{poisson}.  For example, this command shows runtime options which control the \pKSP object:
\begin{cline}
...
Regarding ii), recall that we used \texttt{-dm\_view} to show properties of the \pDM object in \texttt{poisson}.  We should do the same for the \pKSP object:
\begin{cline}
$ ./poisson -ksp_view
...
FIXME: see \verb|-log_summary| output
...
\item Because the matrix set up by \texttt{poisson.c} is symmetric positive definite, the CG iteration applies.  Compare performance of Jacobi-preconditioned and un-preconditioned CG, by doing
\begin{cline}
$ timer ./poisson -da_refine 5 -ksp_converged_reason -ksp_type cg -pc_type X
\end{cline}
%$
for \texttt{X}=\texttt{preonly},\texttt{jacobi} in turn.  (Add \texttt{-ksp\_view} to these runs to see the solver configuration.  Why is Jacobi preconditioning irrelevant here, i.e.~no better than \texttt{-pc\_type none}?  Make a fair comparison to the direct method represented by \texttt{-ksp\_type preonly -pc\_type cholesky} by tightening the tolerance for CG to \texttt{-ksp\_rtol 1.0e-12}.  (The effectiveness of iteration should be clear.)  Finally, add \texttt{-log\_summary} to the un-preconditioned CG run, e.g.
\begin{cline}
$ ./poisson -ksp_converged_reason -ksp_type cg -pc_type none -log_summary
\end{cline}
%$
By looking at the ``Count'' column of ``Event Stage 2: Solve'', confirm that the computational work of one CG iteration consists of two inner products (\texttt{VecTDot}), three vector updates (\texttt{VecAXPY} and \texttt{VecAyPX}), and one matrix-vector product (\texttt{MatMult}).
\end{enumerate}
