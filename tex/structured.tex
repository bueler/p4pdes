...
Unfortunately, even with the preconditioners tried above, CG has a distinct flaw.\sidenote{At least from a 21st-century point of view.  The above timing table might have satisfied in 1970.}  The iteration count grows with refined grids, so that
\begin{quote}
...
Of course, this is un-preconditioned CG.  While it is not surprising that \texttt{-pc\_type jacobi} does no better (not shown but see exercises), our favorite method so far has the same poor scaling.  Figure \ref{fig:poisson-cg-scale} shows that, while the \texttt{icc} preconditioner reduces the iteration count, the rate at which iterations grow is no better than \texttt{none}.  Though IC($0$) gives somewhat faster solves and lower iteration counts, the \emph{scaling} of those iteration counts is the same: doubling with each factor-of-two grid refinement.
