...
But to shown this linear system in traditional form we must globally-order the unknowns.  Such an ordering is implemented inside a \PETSc \pDMDA, but a feature of \PETSc is that it is hidden and can often be ignored.  We are explicit about the ordering here for the purpose of displaying the system in matrix-vector form, but the code we actually write only uses the grid-wise coordinates $(i,j)$.  The ability to assemble \pMats and \pVecs with $(i,j)$-type indexing is one reason \pDMDA codes can be quite short.
...
\section{A first example code for the Poisson problem}

At this point we need to set up a particular Poisson problem so our first example code can solve it.  To do this we \emph{choose} an exact solution, taking care that it satisfies our homogeneous Dirichlet boundary conditions ($u=0$ along $\partial \mathcal{S}$):\sidenote{Noting that the truncation error term $O(h^2)$ in equation \eqref{secondderivativeFD} has a coefficient proportional to fourth derivatives, our FD method will \emph{not} be exact on this problem.  This is good; we would not want the simpler form $u(x,y)=(x-x^2)(y-y^2)$, for example, to test convergence rate of the code.}
\begin{equation}
...
\end{equation}
(This same problem appears in Chapter 4 of \citep{Briggsetal2000}, so we are not being original.) % page 64
...
The code \texttt{c2poisson.c} uses our finite difference method \eqref{poissonsquareFD} to solve the Poisson problem with $f$ from \eqref{manufacturedf}.  The first part, in Figure \ref{code:ctwopoissonrhs}, isolates the computation of $u$ from \eqref{exactsolution} and $f$ from  \eqref{manufacturedf} into a method called \texttt{formRHSandExact()}.  It uses only local grid coordinates $(i,j)$ for the computation, with loops over index ranges as shown in Figure \ref{fig:localpartofgrid}. \PETSc pointer arithmetic (i.e.~tricks) allows us to index the C arrays we get from \texttt{DMDAVecGetArray()} using $(i,j)$.  When we are done with computing \pVecs we restore the C arrays by calling \texttt{DMDAVecRestoreArray()}, and we explicitly ask for the \pVec objects to be assembled by calling \texttt{VecAssemblyBegin/End()}.

Next, Figure \ref{code:ctwopoissoncreate} shows how \texttt{c2poisson.c} creates the various parallel objects needed to solve the Poisson problem, namely one \pDM, one \pMat, and three \pVecs.  Note that a \pDM object can compute appropriate matrix and vector sizes from the grid dimensions, so we call \texttt{DMCreateMatrix()} and \texttt{DMCreateGlobalVector()} to create \pMat and \pVec objects, respectively.  Then we call \texttt{formdirichletlaplacian()} (see Figure \ref{code:structuredlaplacian}) and \texttt{formRHSandExact()} (see Figure \ref{code:ctwopoissonrhs}).
...
\PETSc describes the linear system ``$A\bu=\bb$'' by one \pMat \texttt{A} and two \pVecs (\texttt{u} and \texttt{b}), but the \emph{method} for solving the linear system is a ``Krylov space'' solver object of type \pKSP.  The part of \texttt{c2poisson.c} that creates this object is in Figure \ref{code:ctwopoissonsolve}.  Krylov space methods \citep{TrefethenBau} build a numerical solution $\tilde\bu$ by applying $A$ to vectors, but without using other manipulations of the entries of $A$.  For instance, the approximate solution $\tilde\bu$ might be an element of the $n$th Krylov space
    $$\mathcal{K} = \operatorname{span}\{\bb,A\bb,A^2\bb,\dots,A^{n-1}\bb\},$$
in which case
    $$\tilde\bu = c_0 \bb + c_1 A \bb + c_2 A^2 \bb + \dots + c_{n-1} A^{n-1} \bb$$
or equivalently
    $$\tilde\bu = p(A) \bb$$
for the $n-1$ degree polynomial
    $$p(x) = c_0 + c_1 x + c_2 x^2 + \dots + c_{n-1} x^{n-1}$$
applied to $A$.  Well-known methods CG, GMRES, MINRES, and others \citep{Greenbaum1997,Saad2003} solve a linear system this way.  Almost every Krylov method is implemented in \PETSc, and we get access to all of them via a \pKSP object.  We will return to using Krylov space methods in \PETSc in later chapters.

\cinputpart{c2poisson.c}{Solve using \pKSP, and report on solution.}{III}{//SOLVE}{//ENDSOLVE}{code:ctwopoissonsolve}

To finish showing the code, in Figure \ref{code:ctwopoissonsolve} we create the \pKSP object and tell it about \texttt{A} through a call to \texttt{KSPSetOperators()}.  Inside a \pKSP there are two ways \texttt{A} is used, namely as the system matrix and as a possible preconditioner---more on this later---which explains why \texttt{A} appears twice when calling \texttt{KSPSetOperators()}.  We also call \texttt{KSPSetFromOptions()} so that we can change the \pKSP type at runtime (illustrated below).  Solving the system means calling \texttt{KSPSolve()}.  Then we report on the solution by computing the numerical error by the norm $\|u-u_{exact}\|_\infty$.  Then we wrap up by destroying objects and calling \texttt{PetscFinalize()}.
...
\item how to improve the quality of our numerical solution? (\emph{convergence})
\item how to get high performance? (\emph{scaling})
\item how to see inside \PETSc so we know what is going on? (\emph{exposure})
\end{itemize}

The \PETSc paradigm is that the choices of solver and parameters are controllable, and exposable, at runtime, once we have adequately described the problem to \PETSc.  Thereby we can both put-off questions of convergence, scaling, and exposure until runtime, and we can address these questions empirically based on actual performance.


\section{Convergence}

By ``convergence'' we mean that we want to show that the errors made by our numerical solver decrease in the expected way, namely by refining the grid so that the finite differences are better approximations of the corresponding derivatives.  Furthermore we want to know that the rate at which the error decreases is as expected in theory.

If we want a smaller error specify a finer grid.  There are two ways to do this, both of which address the \pDM object by options.  One is to specify the grid directly:
\begin{Verbatim}[fontsize=\small]
$ ./c2poisson -da_grid_x 100 -da_grid_y 100
on 100 x 100 grid:  error |u-uexact|_inf = 5.76054e-06
\end{Verbatim}
The other is to instruct the \pDM to refine the grid by factors of two.  Here the number of subintervals is increased by a factor of two, so this call replaces our default grid of 9 by 9 subintervals in the two directions by 18 in each direction:
\begin{Verbatim}[fontsize=\small]
$ ./c2poisson -da_refine 1
on 19 x 19 grid:  error |u-uexact|_inf = 0.000155374
\end{Verbatim}

Thus we can check convergence by generating error data.  Here is a loop using the Bash shell:
\begin{Verbatim}[fontsize=\small]
...
\end{Verbatim}
The data is shown in Figure \ref{fig:c2poisson-conv} as the circles.
...
FIXME: complaints are that $K=5$ and $K=6$ cases are slow (about 2 minutes for latter; more later), convergence is below $h^2$ so maybe we have implementation error?
...
FIXME: asking for smaller residual
\begin{Verbatim}[fontsize=\small]
$ ./c2poisson -da_refine 4 -ksp_rtol 1.0e-12
\end{Verbatim}
