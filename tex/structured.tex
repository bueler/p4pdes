...
The code \texttt{c2poisson.c} uses our finite difference method \eqref{poissonsquareFD} to solve the Poisson problem with $f$ from \eqref{manufacturedf}.  The first part, in Figure \ref{code:ctwopoissonrhs}, isolates the computation of $u_{ex}$ from \eqref{exactsolution} and $f$ from  \eqref{manufacturedf} into a method called \texttt{formRHSandExact()}.  It uses only local grid coordinates $(i,j)$ for the computation, with loops over index ranges as shown in Figure \ref{fig:localpartofgrid}. \PETSc pointer arithmetic (i.e.~tricks) allows us to index the C arrays we get from \texttt{DMDAVecGetArray()} using $(i,j)$.  When we are done with computing \pVecs we restore the C arrays by calling \texttt{DMDAVecRestoreArray()}, and we explicitly ask for the \pVec objects to be assembled by calling \texttt{VecAssemblyBegin/End()}.
...
$ ./c2poisson -ksp_monitor
  ...
  3 KSP Residual norm 2.915161628052e-03 
  4 KSP Residual norm 5.145699777443e-04 
  5 KSP Residual norm 6.960595794927e-05 
...
$ ./c2poisson -ksp_monitor -ksp_rtol 1.0e-12
  ...
  6 KSP Residual norm 6.995040432502e-06 
  7 KSP Residual norm 8.593881990968e-07 
  8 KSP Residual norm 8.624906750384e-08 
  9 KSP Residual norm 7.509270093428e-09 
 10 KSP Residual norm 5.508827844317e-10 
 11 KSP Residual norm 6.349819528978e-11 
 12 KSP Residual norm 4.328647633960e-12 
...
\end{Verbatim}
On this coarse grid the numerical error is nearly the same, but on finer grids the fact that the linear system is solved more exactly will bring the numerical solution closer to the exact one.

In fact, rerunning the Bash loop with the stronger tolerance yields this data,
\begin{Verbatim}[fontsize=\small]
...
\end{Verbatim}
which is plotted as the circles in Figure \ref{fig:c2poisson-conv}.  The linear-fit rate of this logarithmic error data is $O(h^{1.9999})$, suggesting strongly that our implementation is correct.
...
FIXME: complaint: $K=5$ and $K=6$ cases are slow (about 2 minutes for latter; more later)
