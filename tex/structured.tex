...
\item The relative 2-norm of the residual is likewise bounded with an additional factor:
	$$\frac{\|\br_k\|_2}{\|\br_0\|_2} \le 2 \sqrt{\kappa} \left(\frac{\sqrt{\kappa}-1}{\sqrt{\kappa}+1}\right)^k.$$
\end{enumerate}
\end{theorem}

Consider the number of iterations needed to reduce the $A$-norm of the error by a factor of $\eps<0$.  Noting $(x-1)/(x+1) = 1 - 2/(x+1)$, and using a standard approximation $\ln(1-x) \approx -x$ for small $x$, when $\sqrt{\kappa}$ is large then part (\emph{ii}) says that $\|\be_k\|_A$ is reduced from its initial value $\|\be_0\|_A$ by a factor of $\eps$ if
    $$k \le \frac{\ln(\eps/2)}{\ln\left(1 - \frac{2}{\sqrt{\kappa}+1}\right)} = -\frac{\ln(\eps/2)}{2} (\sqrt{\kappa}+1).$$
In abbreviated form,
\begin{equation}
\frac{\|\be_k\|_A}{\|\be_0\|_A} \le \eps \quad \text{if} \quad k = O(\sqrt{\kappa}).  \label{cgkappaiter}
\end{equation}
The same conclusion applies when we consider a reduction in $\|\br_k\|_2$ by a factor of $\eps$, though the analysis is not so simple; see the Exercises.

Though the theorem states an important-to-understand connection between $\kappa$ and iteration count, the reader should keep in mind that a precise value of $\kappa$ is not typically available.  Nonetheless, on small examples we can ask \PETSc to approximate eigenvalues of $A$ as the Krylov iteration proceeds.  This provides a reasonably-clear explanation of the poor scaling of CG iteration count in this symmetric-matrix case.

Concretely, noting that we want the eigenvalues of the \emph{un}-preconditioned operator, so we use either no preconditioner or right-side-preconditioning (\texttt{-ksp\_pc\_side RIGHT}),
\begin{cline}
...
\end{cline}
%$
In fact $17$ eigenvalues\sidenote{There were $17$ \pKSP iterations---confirm this with option \texttt{-ksp\_converged\_reason}---and the method computes one additional eigenvalue per iteration.} are shown in this run, but we suppress all but the first and last because we only these are needed to compute an approximate $\kappa$.

The approximate condition number on the above grid, with spacing $h=1/8$, is
    $$\kappa\big|_{h=1/8} \approx \frac{7.69543}{0.304482} = 25.274.$$
Because we observe that the CG iteration count increases with refinement, we redo this with option \texttt{-da\_refine 1}:
    $$\kappa\big|_{h=1/16} \approx \frac{7.92311}{0.0768589} = 103.09.$$

In the fact that $103 \approx 4 \times 25$, and for similar results from \texttt{-da\_refine 2} and so on, we now see the issue clearly.  The condition number $\kappa$ is increasing by a factor of four for each grid refinement by a factor of two (i.e.~$h\to h/2$).  Then \eqref{cgkappaiter} says we should expect an increase in iteration count by a factor of two.  In fact the $2$-norm condition number of the discrete Poisson operator (matrix) $A_h$ on a uniform rectangular grid with spacing $h$ is known to be
    $$\kappa_2(A_h) = O(h^{-2})$$
as an \emph{a priori} statement.  This can be shown through either exact eigen-analysis \citep{Briggsetal2000,Elmanetal2005}, though such analysis is special to the uniform rectangular case, or through a more robust and general FEM error analysis which applies to unstructured grids with a mesh-uniformity bound \citep{Elmanetal2005}.

FIXME: preconditioning?  above explains \citet[][p.~82]{Elmanetal2005} comment
\begin{quote}
...
FIXME: but even more bad news: lu beats cg+icc anyway.  this is because lu is applied with nested-dissection () by default, so lu+nd and cholesky+nd are quite good; 

FIXME  We need to find a truly-effective preconditioner.  before dispairing on iteration, however, we are headed toward multigrid

FIXME  Ran \texttt{poisson} with options \texttt{-ksp\_converged\_reason -da\_refine 7} gives $1025 \times 1025$ grid and thus about $10^6$ degrees of freedom.

\begin{cline}
./poisson ... -ksp_type cg -pc_type icc -ksp_rtol 1.0e-14
./poisson ... -ksp_type preonly -pc_type cholesky -pc_factor_mat_ordering_type nd
./poisson ... -ksp_type preonly -pc_type lu
./fish2   ... -ksp_type cg -pc_type mg  -ksp_rtol 1.0e-14
\end{cline}
...
poisson & cg         & icc      & 127.63 & 1262 \\ % -ksp_rtol 1.0e-14
        & preonly    & cholesky &  80.00 &    1 \\ % -pc_factor_mat_ordering_type nd
        &            & lu       &  56.45 &    1 \\
fish2   & cg         & mg       &  10.43 &    7 \\ % -ksp_rtol 1.0e-14
\end{tabular}
}
\caption{FIXME} \label{tab:cgdirectmg}
\end{table}
