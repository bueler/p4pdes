...
We start with the Poisson problem on a square because it is a straightforward PDE problem.  In solving it we will learn how to use key parts of \PETSc: we will build a structured grid using a \PETSc \pDMDA, then assemble a \pMat in parallel based on this grid, and then solve in parallel using a \pKSP object.  Thus much of the code we write will look like that for the linear systems we solved in Chapter \ref{chap:getstarted}.
...
Consider the lines of code in Figure \ref{code:dmdacreatetwod}, an extract from a code later in this Chapter.  These lines create a \PETSc \pDM object for a grid like Figure \ref{fig:unitsquaregrid}.  A \pDM is an abstract type for describing the topology (connectedness) of a grid, \emph{and} the way it is distributed across \MPI processes, \emph{and} the way each process can access data from its neighbors.  The specific variable \texttt{da} in Figure \ref{code:dmdacreatetwod} is a ``\pDMDA'', which is the subclass of \pDM s which are structured grids.  (``\pDM'' might stand for ``distributed mesh,'' and ``DA'' for ``distributed array''.)
...
\end{cline}
then a structured grid will be created which is exactly as shown in Figure \ref{fig:unitsquaregrid}.  In this case all points of the grid are ``owned'' by the single process created when we ran \texttt{c2poisson} (i.e.~without \MPI).
...
If we run with multiple \MPI processes by
\begin{cline}
...
The fifth and sixth arguments of ``\texttt{-10}'' to \texttt{DMDACreate2d()} in Figure \ref{code:dmdacreatetwod} are used to set default dimensions $M=10$ and $N=10$, but we have seen that these defaults can be overridden by runtime options \texttt{-da\_grid\_x} and \texttt{-da\_grid\_y}.  The command
\begin{cline}
...
\cinputpart{structuredpoisson.c}{Fill matrix entries using \texttt{MatSetValuesStencil}.}{I}{//CREATEMATRIX}{//ENDCREATEMATRIX}{code:structuredpoisson}
