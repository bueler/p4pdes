...
\medskip
We can now show the code that assembles ``$A$'' in linear system \eqref{poissonlinearsystem}; see Figure \ref{code:structuredpoisson}.  For clarity and code reuse we have isolated this method \texttt{formdirichletlaplacian()} in a separate file.  Recall that from now on in this book we automatically strip ``\texttt{ierr =}'' and ``\texttt{CHKERRQ(ierr);}'' error-checking clutter from the displayed code, but it is still present in the source.
...
Still considering the code in Figure \ref{code:structuredpoisson}, the \pMat object $A$ assembled by \texttt{formdirichletlaplacian()}  has ranges of rows owned by each process, the standard parallel layout \texttt{MATMPIAIJ} of \pMat objects in \PETSc (Chapter \ref{chap:getstarted}).  This fact is another one we can often forget on a \pDMDA-managed structured grid, because we work with the locally-owned subgrid using $(i,j)$ indices.  We get to think of the ``part of the problem owned by the process'' as just the part of the grid owned by the process, as in Figures \ref{fig:unitsquaregridparallel}--\ref{fig:unitsquaregrideight}.

In particular, local indices $(i,j)$ can be used when inserting entries into a dynamical data structure for matrix assembly, namely the \pMat \texttt{A}.  Thus in Figure \ref{code:structuredpoisson} we see one use of \texttt{MatSetValuesStencil()} for each locally-owned grid point.  For a generic interior point this command inserts five coefficients into the matrix.  The key data structure is of type \texttt{MatStencil}, an apparently-trivial struct
\begin{code}
typedef struct {
  PetscInt k,j,i,c;
} MatStencil;
