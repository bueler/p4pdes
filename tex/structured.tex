...
\end{cline}
Recall that a default $10\times 10$ grid was chosen in calling \texttt{DMDACreate2d()}.  We see seven iterations of the KSP method, a small final residual norm, and an apparently small numerical error.  It is reasonable to think that we have solved a first PDE problem, but skepticism, and further inspection, is in order.
...
\end{itemize}
In \PETSc the choices of solver and parameters are controllable and exposable at runtime, once we have adequately-described the problem to \PETSc, and we have just gotten started on ``exposure.''  We have, until now, delayed questions of convergence and efficiency, or just scaling, until runtime.  We can make our choice empirical, i.e.~based on actual performance.  In any case, runtime control is a topic that we will return to frequently.  But we start right now with measuring convergence of \texttt{poisson.c}.
