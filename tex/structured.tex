...
Unfortunately, even with the preconditioners tried above, CG remains flawed.\sidenote{At least from a 21st-century point of view.  The above timing table might have satisfied in 1975.}  The leading flaw is that iteration count grows with refined grids, so that \citet[][p.~76]{Elmanetal2005} says
\begin{quote}
...
\end{cline}
Of course, this is un-preconditioned CG.  While it is not surprising that \texttt{-pc\_type jacobi} does no better (not shown), poor scaling also applies to our favorite method so far:

FIXME: add this and above data to figure
\begin{cline}
$ for NN in 1 2 3 4 5; do
