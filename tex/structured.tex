...
This is the point at which some theory about CG would help to understand results, but stating the theorem needs some preparation.  Given $A$ a real, $N\times N$, symmetric, and positive-definite matrix we can define a new norm on $v\in\RR^N$,
\begin{equation}
\|v\|_A = \ip{v}{Av} = v^\top Av.  \label{normAdefn}
\end{equation}
Recall the notation that $\be_k = \bu_k - \bu$ is the error in solving $A\bu=\bb$.  Note that, CG being a Krylov method, its $k$th error satisfies
    $$\be_k = \be_0 + c_1 A \be_0 + \dots + c_k A^k \be_0$$
for some real coefficients $c_j$.  Also, denote by $\sigma_j(A)$ the ordered singular values with subscript $0$ indicating the largest value.  Recall that $\sigma_j(A)=\lambda_j(A)$, where $\lambda_j(A)$ are the ordered eigenvalues of $A$, because $A$ is symmetric and positive-definite.  Thus the 2-norm condition of a symmetric positive-definite matrix $A$ is also the ratio of eigenvalues,
...
\item The $k$th error is bounded by an amount which is computable from the condition number $\kappa=\kappa_2(A)$, namely
	$$\|\be_k\|_A \le 2 \left(\frac{\sqrt{\kappa}-1}{\sqrt{\kappa}+1}\right)^k \|\be_0\|_A.$$
\end{itemize}
\end{theorem}
...
Ran \texttt{poisson} with options \texttt{-ksp\_converged\_reason -da\_refine 7} gives $1025 \times 1025$ grid and thus about $10^6$ degrees of freedom.
...
\item Show that \eqref{normAdefn} defines a norm on $\RR^N$.  By correctly-defining the matrix $\sqrt{A}$, show that $\|v\|_A = \|\sqrt{A}v\|_2$.
