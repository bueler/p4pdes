...
Unfortunately, even with the preconditioners tried above, CG has a distinct flaw.\sidenote{At least from a 21st-century point of view.  The above timing table might have satisfied in 1970.}  The iteration count grows with refined grids, so that \citep[p.~76]{Elmanetal2005}
\begin{quote}
...
Of course, this is un-preconditioned CG.  While it is not surprising that \texttt{-pc\_type jacobi} does no better (not shown; see exercises), our favorite method so far has the same poor scaling.  Figure \ref{fig:poisson-cg-scale} shows that though IC($0$) gives somewhat faster solves and lower iteration counts, the \emph{scaling} of those counts is the same as \texttt{-pc\_type none}: the number of iterations doubles with each factor-of-two grid refinement.
...
Some theory about CG would help to understand these results, but stating a theorem requires preparation.  Suppose that $A$ is a real, $N\times N$, symmetric, and positive-definite matrix.  We can define a new norm, the \emph{$A$-norm}, on $v\in\RR^N$,
\begin{equation}
...
Now we can state a useful theorem giving the sense in which CG is optimal, plus a practical error bound.  The second part of the theorem follows from the first by using Chebyshev polynomials to construct a polynomial which is small on the interval $[\lambda_{N-1}(A),\lambda_0(A)]$.

\begin{theorem} \citep{Greenbaum1997} Suppose $\bu_k$ are the iterates from CG.  Then:\begin{itemize}
...
FIXME: estimate, including computation using
\begin{cline}
...
\texttt{
\begin{tabular}{lllll}
\underline{Code}\hspace{0.2in} & \underline{KSP}\hspace{0.3in} & \underline{PC}\hspace{0.4in} & \underline{\textrm{time (s)}}\hspace{0.1in} & \underline{iterations} \\
...
\caption{FIXME} \label{tab:directbeatscgexceptmg}
\end{table}
...
\item For a symmetric positive-definite matrix $A$, show that \eqref{normAdefn} defines a norm on $\RR^N$.  By correctly-defining the matrix $\sqrt{A}$, show that $\|v\|_A = \|\sqrt{A}v\|_2$.
