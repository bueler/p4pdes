...
To make our test function space $H_{0D}^1(\Omega)$ a true subspace\sidenote{I.e.~to make our FEM \emph{conforming}.} of $H^1(\Omega)$ we require that $\Omega$ be polygonal, with $\partial\Omega$ a closed polygon.  Furthermore we require that segments of $\partial\Omega$ be either entirely in $\partial_D\Omega$ or entirely in $\partial_N\Omega$.  We also assume $\partial_D\Omega$ is a closed set for reasons which will be clarified below.

\begin{marginfigure}
\input{blob.tikz}
\caption{FIXME a polygonal $\Omega$}
...
\begin{marginfigure}
\input{blob.1.elenum.tikz}
\caption{A triangulation $\Th$ with $K=15$ triangles (elements) numbered $k=0,1,\dots,K-1$.}
\label{fig:un:number-elements}
\end{marginfigure}

\begin{marginfigure}
\input{blob.1.nodenum.tikz}
\caption{The node number for the same triangulation as in Figure \ref{fig:un:number-elements}.  There are $N=13$ nodes numbered $j=0,1,\dots,N-1$.  Nodes $\bx_0$, $\bx_1$, $\bx_2$, $\bx_3$ are in the Dirichlet boundary $\partial_D\Omega$ (bold segments).}
\label{fig:un:number-nodes}
\end{marginfigure}
