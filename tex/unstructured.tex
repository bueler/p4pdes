...
The cliched Poisson problem can be exploited more.  It gives us the opportunity to use \PETSc for some important tasks we have not yet seen, including reading an unstructured mesh into \PETSc \pVec s, preallocation of a \pMat in parallel, and use of a linear solver (a \pKSP object) to solve large sparse systems.
...
\cinputpart{c3convert.c}{Start reading ASCII files with the \textsc{triangle}-generated mesh: Go to rank zero processor, read node header, and allocate \pVec s accordingly.  Then read nodes.}{II}{//ENDFILENAME}{//ENDREADNODES}{code:triangleparttwo}
