...
\item If $u \in H_{gD}^1(\Omega)$ solves the weak form then we accept it, by definition, as a solution of the Poisson problem, and, in informal speaking, one might even say that it solves \eqref{eq:un:poissonstrong}.  If also it is well-behaved (i.e.~$u \in C^2(\Omega) \cap C(\overline \Omega)$) then reversing the above derivation shows that it solves the strong form.\sidenote{There exist more-or-less technical conditions, from our point of view, on the domain and the boundary data so that one can show $u$ solving \eqref{eq:un:poissonweak} is this well-behaved in some situations \citep{Evans2010}.}
\end{itemize}

If $\partial_D \Omega$ has positive size, in an appropriate sense, then a solution to weak formulation \eqref{eq:un:poissonweak} exists and is unique.\sidenote{Proofs of this well-posedness claim are in \citet{Ciarlet2002} and in \citet{Evans2010}.}  Also, reminiscent of Chapter \ref{chap:un}, we could formulate our Poisson problem as a minimization; the standard way of doing this gives a formulation equivalent to the weak form \citep{Evans2010}.  However, our goal in this Chapter is to actually implement a numerical method for \eqref{eq:un:poissonweak}, so that we can examine computational options and performance, and thus follow neither of these mathematical tangents.
...
\inputfromline{../c/ch9/bump.poly}{\CODELOC bump.poly}{A description of the boundary polygon in Figure \ref{fig:triangulation}, suitable for reading by \Triangle.}{16}{code:bumppoly}
