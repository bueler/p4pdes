...
%   where last 3 args are either "dashed" or empty
\def \standardSNESstack#1#2#3#4{
...
\draw[line width=1pt] (3,7) node[userfcn,minimum width=95mm] (usercode) {user code \\ \vspace{10mm}};
\draw[line width=1pt] (-0.5,7.2) node[userfcn,#2] (objcode) {objective};
\draw[line width=1pt] (3,6.5) node[userfcn] (rescode) {residual};
\draw[line width=1pt] (6.7,7.2) node[userfcn,#3] (jaccode) {Jacobian};
...
\draw[line width=1pt] (4,4) node[component,#4] (dmda) {\complabel{\pDMDA}{structured grid}};
...
\path
   ...
   ([xshift=2em]usercode.south) edge[->,#4] node {} (dmda)
...
%   where last 2 args are either "dashed" or empty
\def \standardTSstack#1#2#3{
\begin{tikzpicture}[scale=#1,
                    >={Latex[length=2mm]},
  component/.style={
     rectangle,draw,fill=white,align=center,line width=1pt},
...
\draw[line width=1pt] (3,8) node[userfcn,minimum width=90mm] (usercode) {user code \\ \vspace{10mm}};
\draw[line width=1pt] (1,7.7) node[userfcn] (rescode) {right-hand side (RHS)};
\draw[line width=1pt] (5.5,7.7) node[userfcn,#2] (jaccode) {RHS Jacobian};

\draw[line width=1pt] (-0.5,5.0) node[component] (ts) {\complabel{\pTS}{time-stepping}};

\draw[line width=1pt] (-0.5,3) node[component] (snes) {\complabel{\pSNES}{nonlinear solver}};
\draw[line width=1pt] (-0.5,1) node[component] (ksp)  {\complabel{\pKSP}{linear solver}};
\draw[line width=1pt] (-0.5,-1) node[component] (pc)   {\complabel{\pPC}{preconditioner}};

\draw[line width=1pt] (3.5,3.5) node[component,#3] (dmda) {\complabel{\pDMDA}{structured grid}};

\draw[line width=1pt] (3,0) node[component] (matj)   {\usedlabel{\pMat}{Jacobian}};
\draw[line width=1pt] (7,0) node[component] (vecs)   {\pVecs \\ \footnotesize  \emph{solution, other fields}};

\path
   ([xshift=2em]rescode.south) edge[->,bend left] node {} ([xshift=-2em]vecs.north)
   ([xshift=1.3em]jaccode.south) edge[->,#2] node {} ([xshift=0em]vecs.north)
   (jaccode.south) edge[->,bend left,#2] node {} (matj)
   ([xshift=-10em]usercode.south) edge[->] node {} (ts)
   ([xshift=2em]usercode.south) edge[->,#3] node {} (dmda)
   ([xshift=10em]usercode.south) edge[->] node {} ([xshift=2em]vecs.north)
   (ts) edge node {} (snes)
   (snes) edge node {} (ksp)
   (ksp) edge node {} (pc);
\end{tikzpicture}
...
\newcommand{\one}{{\large \mathbb{1}}}  % characteristic functions e.g. of elements
