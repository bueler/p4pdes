...
% usage: \standardSNESstack{scale}{objective}{Jacobian}{DMDA}{DMPlex}
%   where last 4 args are either "dashed" or empty
\def \standardSNESstack#1#2#3#4#5{
...
\draw[line width=1pt,dashed] (8.5,4.5) node[component,#5] (dmplex) {\complabel{\pDMPlex}{mesh}};
...
\path
   ...
   ([xshift=14em]usercode.south) edge[->,#5] node {} (dmplex)
...
\draw[line width=1pt] (3,8) node[userfcn,minimum width=80mm] (usercode) {user code \\ \vspace{10mm}};
\draw[line width=1pt] (1,7.7) node[userfcn] (rescode) {right side};
\draw[line width=1pt] (5.5,7.7) node[userfcn,#2] (jaccode) {Jacobian};
...
\draw[line width=1pt] (4,4) node[component,#3] (dmda) {\complabel{\pDMDA}{structured grid}};
\draw[line width=1pt,dashed] (8.5,4.5) node[component,#4] (dmplex) {\complabel{\pDMPlex}{mesh}};
...
\path
   ([xshift=2em]rescode.south) edge[->,bend left] node {} ([xshift=2.5em]vecs.north)
   ([xshift=1em]jaccode.south) edge[->,#2] node {} ([xshift=1em]vecs.north)
   ...
   ([xshift=14em]usercode.south) edge[->,#4] node {} (dmplex)
   ([xshift=9em]usercode.south) edge[->] node {} (vecs)
   (ts) edge node {} (snes)
   (snes) edge node {} (ksp)
   (ksp) edge node {} (pc);
